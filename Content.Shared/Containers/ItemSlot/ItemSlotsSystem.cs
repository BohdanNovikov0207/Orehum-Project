...
public sealed partial class ItemSlotsSystem : EntitySystem
{
    ...
    private void Oninitialize(EntityUid uid, ItemSlotsComponent itemSlots, ComponentInit args)
    {
        ...
        itemSlots.Slots =
            itemSlots.Slots.OrderByDescending(pair => pair.Value.Priority)
                .ToDictionary(); //Goob Edit - Ordered lists are good.
    }
    ...
    private void OnInteractHand(EntityUid uid, ItemSlotsComponent itemSlots, InteractHandEvent args)
    {
        ...
        foreach (var slot in itemSlots.Slots.Values)
        {
            if (!slot.EjectOnInteract || slot.Item == null || !CanEject(uid, args.User, slot, args.User))
                continue;
    ...
    private void OnUseInHand(EntityUid uid, ItemSlotsComponent itemSlots, UseInHandEvent args)
    {
        ...
        foreach (var slot in itemSlots.Slots.Values)
        {
            if (!slot.EjectOnUse || slot.Item == null || !CanEject(uid, args.User, slot, args.User))
                continue;
    ...
    /// <summary>
    /// Insert an item into a slot. This does not perform checks, so make sure to also use
    /// <see
    ///     cref="CanInsert" />
    /// or just use <see cref="TryInsert" /> instead.
    /// </summary>
        ...
        bool swap = false)
    {
        ...
        return _containers.CanInsert(usedUid, slot.ContainerSlot, swap);
    }
        ...
        bool excludeUserAudio = false)
    {
        ...
        Insert(ent, itemSlot, item, user, excludeUserAudio);
        return true;
    }
    ...
    /// False returns any slot that is able to receive <paramref name="item" />.
    /// </param>
    ...
    /// Useful for predicted interactions
    /// </param>
    private void Eject(EntityUid uid,
        ItemSlot slot,
        EntityUid item,
        EntityUid? user,
        bool excludeUserAudio = false,
    ...
    /// </returns>
    public bool TryEjectToHands(EntityUid uid,
        ItemSlot slot,
        EntityUid? user,
        bool excludeUserAudio = false,
        bool doAfter = true)
        ...
        GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        foreach (var slot in itemSlots.Slots.Values)
        {
            ...
            AlternativeVerb verb = new()
            {
                ...
                Act = () => TryEjectToHands(uid, slot, args.User, true),
            };
