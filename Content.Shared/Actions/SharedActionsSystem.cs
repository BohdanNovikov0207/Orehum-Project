...
public abstract class SharedActionsSystem : EntitySystem
{
    ...
    private bool ValidateBaseTarget(EntityUid user, EntityCoordinates coords, Entity<TargetActionComponent> ent)
    {
        ...
        var hasNoSpecificComponents =
            !HasComp<StationAiOverlayComponent>(user) && !HasComp<AbductorScientistComponent>(user); // Shitmed Change
    ...
    /// </summary>
    public bool CanPerformAction(Entity<ActionsComponent?> user,
        Entity<ActionComponent> action,
    ...
    /// <inheritdoc
    ///     cref="AddAction(Robust.Shared.GameObjects.EntityUid,ref System.Nullable{Robust.Shared.GameObjects.EntityUid},string?,Robust.Shared.GameObjects.EntityUid,ActionsComponent?)" />
    public bool AddAction(EntityUid performer,
        [NotNullWhen(true)] ref EntityUid? actionId,
        ...
        Entity<ActionComponent?>? action)
    {
        ...
        DebugTools.Assert(ent.Comp.Container == null ||
                          _net.IsClient && action?.Comp?.ClientExclusive is false || // Goob edit
    ...
    private void OnDidUnequip(EntityUid uid, ActionsComponent component, DidUnequipEvent args)
    {
        ...
        {
            var ev = new GetItemActionsEvent(_actionContainer,
                args.Equipee,
                args.Equipment,
                isEquipping: false); // Lavaland Change - added false for isEquipping
            RaiseLocalEvent(args.Equipment, ev);
