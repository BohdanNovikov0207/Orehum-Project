...
public abstract partial class SharedMoverController
{
    ...
    protected void SetMoveInput(Entity<InputMoverComponent> entity, MoveButtons buttons)
    {
        ...
        var vector2 = DirVecForButtons(buttons);
        var vector2i = new Vector2i((int) vector2.X, (int) vector2.Y);
        var dir = vector2i == Vector2i.Zero ? Direction.Invalid : vector2i.AsDirection();
    ...
    private void OnMoverHandleState(Entity<InputMoverComponent> entity, ref ComponentHandleState args)
    {
        ...
        // Shitmed Change Start
        var vector2 = DirVecForButtons(entity.Comp.HeldMoveButtons);
        var vector2i = new Vector2i((int) vector2.X, (int) vector2.Y);
        var dir = vector2i == Vector2i.Zero ? Direction.Invalid : vector2i.AsDirection();
        // Shitmed Change End
        ...
        {
            var moveEvent =
                new MoveInputEvent(entity,
                    entity.Comp.HeldMoveButtons,
                    dir,
                    state.HeldMoveButtons != 0); // Shitmed Change
    ...
    private bool TryUpdateRelative(EntityUid uid, InputMoverComponent mover, TransformComponent xform)
    {
        ...
        else if (MapGridQuery.HasComp(relative) &&
                 (MapQuery.HasComp(mover.RelativeEntity) || MapGridQuery.HasComp(mover.RelativeEntity)))
        {
    ...
    private sealed class MoverDirInputCmdHandler : InputCmdHandler
    {
            ...
            IFullInputCmdMessage message)
        {
            ...
            _controller.HandleDirChange(session.AttachedEntity.Value,
                _dir,
                message.SubTick,
    ...
    private sealed class WalkInputCmdHandler : InputCmdHandler
    {
            ...
            IFullInputCmdMessage message)
        {
            ...
            _controller.HandleRunChange(session.AttachedEntity.Value,
                message.SubTick,
                message.State == BoundKeyState.Down);
            return false;
        }
    ...
    private sealed class ShuttleInputCmdHandler : InputCmdHandler
    {
            ...
            IFullInputCmdMessage message)
        {
            ...
            _controller.HandleShuttleInput(session.AttachedEntity.Value,
                _button,
                message.SubTick,
                message.State == BoundKeyState.Down);
            return false;
