...
public sealed partial class CCVars
{
    ...
    /// </summary>
    [CVarControl(AdminFlags.Server | AdminFlags.Mapping, 0, int.MaxValue)]
    ...
    /// <summary>
    /// Impulse multiplier for player interactions that move grids (other than shuttle thrusters, gyroscopes and grid
    /// collisons).
    ...
    /// At the moment they have a very low mass of roughly 0.48 kg per tile independent of any walls or anchored objects on
    /// them.
    /// </summary>
    ...
    public static readonly CVarDef<float> MinimumImpactInertia =
        CVarDef.Create("shuttle.impact.minimum_inertia",
            5f * 50f,
            CVar.SERVERONLY); // 100tile grid (cargo shuttle) going at 5 m/s

    /// <summary>
    /// Minimum velocity difference between 2 bodies for a shuttle impact to be guaranteed to trigger any special behaviors
    /// like damage.
    /// </summary>
    [CVarControl(AdminFlags.VarEdit)]
    public static readonly CVarDef<float> MinimumImpactVelocity =
        CVarDef.Create("shuttle.impact.minimum_velocity",
            15f,
            CVar.SERVERONLY); // needed so that random space debris can be rammed
    ...
    public static readonly CVarDef<float> ImpactMinThrowVelocity =
        CVarDef.Create("shuttle.impact.min_throw_velocity",
            1f,
            CVar.SERVERONLY); // due to how it works this is about 16 m/s for cargo shuttle
