...
public sealed partial class CCVars
{
    ...
    /// </summary>
    /// <remarks>
    /// This only takes effect if an explosion needs more than one tick to process (i.e., covers more than
    /// <see
    ///     cref="ExplosionTilesPerTick" />
    /// tiles). If this is not enabled, the node-system will rebuild its graph
    ...
    /// </summary>
    /// <remarks>
    /// Actual area may be larger, as it currently doesn't terminate mid neighbor finding. I.e., area may be that of a ~51 tile
    /// radius circle instead.
    /// </remarks>
    ...
    /// updating the grid tiles should be done incrementally at the end of every tick, or only once the explosion has finished
    /// processing.
    /// </summary>
    /// <remarks>
    ...
    /// processing/damaging occurs in separate ticks. This helps reduce the single-tick lag if both
    /// <see
    ///     cref="ExplosionMaxProcessingTime" />
    /// and <see cref="ExplosionTilesPerTick" /> are large. I.e., instead of
