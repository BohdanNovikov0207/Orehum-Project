...
public abstract partial class SharedToolSystem : EntitySystem
{
    ...
    public void PlayToolSound(EntityUid uid,
        ToolComponent tool,
        EntityUid? user,
        AudioParams? audioParams = null) // Goob - audioParams
    {
    ...
    /// <param name="target">
    /// The entity that the tool is being used on. This is also the entity that will receive the
    /// event. If null, the event will be broadcast
    /// </param>
        ...
        ToolComponent? toolComponent = null)
    {
        ...
        var doAfterArgs = new DoAfterArgs(EntityManager, user, doAfterLength, toolEvent, tool, target, tool)
        {
            BreakOnDamage = true,
            BreakOnMove = true,
            BreakOnWeightlessMove = false,
            NeedHand = tool != user,
            AttemptFrequency = fuel > 0 || toolComponent.AlwaysCheckDoAfter
    ...
    public bool HasQuality(EntityUid uid, [ForbidLiteral] string quality, ToolComponent? tool = null) =>
        Resolve(uid, ref tool, false) && tool.Qualities.Contains(quality);

    /// <summary>
    /// Whether a tool entity has all specified qualities or not.
    /// </summary>
    [PublicAPI]
    public bool HasAllQualities(EntityUid uid,
        [ForbidLiteral] IEnumerable<string> qualities,
        ToolComponent? tool = null) => Resolve(uid, ref tool, false) && tool.Qualities.ContainsAll(qualities);

    private bool CanStartToolUse(EntityUid tool,
        EntityUid user,
        EntityUid? target,
        float fuel,
    ...
    public sealed partial class ToolDoAfterEvent : DoAfterEvent // Goob - Protected -> Public
    {
        ...
        {
            DebugTools.Assert(wrappedEvent.GetType().HasCustomAttribute<NetSerializableAttribute>(),
        ...
        public override bool IsDuplicate(DoAfterEvent other) => other is ToolDoAfterEvent toolDoAfter &&
                                                                WrappedEvent.IsDuplicate(toolDoAfter.WrappedEvent);
    }
