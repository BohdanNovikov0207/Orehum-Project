...
public sealed class StatusEffectsSystem : EntitySystem
{
    ...
    private void OnGetState(EntityUid uid, StatusEffectsComponent component, ref ComponentGetState args) =>
        ...
        args.State = new StatusEffectsComponentState(new Dictionary<string, StatusEffectState>(component.ActiveEffects),
            new List<string>(component.AllowedEffects));
    ...
    private void OnHandleState(EntityUid uid, StatusEffectsComponent component, ref ComponentHandleState args)
    {
        ...
        {
            component.ActiveEffects[key] = new StatusEffectState(effect);
        }
    ...
    /// The time at which the status effect started. This exists mostly for prediction
    /// resetting.
    /// </param>
    ...
    /// will not be useful.
    /// </remarks>
    private (TimeSpan, TimeSpan)? GetAlertCooldown(EntityUid uid,
    ...
    /// <param name="status">The status effects component to change, if you already have it.</param>
    /// <param name="remComp">
    /// If true, status effect removal will also remove the relevant component. This option
    /// exists mostly for prediction resetting.
    /// </param>
        ...
        StatusEffectsComponent? status = null)
    {
        ...
        if (_prototypeManager.TryIndex<StatusEffectPrototype>(key, out var proto)
            && proto.Alert != null)
        {
            var cooldown = GetAlertCooldown(uid, proto.Alert.Value, status);
