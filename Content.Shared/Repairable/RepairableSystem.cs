...
public sealed class RepairableSystem : EntitySystem
{
    ...
    // Goob edit start
    /// <summary>
    /// Method <c>ApplyRepairs</c> Applies repair according to "RepairableComponent" present on entity. Returns false if fail
    /// or nothing else to repair.
    /// </summary>
    ...
    public bool ApplyRepairs(Entity<RepairableComponent> ent, EntityUid user)
    {
        ...
        if (TryComp<BodyComponent>(ent.Owner, out var body) && ent.Comp.Damage != null &&
            body != null) // repair entities with bodies
        {
            ...
            if (TryComp<TargetingComponent>(user, out var targeting))
            {
                ...
                var targetedBodyPart = _bodySystem.GetBodyChildrenOfType(ent, partType, body, symmetry)
                    .ToList()
                    .FirstOrDefault();
            ...
            if (!_healingSystem.IsBodyDamaged((ent.Owner, body),
                    null,
                    repairHealing,
                    targetedWoundable)) // Check if there is anything to heal on the initial limb target
            {
                if (_healingSystem.TryGetNextDamagedPart(ent, repairHealing, out var limbTemp) &&
                    limbTemp is not null) // If not then get the next limb to heal
                    targetedWoundable = limbTemp.Value;
            }
            ...
            {
                var healedBleedWound = false;
                FixedPoint2 modifiedBleedStopAbility = 0;
                healedBleedWound = _wounds.TryHealBleedingWounds(targetedWoundable,
                    repairHealing.BloodlossModifier,
            ...
            var damageChanged =
                _damageableSystem.TryChangeDamage(targetedWoundable, ent.Comp.Damage, true, false, origin: user);
            _adminLogger.Add(LogType.Healed,
                $"{ToPrettyString(user):user} repaired {ToPrettyString(ent.Owner):target} by {damageChanged?.GetTotal()}");
        ...
        {
            var damageChanged =
                _damageableSystem.TryChangeDamage(ent.Owner, ent.Comp.Damage, true, false, origin: user);
            _adminLogger.Add(LogType.Healed,
                $"{ToPrettyString(user):user} repaired {ToPrettyString(ent.Owner):target} by {damageChanged?.GetTotal()}");
        }
        else
        {
            ...
            _adminLogger.Add(LogType.Healed,
                $"{ToPrettyString(user):user} repaired {ToPrettyString(ent.Owner):target} back to full health");
        }
    ...
    private void OnRepairFinished(Entity<RepairableComponent> ent, ref RepairFinishedEvent args)
    {
        if (args.Cancelled)
            return;
        // Goob edit start
        var repairStatus = ApplyRepairs(ent, args.User);
        ...
        if (repairStatus && HasComp<BodyComponent>(ent) && args.Used != null)
        {
            ...
            args.Handled = _toolSystem.UseTool(args.Used.Value,
                args.User,
                ent.Owner,
                delay,
                ent.Comp.QualityNeeded,
                new RepairFinishedEvent(),
                ent.Comp.FuelCost); // args.Repeat doesn't work because this current event is a wrapped event of ToolDoAfterEvent
        }
        // Goob edit end
    ...
    private void Repair(Entity<RepairableComponent> ent, ref InteractUsingEvent args)
    {
        ...
        args.Handled = _toolSystem.UseTool(args.Used,
            args.User,
            ent.Owner,
            delay,
            ent.Comp.QualityNeeded,
