...
public abstract class SharedGasTileOverlaySystem : EntitySystem
{
    ...
    private void OnGetState(EntityUid uid, GasTileOverlayComponent component, ref ComponentGetState args)
    {
        ...
        args.State = new GasTileOverlayDeltaState(data, new HashSet<Vector2i>(component.Chunks.Keys));
    }

    public static Vector2i GetGasChunkIndices(Vector2i indices) => new((int) MathF.Floor((float) indices.X / ChunkSize),
        (int) MathF.Floor((float) indices.Y / ChunkSize));
