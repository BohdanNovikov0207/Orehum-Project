...
public abstract class SharedPopupSystem : EntitySystem
{
    ...
    /// </summary>
    public abstract void PopupPredictedCursor(string? message,
    ...
    /// <param name="recordReplay">
    /// If true, this pop-up will be considered as a globally visible pop-up that gets shown during
    /// replays.
    /// </param>
    public abstract void PopupCoordinates(string? message,
        EntityCoordinates coordinates,
    ...
    /// <summary>
    /// Variant of <see cref="PopupCoordinates(string, EntityCoordinates, PopupType)" /> that sends a pop-up to the player
    /// attached to some entity.
    /// </summary>
    public abstract void PopupCoordinates(string? message,
        EntityCoordinates coordinates,
        EntityUid recipient,
    ...
    /// <summary>
    /// Variant of <see cref="PopupCoordinates(string, EntityCoordinates, PopupType)" /> that sends a pop-up to a specific
    /// player.
    /// </summary>
    public abstract void PopupCoordinates(string? message,
        EntityCoordinates coordinates,
    ...
    /// </summary>
    public abstract void PopupPredictedCoordinates(string? message,
        EntityCoordinates coordinates,
        EntityUid? recipient,
        PopupType type = PopupType.Small);
    ...
    /// <summary>
    /// Variant of <see cref="PopupCursor(string, EntityUid, PopupType)" /> that only runs on the client, outside of
    /// prediction.
    ...
    /// <summary>
    /// Variant of <see cref="PopupEntity(string, EntityUid, EntityUid, PopupType)" /> that only runs on the client, outside of
    /// prediction.
    /// Useful for shared code that is always ran by both sides to avoid duplicate popups.
    /// </summary>
    public abstract void PopupClient(string? message,
        EntityUid uid,
        EntityUid? recipient,
        PopupType type = PopupType.Small);

    /// <summary>
    /// Variant of <see cref="PopupCoordinates(string, EntityCoordinates, PopupType)" /> that only runs on the client, outside
    /// of prediction.
    /// Useful for shared code that is always ran by both sides to avoid duplicate popups.
    /// </summary>
    public abstract void PopupClient(string? message,
    ...
    /// <summary>
    /// Variant of <see cref="PopupEntity(string, EntityUid, EntityUid, PopupType)" /> for use with prediction. The local
    /// client will show
    /// the popup to the recipient, and the server will show it to every other player in PVS range. If recipient is null, the
    /// local client
    /// will do nothing and the server will show the message to every player in PVS range.
    /// </summary>
    public abstract void PopupPredicted(string? message,
        EntityUid uid,
        EntityUid? recipient,
        PopupType type = PopupType.Small);
    ...
    /// <summary>
    /// Variant of <see cref="PopupPredicted(string?, EntityUid, EntityUid?, PopupType)" /> that displays
    /// <paramref name="recipientMessage" />
    /// to the recipient and <paramref name="othersMessage" /> to everyone else in PVS range.
    /// </summary>
    public abstract void PopupPredicted(string? recipientMessage,
...
/// Actions which have different popups for the user vs. others should use a larger popup for the user and a smaller popup
/// for others.
