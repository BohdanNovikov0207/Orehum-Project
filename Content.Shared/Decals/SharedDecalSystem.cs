...
public abstract class SharedDecalSystem : EntitySystem
{
    ...
    public static Vector2i GetChunkIndices(Vector2 coordinates) => new((int) Math.Floor(coordinates.X / ChunkSize),
    ...
    private void OnGetState(EntityUid uid, DecalGridComponent component, ref ComponentGetState args)
    {
        ...
        args.State =
            new DecalGridDeltaState(data, new HashSet<Vector2i>(component.ChunkCollection.ChunkCollection.Keys));
    }

    private void OnGridInitialize(GridInitializeEvent msg) => EnsureComp<DecalGridComponent>(msg.EntityUid);
    ...
    protected bool RemoveDecalInternal(EntityUid gridId,
        uint decalId,
    ...
    protected virtual void OnDecalRemoved(EntityUid gridId,
        uint decalId,
    ...
    public virtual HashSet<(uint Index, Decal Decal)> GetDecalsInRange(EntityUid gridId,
        Vector2 position,
