...
public abstract class SharedCuffableSystem : EntitySystem
{
    ...
    private void OnStartup(EntityUid uid, CuffableComponent component, ComponentInit args) => component.Container =
        _container.EnsureContainer<Container>(uid, Factory.GetComponentName(component.GetType()));
    ...
    private void OnCuffsRemovedFromContainer(EntityUid uid,
    ...
    private void OnBuckleAttempt(Entity<CuffableComponent> ent,
        EntityUid? user,
        ref bool cancelled,
        bool buckling,
        bool popup)
    ...
    /// </summary>
    public bool TryAddNewCuffs(EntityUid target,
        EntityUid user,
        EntityUid handcuff,
    ...
    public bool TryCuffing(EntityUid user,
        EntityUid target,
        EntityUid handcuff,
        ...
        CuffableComponent? cuffable = null)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("handcuff-component-cannot-drop-cuffs",
    ...
    /// <param name="requireFullyCuffed">
    /// when true, return false if the target is only partially cuffed (for things with more
    /// than 2 hands)
    /// </param>
    /// <returns></returns>
    ...
    /// <param name="cuffsToRemove">
    /// Optional param for the handcuff entity to remove from the cuffed entity. If null, uses the
    /// most recently added handcuff entity.
    /// </param>
    /// <param name="cuffable"></param>
    /// <param name="cuff"></param>
    public void TryUncuff(EntityUid target,
        EntityUid user,
    ...
    public void Uncuff(EntityUid target,
        EntityUid? user,
        ...
        if (cuffable.CuffedHandCount == 0)
        {
            if (user != null)
            {
                if (shoved)
                {
                    ...
                    _popup.PopupClient(Loc.GetString("cuffable-component-remove-cuffs-success-message"),
                        user.Value,
                        user.Value);
            }
    ...
    public IReadOnlyList<EntityUid> GetAllCuffs(CuffableComponent component) => component.Container.ContainedEntities;
