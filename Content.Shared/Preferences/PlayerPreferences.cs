...
public sealed class PlayerPreferences
{
    ...
    public PlayerPreferences(IEnumerable<KeyValuePair<int, ICharacterProfile>> characters,
    ...
    public ICharacterProfile GetProfile(int index) => _characters[index];

    public int IndexOfCharacter(ICharacterProfile profile) =>
        _characters.FirstOrNull(p => p.Value == profile)?.Key ?? -1;

    public bool TryIndexOfCharacter(ICharacterProfile profile, out int index) =>
        (index = IndexOfCharacter(profile)) != -1;
}
