...
public sealed class DamageableSystem : EntitySystem
{
    ...
    private readonly ProtoId<DamageGroupPrototype>[] _vitalOnlyDamageTypes =
        { "Airloss", "Toxin", "Genetic", "Metaphysical" }; // Goobstation
        ...
        DamageSpecifier? uncappedDamage = null) // Goobstation
    {
        ...
        RaiseLocalEvent(uid,
            new DamageChangedEvent(component,
                damageDelta,
                interruptsDoAfters,
                origin,
                ignoreBlockers,
                uncappedDamage)); // Goob edit
    }
        ...
        bool canMiss = true)
    {
            ...
            && body.BodyType == BodyType.Complex)
        {
            ...
            var appliedDamage = ApplyDamageToBodyParts(uid.Value,
                damage,
                origin,
                ignoreResistances,
                interruptsDoAfters,
            ...
            // Goobstation start
            var appliedVitalDamage = ApplyDamageToBodyParts(uid.Value,
                vitalDamage,
                origin,
                ignoreResistances,
                interruptsDoAfters,
        ...
        return ApplyDamageToEntity(uid.Value,
            damage,
            ignoreResistances,
            interruptsDoAfters,
            origin,
        ...
        bool canMiss = true)
    {
            ...
            && targetPart != 0 && (targetPart & (targetPart - 1)) != 0)
        {
            ...
            foreach (var flag in primitiveFlags)
            {
                ...
                if (targetPart.Value.HasFlag(flag))
                {
                    ...
                    var parts = _body.GetBodyChildrenOfTypeWithComponent<DamageableComponent>(uid,
                            query.Type,
            ...
            for (var i = 0; i < targetedBodyParts.Count; i++)
            {
                ...
                var partDamageResult = TryChangeDamage(partId,
                    modifiedDamage,
                    ignoreResistances,
                    interruptsDoAfters,
                    partDamageable,
                ...
                if (partDamageResult != null && !partDamageResult.Empty)
                {
                    ...
                    */
                    foreach (var (type, damageFromDict) in modifiedDamage.DamageDict)
                    {
                        ...
                        {
                            if (surplusHealing.DamageDict.TryGetValue(type, out _))
        ...
        else
        {
            ...
            totalAppliedDamage = TryChangeDamage(chosenTarget.Id,
                adjustedDamage,
                ignoreResistances,
                interruptsDoAfters,
                partDamageable,
        ...
        bool ignoreBlockers = false)
    {
        ...
        if (!Resolve(bodyPartUid, ref bodyPart, false)
    ...
    public void ChangeDamageContainer(EntityUid uid, string newDamageContainerId, DamageableComponent? component = null)
    {
        ...
        if (!_prototypeManager.TryIndex<DamageContainerPrototype>(newDamageContainerId,
    ...
    {
        if (_netMan.IsServer)
            args.State = new DamageableComponentState(component.Damage.DamageDict,
        ...
        else
        {
            ...
            args.State = new DamageableComponentState(component.Damage.DamageDict.ShallowClone(),
                component.DamageContainerID,
                component.DamageModifierSetId,
                component.HealthBarThreshold);
        }
    ...
    private void OnRejuvenate(EntityUid uid, DamageableComponent component, RejuvenateEvent args)
    {
        ...
        _mobThreshold.SetAllowRevives(uid,
            true,
            thresholds); // do this so that the state changes when we set the damage
...
public sealed class DamageModifyEvent : EntityEventArgs, IInventoryRelayEvent
{
    ...
    public DamageModifyEvent(EntityUid target,
        DamageSpecifier damage,
        EntityUid? origin = null,
        TargetBodyPart? targetPart = null) // Shitmed + Goobstation Change
    {
...
public sealed class DamageChangedEvent : EntityEventArgs
{
    ...
    public DamageChangedEvent(DamageableComponent damageable,
        ...
        EntityUid? origin,
        bool ignoreBlockers = false,
        DamageSpecifier? uncapped = null) // Shitmed Change, Goob edit
    {
