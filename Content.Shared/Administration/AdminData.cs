...
public sealed class AdminData
{
    ...
    public bool HasFlag(AdminFlags flag, bool includeDeAdmin = false) =>
        (includeDeAdmin || Active) && (Flags & flag) == flag;

    /// <summary>
    /// Check if this admin can spawn stuff in with the entity/tile spawn panel.
    /// </summary>
    public bool CanAdminPlace() => HasFlag(AdminFlags.Spawn);

    /// <summary>
    /// Check if this admin can execute server-side C# scripts.
    /// </summary>
    public bool CanScript() => HasFlag(AdminFlags.Host);

    /// <summary>
    /// Check if this admin can open the admin menu.
    /// </summary>
    public bool CanAdminMenu() => HasFlag(AdminFlags.Admin);

    /// <summary>
    /// Check if this admin can be hidden and see other hidden admins.
    /// </summary>
    public bool CanStealth() => HasFlag(AdminFlags.Stealth);

    public bool CanAdminReloadPrototypes() => HasFlag(AdminFlags.Host);
}
