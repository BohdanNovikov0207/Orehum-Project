...
public sealed partial class Solution : IEnumerable<ReagentQuantity>, ISerializationHooks, IRobustCloneable<Solution>
{
    ...
    public Solution Clone() => new(this);
    ...
    /// </summary>
    public bool CanAddSolution(Solution solution) => solution.Volume <= AvailableVolume;
    ...
    public float GetThermalEnergy(IPrototypeManager? protoMan) => GetHeatCapacity(protoMan) * Temperature;
    ...
    public bool ContainsReagent(string reagentId, List<ReagentData>? data)
        => ContainsReagent(new ReagentId(reagentId, data));
    ...
    /// </summary>
    [Pure]
    public FixedPoint2 GetReagentQuantity(ReagentId id) => GetReagent(id).Quantity;
    ...
    /// <param name="quantity">The quantity in milli-units.</param>
    public void AddReagent(ReagentPrototype proto,
        ...
        bool ignoreReagentData = false)
    {
        ...
        var reagentIndices = new List<int>();
        var totalRemoveVolume = 0;
    ...
    /// <returns>How much reagent was actually removed. Zero if the reagent is not present on the solution.</returns>
    public FixedPoint2 RemoveReagent(string prototype,
        FixedPoint2 quantity,
        List<ReagentData>? data = null,
        bool ignoreReagentData = false) => RemoveReagent(new ReagentQuantity(prototype, quantity, data),
        ignoreReagentData: ignoreReagentData);
    ...
    /// <param name="quantity">The amount of reagent to remove.</param>
    /// <returns>How much reagent was actually removed. Zero if the reagent is not present on the solution.</returns>
    public FixedPoint2 RemoveReagent(ReagentId reagentId,
        FixedPoint2 quantity,
        bool preserveOrder = false,
        bool ignoreReagentData = false) =>
        RemoveReagent(new ReagentQuantity(reagentId, quantity), preserveOrder, ignoreReagentData);
    ...
    /// <summary>
    /// Variant of <see cref="SplitSolution(FixedPoint2)" /> that doesn't return a new solution containing the removed
    /// reagents.
    /// </summary>
    ...
    public void AddSolution(Solution otherSolution, IPrototypeManager? protoMan)
    {
        ...
        {
            if (freshestDnasFromOther.TryGetValue(dna.DNA, out var fresherFreshness) &&
                fresherFreshness > dna.Freshness)
    ...
    public Color GetColor(IPrototypeManager? protoMan) => GetColorWithout(protoMan);
    ...
    public IEnumerator<ReagentQuantity> GetEnumerator() => Contents.GetEnumerator();
