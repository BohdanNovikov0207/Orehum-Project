...
public abstract partial class SharedInteractionSystem : EntitySystem
{
    ...
    /// </summary>
    private void OnBoundInterfaceInteractAttempt(Entity<UserInterfaceComponent> ent,
        ref BoundUserInterfaceMessageAttempt ev)
    {
        _uiQuery.TryComp(ev.Target, out var aUiComp);

        // CorvaxGoob-GhostUIViewing-Start
        if (TryComp<GhostComponent>(ev.Actor, out var ghost) &&
            !ghost.CanGhostInteract) // Полностью игнорить если не гост или который может прямо взамоимодействовать с UI
        {
    ...
    public bool HandleAltUseInteraction(ICommonSession? session, EntityCoordinates coords, EntityUid uid)
    {
        ...
        UserInteraction(user.Value, coords, uid, true, checkAccess: ShouldCheckAccess(user.Value));
    ...
    public bool HandleUseInteraction(ICommonSession? session, EntityCoordinates coords, EntityUid uid)
    {
        ...
        UserInteraction(userEntity.Value,
            coords,
    ...
    private bool ShouldCheckAccess(EntityUid user) =>
        ...
        !_tagSystem.HasTag(user, BypassInteractionRangeChecksTag);
    ...
    public bool CombatModeCanHandInteract(EntityUid user, EntityUid? target)
    {
        ...
        if (target == null || !_handsQuery.TryComp(user, out var hands) ||
            _hands.GetActiveItem((user, hands)) is not null)
            return false;
        ...
        bool checkCanUse = true)
    {
        ...
        var inRangeUnobstructed = target == null
            ...
            : !checkAccess ||
              InRangeUnobstructed(user, target.Value); // permits interactions with wall mounted entities
        ...
        if (target == used)
        {
            UseInHandInteraction(user, target.Value, false, false);
            return;
        }
    ...
    private bool IsDeleted(EntityUid uid) => TerminatingOrDeleted(uid) || EntityManager.IsQueuedForDeletion(uid);
    ...
    public bool InteractHand(EntityUid user, EntityUid target) // Goobstation - useful return value
    {
        ...
        if (ev.Handled)
        {
            _adminLogger.Add(LogType.InteractHand,
                LogImpact.Low,
        ...
        _adminLogger.Add(LogType.InteractHand,
            LogImpact.Low,
            $"{ToPrettyString(user):user} interacted with {ToPrettyString(target):target}");
    ...
    public void InteractUsingRanged(EntityUid user,
        EntityUid used,
        EntityUid? target,
        ...
        bool inRangeUnobstructed)
    {
        ...
        if (RangedInteractDoBefore(user, used, target, clickLocation, inRangeUnobstructed, false))
            return;
        ...
        DebugTools.Assert(!IsDeleted(user) && !IsDeleted(used) && !IsDeleted(target));
        InteractDoAfter(user, used, target, clickLocation, inRangeUnobstructed, false);
    }
    ...
    protected bool ValidateInteractAndFace(EntityUid user, EntityCoordinates coordinates)
    {
        ...
        if (!HasComp<NoRotateOnInteractComponent>(user) && (!TryComp(user, out InputMoverComponent? mover) ||
                                                            (mover.HeldMoveButtons & MoveButtons.AnyDirection) == 0x0))
        ...
        Ignored? predicate = null)
    {
        ...
        var rayResults = _broadphase.IntersectRayWithPredicate(origin.MapId, ray, dir.Length(), predicate.Invoke, false)
            .ToList();
    ...
    /// If true, if the broadphase query returns an overlap (0f distance) this function will early
    /// out true with no raycast made.
    /// </param>
    public bool InRangeUnobstructed(
        Entity<TransformComponent?> origin,
        Entity<TransformComponent?> other,
        ...
        bool overlapCheck = true)
    {
        ...
        var targetRot = _transform.GetWorldRotation(otherCoordinates.EntityId) + otherAngle;
            ...
            Resolve(origin, ref origin.Comp))
        {
            ...
            else
            {
                ...
                originPos = _transform.GetMapCoordinates(origin, origin.Comp);
        ...
        if (inRange)
        {
            ...
            inRange = InRangeUnobstructed(originPos,
                targetPos,
                range,
                collisionMask,
                rayPredicate,
        ...
        Ignored? predicate = null)
    {
        ...
        if (_itemQuery.HasComp(target) && _physicsQuery.TryComp(target, out var physics) && physics.CanCollide)
        {
            ...
            foreach (var otherEnt in _lookup.GetEntitiesInRange(target, 0.01f, LookupFlags.Static))
            {
                if (target == otherEnt ||
    ...
    public bool InRangeUnobstructed(
        ...
        Ignored? predicate = null,
        bool popup = false) =>
        InRangeUnobstructed(origin, _transform.ToMapCoordinates(other), range, collisionMask, predicate, popup);
        ...
        bool popup = false)
    {
        ...
        var inRange = InRangeUnobstructed(originPosition,
            other,
            range,
            collisionMask,
            combinedPredicate,
    ...
    /// <param name="target">
    /// Entity getting interacted with by the <paramref name="user" /> using the
    /// <paramref name="used" /> entity.
    /// </param>
    /// <param name="clickLocation">The location that the <paramref name="user" /> clicked.</param>
    /// <param name="checkCanInteract">
    /// Whether to check that the <paramref name="user" /> can interact with the
    /// <paramref name="target" />.
    /// </param>
    /// <param name="checkCanUse">
    /// Whether to check that the <paramref name="user" /> can use the
    /// <paramref name="used" /> entity.
    /// </param>
        ...
        bool checkCanUse = true)
    {
        ...
        if (RangedInteractDoBefore(user, used, target, clickLocation, true, false))
            return true;
        ...
        if (InteractDoAfter(user, used, target, clickLocation, true, false))
            return true;
    ...
    /// </param>
    /// <param name="canReach">
    /// Whether the <paramref name="user" /> is in range of the <paramref name="target" />.
    /// </param>
    /// <returns>True if the interaction was handled. Otherwise, false.</returns>
    public bool InteractDoAfter(EntityUid user,
        EntityUid used,
        EntityUid? target,
        EntityCoordinates clickLocation,
        bool canReach,
    ...
    public bool SupportsComplexInteractions(EntityUid user) => _actionBlockerSystem.CanComplexInteract(user);
        ...
        bool checkDeletion = true)
    {
        ...
        if (activateMsg.Handled)
        {
            ...
            if (!activateMsg.WasLogged)
                _adminLogger.Add(LogType.InteractActivate,
                    LogImpact.Low,
...
/// <summary>
/// Override event raised directed on a user to check InRangeUnoccluded AND InRangeUnobstructed to the target if you
