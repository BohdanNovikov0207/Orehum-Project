...
public sealed class UdderSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var udder))
        {
            ...
            _solutionContainerSystem.TryAddReagent(udder.Solution.Value,
                udder.ReagentId,
    ...
    private void AttemptMilk(Entity<UdderComponent?> udder, EntityUid userUid, EntityUid containerUid)
    {
        ...
        var doargs = new DoAfterArgs(EntityManager, userUid, 5, new MilkingDoAfterEvent(), udder, udder, containerUid)
        {
            BreakOnMove = true,
            BreakOnDamage = true,
    ...
    private void OnDoAfter(Entity<UdderComponent> entity, ref MilkingDoAfterEvent args)
    {
        ...
        _popupSystem.PopupClient(Loc.GetString("udder-system-success",
                ("amount", quantity),
