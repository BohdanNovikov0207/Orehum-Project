...
public abstract partial class SharedHandsSystem
{
    ...
    protected virtual void HandleEntityInserted(EntityUid uid,
    ...
    /// <summary>
    /// Tries to pick up an entity to a specific hand. If no explicit hand is specified, defaults to using the currently active
    /// hand.
    /// </summary>
        ...
        ItemComponent? item = null)
    {
        ...
        if (animate)
        {
            ...
            if (itemPos.MapId == xform.MapID
                && (itemPos.Position - TransformSystem.GetMapCoordinates(uid, xform).Position).Length() <=
                MaxAnimationRange
    ...
    /// By default it does check if it's possible to drop items
    /// </summary>
    public bool TryForcePickupAnyHand(EntityUid uid,
        EntityUid entity,
        bool checkActionBlocker = true,
        HandsComponent? handsComp = null,
        ItemComponent? item = null)
    ...
    /// <summary>
    /// Checks whether a given item will fit into a specific user's hand. Unless otherwise specified, this will also check the
    /// general CanPickup action blocker.
    /// </summary>
    ...
    /// <summary>
    /// Puts an entity into the player's hand, assumes that the insertion is allowed. In general, you should not be calling
    /// this function directly.
    /// </summary>
    public virtual void DoPickup(EntityUid uid,
        string hand,
        EntityUid entity,
        ...
        bool log = true)
    {
        ...
        {
            Log.Error(
                $"Failed to insert {ToPrettyString(entity)} into users hand container when picking up. User: {ToPrettyString(uid)}. Hand: {hand}.");
            return;
        }

        _interactionSystem.DoContactInteraction(uid,
            entity,
            null,
            true); //Possibly fires twice if manually picked up via interacting with the object

        if (log)
            _adminLogger.Add(LogType.Pickup,
                LogImpact.Low,
                $"{ToPrettyString(uid):user} picked up {ToPrettyString(entity):entity}");
