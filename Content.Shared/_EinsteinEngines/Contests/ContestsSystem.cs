...
// Goobstation end
    ...
    public float MassContest(EntityUid performerUid,
    ...
    public float MassContest(PhysicsComponent performerPhysics,
        bool bypassClamp = false,
        float rangeFactor = 1f,
        float otherMass = AverageMass)
    {
    ...
    /// <summary>
    /// Outputs the ratio of mass between a performer and a target, accepts either EntityUids or PhysicsComponents in any
    /// combination
    /// If you have physics components already in your function, use <see cref="MassContest(PhysicsComponent, float)" />
    /// instead
    /// </summary>
    ...
    public float HealthContest(EntityUid performer, EntityUid target, bool bypassClamp = false, float rangeFactor = 1f)
    {
        ...
        return _allowClampOverride && bypassClamp
            ...
            : (1 - Math.Clamp(perfDamage.TotalDamage.Float() / perfThreshold.Value.Float(), 0, 0.25f * rangeFactor))
              / (1 - Math.Clamp(targetDamage.TotalDamage.Float() / targetThreshold.Value.Float(),
                  0,
                  0.25f * rangeFactor));
    }
    ...
    /// <summary>
    /// These cannot be implemented until AFTER the psychic refactor, but can still be factored into other systems before that
    /// point.
        ...
        bool sumOrMultiply = false)
    {
        ...
        return sumOrMultiply
            ...
            : MassContest(performer, bypassClampMass, rangeFactorMass) * massMultiplier
                                                                       * StaminaContest(performer,
                                                                           bypassClampStamina,
                                                                           rangeFactorStamina) * staminaMultiplier
                                                                       * HealthContest(performer,
                                                                           bypassClampHealth,
                                                                           rangeFactorHealth) * healthMultiplier
                                                                       * MindContest(performer,
                                                                           bypassClampMind,
                                                                           rangeFactorMind) * mindMultiplier;
    }
