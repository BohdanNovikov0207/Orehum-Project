...
public sealed class HeightAdjustSystem : EntitySystem
{
    ...
    public bool SetScale(EntityUid uid, float scale) => SetScale(uid, new Vector2(scale, scale));
    ...
    public bool SetScale(EntityUid uid, Vector2 scale)
    {
        ...
        if (_config.GetCVar(CCVars.HeightAdjustModifiesZoom) &&
            EntityManager.TryGetComponent<ContentEyeComponent>(uid, out var eye))
            _eye.SetMaxZoom(uid, eye.MaxZoom * avg);
        else
            succeeded = false;

        if (_config.GetCVar(CCVars.HeightAdjustModifiesHitbox) &&
            EntityManager.TryGetComponent<FixturesComponent>(uid, out var fixtures))
        {
            foreach (var fixture in fixtures.Fixtures)
            {
                _physics.SetRadius(uid,
                    fixture.Key,
                    fixture.Value,
                    fixture.Value.Shape,
                    MathF.MinMagnitude(fixture.Value.Shape.Radius * avg, 0.49f));
            }
        }
        else
            succeeded = false;
