...
public abstract class SharedHandLabelerSystem : EntitySystem
{
    ...
    private void OnUtilityVerb(EntityUid uid, HandLabelerComponent handLabeler, GetVerbsEvent<UtilityVerb> args)
    {
        if (args.Target is not { Valid: true } target ||
            _whitelistSystem.IsWhitelistFail(handLabeler.Whitelist, target) || !args.CanAccess)
            return;

        var labelerText = handLabeler.AssignedLabel == string.Empty
    ...
    {
        if (args.Target is not { Valid: true } target ||
            _whitelistSystem.IsWhitelistFail(handLabeler.Whitelist, target) || !args.CanReach)
            return;
    ...
    private void OnHandLabelerLabelChanged(EntityUid uid,
