...
public sealed class GlueSystem : EntitySystem
{
    ...
    private bool TryGlue(Entity<GlueComponent> entity, EntityUid target, EntityUid actor)
    {
        ...
        if (HasComp<GluedComponent>(target) || !HasComp<ItemComponent>(target) ||
            HasComp<UnremoveableComponent>(target))
        {
        ...
        if (HasComp<ItemComponent>(target) &&
            _solutionContainer.TryGetSolution(entity.Owner, entity.Comp.Solution, out var solutionEntity, out _))
        {
            var quantity = _solutionContainer.RemoveReagent(solutionEntity.Value,
                entity.Comp.Reagent,
