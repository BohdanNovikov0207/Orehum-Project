...
public sealed class SecretStashSystem : EntitySystem
{
    ...
    private void OnInit(Entity<SecretStashComponent> entity, ref ComponentInit args) => entity.Comp.ItemContainer =
        _containerSystem.EnsureContainer<ContainerSlot>(entity, "stash", out _);
        ...
        if (IsStashOpen(entity))
        {
            if (item != null)
            {
                ...
                if (HasItemInside(entity))
                {
                    ...
                    itemVerb.Message = Loc.GetString("comp-secret-stash-verb-insert-message-item-already-inside",
                        ("stashname", stashName));
                }
                else
                    itemVerb.Message = Loc.GetString("comp-secret-stash-verb-insert-message-no-item",
                        ("item", item),
                        ("stashname", stashName));
            ...
            else
            {
                ...
                itemVerb.Message = Loc.GetString("comp-secret-stash-verb-take-out-message-something",
                    ("stashname", stashName));
                if (!HasItemInside(entity))
                {
                    itemVerb.Disabled = true;
                    itemVerb.Message = Loc.GetString("comp-secret-stash-verb-take-out-message-nothing",
                        ("stashname", stashName));
                }
    ...
    /// and false otherwise.
    /// </returns>
    private bool IsStashOpen(Entity<SecretStashComponent> stash) => _toolOpenableSystem.IsOpen(stash);

    private bool HasItemInside(Entity<SecretStashComponent> entity) =>
        entity.Comp.ItemContainer.ContainedEntity != null;
