...
- For fork-only behavior, prefer extending `_Orehum` or another clearly fork-scoped area instead of hiding fork logic in unrelated upstream files.
...
- Keep edit-marker ranges as narrow as practical. For files that do not use `//` comments, use the native comment syntax while preserving `Orehum edit start` and `Orehum edit end`.
