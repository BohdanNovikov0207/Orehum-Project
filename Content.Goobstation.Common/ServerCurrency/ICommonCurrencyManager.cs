...
{
    event Action? ClientBalanceChange;
    event Action<PlayerBalanceChangeEvent>? BalanceChange;
    ...
    /// <summary>
    /// Converts an integer to a string representing the count followed by the appropriate currency localization (singular or
    /// plural) defined in server_currency.ftl.
    ...
    int AddCurrency(NetUserId userId, int amount);
    ...
    /// <returns>An integer containing the new amount of currency attributed to the player.</returns>
    int RemoveCurrency(NetUserId userId, int amount);
    ...
    (int, int) TransferCurrency(NetUserId sourceUserId, NetUserId targetUserId, int amount);
    ...
    int SetBalance(NetUserId userId, int amount);
    ...
    int GetBalance(NetUserId? userId = null);
}
