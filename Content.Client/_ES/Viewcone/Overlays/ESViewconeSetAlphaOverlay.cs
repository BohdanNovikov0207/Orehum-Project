...
/// This alpha pass only works because of <see cref="ESViewconeResetAlphaOverlay" />, which resets in a later stage of
/// rendering.
/// </summary>
public sealed class ESViewconeSetAlphaOverlay : Overlay
{
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        var eyeRot =
            cone.ViewAngle -
            eye.Rotation; // subtract rotation cuz idk. the lerp adds it but this doesnt want it for some reason idk.
        ...
        foreach (var entry in occludables)
        {
            ...
            var angleAlpha = (float) Math.Clamp(Math.Abs(angleDist.Theta) - radConeAngle * 0.5f + radConeFeather * 0.5f,
                0f,
                radConeFeather) / radConeFeather;
            var distAlpha = Math.Clamp(distLength - cone.ConeIgnoreRadius + cone.ConeIgnoreFeather * 0.5f,
                0f,
                cone.ConeIgnoreFeather) / cone.ConeIgnoreFeather;
