...
public sealed class BarControl : Control
{
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        var fillLeft = _fill * Rows;
        var rowHeight = Size.Y / Rows;
        // Draw by rows, bottom to top.
        var alt = false;
        for (var row = Rows - 1; row >= 0; row--)
        {
            var fill = alt ? FillColor : AltFillColor;
            var empty = alt ? EmptyColor : AltEmptyColor;
            var rowFill = MathF.Min(fillLeft, 1);
            fillLeft -= 1;
            var topLeft = new Vector2(0, rowHeight * row);
            var bottomRight = topLeft + new Vector2(Size.X, Size.Y / Rows);
...
{
    private readonly Texture _emptyTex;
    private readonly Texture _fillTex;
    private readonly Texture _markTex;
    ...
    public FluxBarControl()
    {
        var spriteSys = IoCManager.Resolve<IEntitySystemManager>().GetEntitySystem<SpriteSystem>();
        _emptyTex = spriteSys.Frame0(
            new SpriteSpecifier.Texture(new ResPath("/Textures/_White/Interface/gun-flux-readout-empty.png")));
        _markTex = spriteSys.Frame0(
            new SpriteSpecifier.Texture(new ResPath("/Textures/_White/Interface/gun-flux-readout-mark.png")));
        _fillTex = spriteSys.Frame0(
            new SpriteSpecifier.Texture(new ResPath("/Textures/_White/Interface/gun-flux-readout-fill.png")));
    }
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        if (_safeLimit is { } safe)
            handle.DrawLine(new Vector2(safe * Width, 0), new Vector2(safe * Width, Height), SafeLimitColor);


        UIBox2 SizeBox(UIBox2 box)
        {
            return new UIBox2(Vector2.Zero, box.Size);
        }
    }
