...
public sealed class PathfindingSystem : SharedPathfindingSystem
{
    ...
    public PathfindingDebugMode Modes
    {
        ...
        set
        {
            ...
            if (value == PathfindingDebugMode.None)
            {
                ...
                overlayManager.AddOverlay(new PathfindingOverlay(EntityManager,
                    _eyeManager,
                    _inputManager,
                    _mapManager,
                    _cache,
    ...
    private void OnRoute(PathRouteMessage ev) => Routes.Add((_timing.RealTime + TimeSpan.FromSeconds(0.5), ev));

    private void OnPolys(PathPolysMessage ev) => Polys = ev.Polys;
    ...
    private void OnBreadcrumbs(PathBreadcrumbsMessage ev) => Breadcrumbs = ev.Breadcrumbs;
...
public sealed class PathfindingOverlay : Overlay
{
    ...
    private void DrawScreen(OverlayDrawArgs args, DrawingHandleScreen screenHandle)
    {
        ...
        var aabb = new Box2(mouseWorldPos.Position - SharedPathfindingSystem.ChunkSizeVec,
            ...
            mouseWorldPos.MapId == args.MapId)
        {
            ...
            foreach (var grid in _grids)
            {
                ...
                if (found || !_system.Breadcrumbs.TryGetValue(netGrid, out var crumbs) ||
                    !xformQuery.TryGetComponent(grid, out var gridXform))
                    continue;

                var (_, _, worldMatrix, invWorldMatrix) =
                    _transformSystem.GetWorldPositionRotationMatrixWithInv(gridXform);
                var localAABB =
                    invWorldMatrix.TransformBox(aabb.Enlarged(float.Epsilon - SharedPathfindingSystem.ChunkSize));
                ...
                foreach (var chunk in crumbs)
                {
                    ...
                    {
                        var crumbMapPos = Vector2.Transform(_system.GetCoordinate(chunk.Key, crumb.Coordinates),
                            worldMatrix);
        ...
        if ((_system.Modes & PathfindingDebugMode.Poly) != 0x0 &&
            mouseWorldPos.MapId == args.MapId)
        {
            if (!_mapManager.TryFindGridAt(mouseWorldPos, out var gridUid, out var grid) ||
    ...
    private void DrawWorld(OverlayDrawArgs args, DrawingHandleWorld worldHandle)
    {
            ...
            foreach (var grid in _grids)
            {
                ...
                foreach (var chunk in crumbs)
                {
                    ...
                    foreach (var crumb in chunk.Value)
                    {
                        ...
                        worldHandle.DrawRect(new Box2(coordinate - edgeVec, coordinate + edgeVec),
                        ...
                        foreach (var poly in tile.Value)
                        {
                            foreach (var neighborPoly in poly.Neighbors)
                            {
                                ...
                                if (neighborPoly.NetEntity != poly.GraphUid)
                                {
                                    color = Color.Green;
                                    var neighborMap =
                                        _transformSystem.ToMapCoordinates(_entManager.GetCoordinates(neighborPoly));
        ...
        if ((_system.Modes & PathfindingDebugMode.Routes) != 0x0)
        {
            foreach (var route in _system.Routes)
            {
                ...
                {
                    if (!_entManager.TryGetComponent<TransformComponent>(_entManager.GetEntity(node.GraphUid),
                            out var graphXform))
                        continue;
