...
public abstract class CartridgeLoaderBoundUserInterface : BoundUserInterface
{
    ...
    {
        var message =
            new CartridgeLoaderUiMessage(_entManager.GetNetEntity(cartridgeUid), CartridgeUiMessageAction.Activate);
        SendMessage(message);
    }
    ...
    protected void DeactivateActiveCartridge()
    {
        ...
        var message = new CartridgeLoaderUiMessage(_entManager.GetNetEntity(_activeProgram.Value),
    ...
    {
        var message =
            new CartridgeLoaderUiMessage(_entManager.GetNetEntity(cartridgeUid), CartridgeUiMessageAction.Install);
        SendMessage(message);
    }
    ...
    {
        var message =
            new CartridgeLoaderUiMessage(_entManager.GetNetEntity(cartridgeUid), CartridgeUiMessageAction.Uninstall);
        SendMessage(message);
    }
    ...
    protected CartridgeComponent? RetrieveCartridgeComponent(EntityUid? cartridgeUid) =>
        EntMan.GetComponentOrNull<CartridgeComponent>(cartridgeUid);

    private void SendCartridgeUiReadyEvent(EntityUid cartridgeUid)
    {
        var message =
            new CartridgeLoaderUiMessage(_entManager.GetNetEntity(cartridgeUid), CartridgeUiMessageAction.UIReady);
        SendMessage(message);
    }
