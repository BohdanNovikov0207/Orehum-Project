...
public sealed class PowerMonitoringConsoleNavMapControl : NavMapControl
{
    ...
    private readonly Vector2[] _powerCableOffsets = { new(-0.2f, -0.2f), Vector2.Zero, new(0.2f, 0.2f) };
    ...
    public void DrawCableNetwork(DrawingHandleScreen handle,
        ...
        Color modulator)
    {
        ...
        if (WorldRange / WorldMaxRange > 0.5f)
        {
            ...
            for (var cableNetworkIdx = 0; cableNetworkIdx < cableNetworks.Length; cableNetworkIdx++)
            {
        ...
        else
        {
            ...
            foreach (var line in fullCableNetwork)
            {
                ...
                var leftTop = ScalePosition(new Vector2(Math.Min(line.Origin.X, line.Terminus.X) - 0.1f,
                        Math.Min(line.Origin.Y, line.Terminus.Y) - 0.1f)
                    + cableOffset - offset);

                var rightTop = ScalePosition(new Vector2(Math.Max(line.Origin.X, line.Terminus.X) + 0.1f,
                        Math.Min(line.Origin.Y, line.Terminus.Y) - 0.1f)
                    + cableOffset - offset);

                var leftBottom = ScalePosition(new Vector2(Math.Min(line.Origin.X, line.Terminus.X) - 0.1f,
                        Math.Max(line.Origin.Y, line.Terminus.Y) + 0.1f)
                    + cableOffset - offset);

                var rightBottom = ScalePosition(new Vector2(Math.Max(line.Origin.X, line.Terminus.X) + 0.1f,
                        Math.Max(line.Origin.Y, line.Terminus.Y) + 0.1f)
                    + cableOffset - offset);
            ...
            for (var cableNetworkIdx = 0; cableNetworkIdx < cableVertexUVs.Length; cableNetworkIdx++)
            {
    ...
    public List<PowerMonitoringConsoleLine> GetDecodedPowerCableChunks(Dictionary<Vector2i, PowerCableChunk>? chunks)
    {
                ...
                for (var chunkIdx = 0; chunkIdx < ChunkSize * ChunkSize; chunkIdx++)
                {
                    ...
                    if (relativeTile.X == ChunkSize - 1)
                    {
                        neighbor = chunks.TryGetValue(chunkOrigin + new Vector2i(1, 0), out neighborChunk) &&
                                   (neighborChunk.PowerCableData[cableIdx] &
                                    GetFlag(new Vector2i(0, relativeTile.Y))) != 0x0;
                    }
                    else
                    {
                    ...
                    if (relativeTile.Y == ChunkSize - 1)
                    {
                        neighbor = chunks.TryGetValue(chunkOrigin + new Vector2i(0, 1), out neighborChunk) &&
                                   (neighborChunk.PowerCableData[cableIdx] &
                                    GetFlag(new Vector2i(relativeTile.X, 0))) != 0x0;
                    }
                    else
                    {
        ...
        for (var index = 0; index < _horizLines.Length; index++)
        {
            ...
            {
                decodedOutput.Add(new PowerMonitoringConsoleLine(origin + gridOffset,
                    terminal + gridOffset,
