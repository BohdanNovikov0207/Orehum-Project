...
public sealed class ConstructionPlacementHijack : PlacementHijack
{
    ...
    public ConstructionPrototype? CurrentPrototype { get; }
    ...
    public override bool HijackPlacementRequest(EntityCoordinates coordinates)
    {
        if (CurrentPrototype != null)
        {
            ...
            _constructionSystem.SpawnGhost(CurrentPrototype, coordinates, dir);
        }
    ...
    public override void StartHijack(PlacementManager manager)
    {
        ...
        if (CurrentPrototype is null ||
            !_constructionSystem.TryGetRecipePrototype(CurrentPrototype.ID, out var targetProtoId))
            return;

        if (!IoCManager.Resolve<IPrototypeManager>().TryIndex(targetProtoId, out EntityPrototype? proto))
            return;

        manager.CurrentTextures =
            SpriteComponent.GetPrototypeTextures(proto, IoCManager.Resolve<IResourceCache>()).ToList();
    }
}
