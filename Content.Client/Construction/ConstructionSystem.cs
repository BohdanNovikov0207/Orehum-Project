...
public sealed class ConstructionSystem : SharedConstructionSystem
{
    ...
    private void HandleGhostComponentShutdown(EntityUid uid,
        ...
        ComponentShutdown args) => ClearGhost(component.GhostId);
    ...
    private void WarmupRecipesCache()
    {
        foreach (var constructionProto in PrototypeManager.EnumeratePrototypes<ConstructionPrototype>())
        {
            ...
            do
            {
                ...
                if (node.Entity.GetId(null, null, new GraphNodeEntityArgs(EntityManager)) is not { } entityId)
                {
        ...
        [NotNullWhen(true)] out EntityUid? ghost)
    {
        ...
        if (!CheckConstructionConditions(prototype, loc, dir, user, true))
            return false;
