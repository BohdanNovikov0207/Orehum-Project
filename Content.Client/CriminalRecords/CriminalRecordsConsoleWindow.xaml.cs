...
{
    private static readonly ProtoId<LocalizedDatasetPrototype> ReasonPlaceholders =
        "CriminalRecordsWantedReasonPlaceholders";
    ...
    public void StatusFilterPressed(SecurityStatus statusSelected) => OnStatusFilterPressed?.Invoke(statusSelected);
    ...
    private void PopulateRecordContainer(GeneralStationRecord stationRecord, CriminalRecord criminalRecord)
    {
        ...
        if (criminalRecord.Status != SecurityStatus.None)
            specifier = new SpriteSpecifier.Rsi(new ResPath("Interface/Misc/security_icons.rsi"),
        ...
        if (criminalRecord.Reason is { } reason)
        {
            var message = FormattedMessage.FromMarkupOrThrow(
                Loc.GetString($"criminal-records-console-{criminalRecord.Status.ToString().ToLower()}-reason"));
    ...
    private void FilterListingOfRecords(string text = "") => OnFiltersChanged?.Invoke(_currentFilterType, text);
    ...
    private void GetReason(SecurityStatus status)
    {
        ...
        var placeholder = Loc.GetString("criminal-records-console-reason-placeholder",
            ("placeholder", _random.Pick(placeholders))); // just funny it doesn't actually get used
