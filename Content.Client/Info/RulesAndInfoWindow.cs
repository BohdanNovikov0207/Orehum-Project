...
public sealed class RulesAndInfoWindow : DefaultWindow
{
    ...
    private void PopulateTutorial(Info tutorialList)
    {
        ...
        infoControlSection.ControlsButton.OnPressed +=
            _ => UserInterfaceManager.GetUIController<OptionsUIController>().OpenWindow();
    }

    private static void AddSection(Info info, Control control) => info.InfoContainer.AddChild(control);
    ...
    private static Control MakeSection(string title, string path, bool markup, IResourceManager res) =>
        new InfoSection(title, res.ContentFileReadAllText($"/ServerInfo/{path}"), markup);
}
