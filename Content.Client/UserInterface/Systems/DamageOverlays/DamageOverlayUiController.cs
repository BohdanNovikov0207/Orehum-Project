            ...
            switch (mobState.CurrentState)
            {
                ...
                case MobState.Critical:
                {
                    if (!_mobThresholdSystem.TryGetDeadPercentage(entity,
                            FixedPoint2.Max(0.0, _mobThresholdSystem.CheckVitalDamage(entity, damageable)),
                            out var critLevel)) // GoobStation
                        return;
            ...
            switch (mobState.CurrentState)
            {
                ...
                case MobState.Alive:
                {
                    ...
                    _overlay.PainLevel = FixedPoint2.Min(1f,
                            (consciousness.Cap - consciousness.Consciousness) /
                    ...
                    if (_consciousness.TryGetNerveSystem(_playerManager.LocalEntity!.Value, out var nerveSys) &&
                        _consciousness.TryGetConsciousnessModifier(_playerManager.LocalEntity!.Value,
                            nerveSys.Value,
                            out var modifier,
                            "Suffocation"))
                        _overlay.OxygenLevel = FixedPoint2.Min(1f,
                                modifier.Value.Change / (consciousness.Cap - consciousness.Threshold))
                            .Float();
