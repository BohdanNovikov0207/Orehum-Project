...
public sealed class CargoOrderConsoleBoundUserInterface : BoundUserInterface
{
    ...
    protected override void Open()
    {
        ...
        _menu.OnItemSelected += args =>
        {
            ...
            _orderMenu.ToggleDepartmentSecureCrate.Pressed =
                false; // CorvaxGoob-CargoFeatures : дефолт знач при каждом открытии
            ...
            if (EntMan.TryGetComponent<CargoOrderConsoleComponent>(Owner, out var orderConsole))
            {
                ...
                if (_protoManager.TryIndex(orderConsole.Account, out var accountPrototype))
                    _orderMenu.DeliveryDestination.PlaceHolder = Loc.GetString(
                        accountPrototype.DepartmentDestinationName ??
                        "cargo-console-paper-delivery-destination-default");

                _orderMenu.Requester.Text = localPlayer.HasValue
                    ? _cargoSystem.GenerateRequesterName((Owner, orderConsole), localPlayer.Value)
                    : string.Empty;

                _orderMenu.ToggleDepartmentSecureCrate.Text = Loc.GetString("cargo-console-secure-order-checkbox",
                    ("cost", orderConsole.SecureOrderCost));

                if (_product is not null &&
                    _protoManager.TryIndex<EntityPrototype>(_product.Product, out var cargoProductEntPrototype))
                    _orderMenu.ToggleDepartmentSecureCrate.Disabled =
                        !_cargoSystem.CanBeSecuredDelivery((Owner, orderConsole), cargoProductEntPrototype);
            }
        ...
        _orderMenu.ToggleDepartmentSecureCrate.OnToggled +=
            ToggleDepartmentSecureCrate_OnToggled; // CorvaxGoob-CargoFeatures
    ...
    private void ToggleDepartmentSecureCrate_OnToggled(ButtonToggledEventArgs obj)
    {
        ...
        var cost = obj.Pressed
            ? _product.Cost + orderConsole.SecureOrderCost
            : _product.Cost; // Цена либо с защищённым либо нет
    ...
    private bool AddOrder()
    {
        ...
        SendMessage(new CargoConsoleAddOrderMessage(
            ...
            _orderMenu?.DeliveryDestination.Text == ""
                ? _orderMenu?.DeliveryDestination.PlaceHolder
                : _orderMenu?.DeliveryDestination.Text, // CorvaxGoob-CargoFeatures
