...
public sealed class ParallaxGenerator
{
    ...
    public static Image<Rgba32> GenerateParallax(TomlTable config,
        Size size,
        ISawmill sawmill,
    ...
    private sealed class LayerToAlpha : LayerConversion
    {
        ...
        public override Color ConvertColor(Color input) =>
            new(input.R, input.G, input.B, MathF.Min(input.R + input.G + input.B, 1.0f));
    }
    ...
    private sealed class LayerNoise : Layer
    {
        ...
        public LayerNoise(TomlTable table)
        {
            ...
            if (table.TryGetValue("sourcefactor", out tomlObject))
                SrcFactor = (Color.BlendFactor) Enum.Parse(typeof(Color.BlendFactor),
                    ((TomlValue<string>) tomlObject).Value);

            if (table.TryGetValue("destfactor", out tomlObject))
                DstFactor = (Color.BlendFactor) Enum.Parse(typeof(Color.BlendFactor),
                    ((TomlValue<string>) tomlObject).Value);
    ...
    private sealed class LayerPoints : Layer
    {
        ...
        public override void Apply(Image<Rgba32> bitmap)
        {
            ...
            using (var buffer =
                   new Image<Rgba32>(Configuration.Default, bitmap.Width, bitmap.Height, new Rgba32(0, 0, 0, 0)))
            {
                if (Masked)
