...
public sealed class GeneratedParallaxCache : IPostInjectInit
{
    ...
    void IPostInjectInit.PostInject() => _sawmill = _logManager.GetSawmill("parallax.generated");
        ...
        else
        {
            if (datum.ConfigPath != configPath)
                throw new InvalidOperationException(
                    "Generated parallax layers with the same ID must have the same config path!");
        }
    ...
    private async Task<Texture> LoadTask(string id, ResPath configPath, CancellationToken cancel) =>
        await GenerateTexture(id, configPath, cancel);
    ...
    private static ResPath CachedImagePath(string identifier) => new($"/parallax_{identifier}cache.png");

    private static ResPath PreviousConfigPath(string identifier) => new($"/parallax_{identifier}config_old");
