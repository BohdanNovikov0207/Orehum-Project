...
public sealed class SandboxSystem : SharedSandboxSystem
{
    ...
    private void OnSandboxStatus(MsgSandboxStatus ev) => SetAllowed(ev.SandboxAllowed);
    ...
    public void Respawn() => RaiseNetworkEvent(new MsgSandboxRespawn());

    public void GiveAdminAccess() => RaiseNetworkEvent(new MsgSandboxGiveAccess());

    public void GiveAGhost() => RaiseNetworkEvent(new MsgSandboxGiveAghost());

    public void Suicide() => RaiseNetworkEvent(new MsgSandboxSuicide());
    ...
    public bool Copy(ICommonSession? session, EntityCoordinates coords, EntityUid uid)
    {
        ...
        if (!_map.TryFindGridAt(_transform.ToMapCoordinates(coords), out var gridUid, out var grid) ||
    ...
    public void ToggleLight() => _consoleHost.ExecuteCommand("togglelight");

    public void ToggleFov() => _contentEye.RequestToggleFov();

    public void ToggleShadows() => _consoleHost.ExecuteCommand("toggleshadows");

    public void ToggleSubFloor() => _consoleHost.ExecuteCommand("showsubfloor");

    public void ShowMarkers() => _consoleHost.ExecuteCommand("showmarkers");

    public void ShowBb() => _consoleHost.ExecuteCommand("physics shapes");
}
