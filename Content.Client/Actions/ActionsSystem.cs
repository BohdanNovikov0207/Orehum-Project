...
public sealed class ActionsSystem : SharedActionsSystem
{
    ...
    {
        // TODO: Decouple this.
        ent.Comp.IconColor = _sharedCharges.GetCurrentCharges(ent.Owner) == 0 || !ent.Comp.Enabled
            ? ent.Comp.DisabledIconColor
            : ent.Comp.OriginalIconColor; // WD EDIT
    ...
    public void UnlinkAllActions() => UnlinkActions?.Invoke();
    ...
    private void OnWorldTargetAttempt(Entity<WorldTargetActionComponent> ent, ref ActionTargetAttemptEvent args)
    {
        ...
        if (action.ClientExclusive)
        {
            ...
            RaisePredictiveEvent(new RequestPerformActionEvent(GetNetEntity(uid),
    ...
    private void OnEntityTargetAttempt(Entity<EntityTargetActionComponent> ent, ref ActionTargetAttemptEvent args)
    {
        ...
        if (comp.Event is not { } ev)
        {
            DebugTools.Assert(HasComp<WorldTargetActionComponent>(ent),
                $"Action {ToPrettyString(ent)} requires WorldTargetActionComponent for entity-world targeting");
            return;
        }
