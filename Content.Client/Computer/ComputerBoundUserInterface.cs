...
/// <summary>
/// ComputerBoundUserInterface shunts all sorts of responsibilities that are in the BoundUserInterface for architectural
/// reasons into the Window.
...
public class ComputerBoundUserInterface<TWindow, TState> : ComputerBoundUserInterfaceBase
    where TWindow : BaseWindow, IComputerWindow<TState>, new() where TState : BoundUserInterfaceState
{
    [ViewVariables]
    ...
    protected override void ReceiveMessage(BoundUserInterfaceMessage message) => _window?.ReceiveMessage(message);
}
...
public class ComputerBoundUserInterfaceBase : BoundUserInterface
{
    ...
    public new void SendMessage(BoundUserInterfaceMessage msg) => base.SendMessage(msg);
}
