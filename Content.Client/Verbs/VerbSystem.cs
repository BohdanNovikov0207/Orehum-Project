...
public sealed class VerbSystem : SharedVerbSystem
{
    ...
    private void OnLookupChanged(float val) => _lookupSize = val;
    ...
    public bool TryGetEntityMenuEntities(MapCoordinates targetPos, [NotNullWhen(true)] out List<EntityUid>? entities)
    {
        ...
        if ((visibility & MenuVisibility.InContainer) != 0)
        {
            ...
            foreach (var e in _lookup.GetEntitiesInRange(targetPos, _lookupSize, flags))
            {
    ...
    /// </summary>
    public SortedSet<Verb> GetVerbs(NetEntity target,
        EntityUid user,
        List<Type> verbTypes,
    ...
    /// </remarks>
    public void ExecuteVerb(EntityUid target, Verb verb) => ExecuteVerb(GetNetEntity(target), verb);
    ...
    private void HandleVerbResponse(VerbsResponseEvent msg) => OnVerbsResponse?.Invoke(msg);
}
