...
public sealed partial class EntityMenuUIController
{
    ...
    private List<List<EntityUid>> GroupEntities(IEnumerable<EntityUid> entities, int depth = 0)
    {
        ...
        {
            var newEntities = entities.GroupBy(e => e, new PrototypeAndStatesContextMenuComparer(depth, _entityManager))
                .ToList();
            return newEntities.Select(grp => grp.ToList()).ToList();
        }
    }
    ...
    private sealed class PrototypeAndStatesContextMenuComparer : IEqualityComparer<EntityUid>
    {
        ...
        {
            (a, b, entMan) => entMan.GetComponent<MetaDataComponent>(a).EntityPrototype!.ID ==
        ...
        {
            (e, entMan) =>
                EqualityComparer<string>.Default.GetHashCode(entMan.GetComponent<MetaDataComponent>(e).EntityPrototype!
                    .ID),
            (e, entMan) =>
            {
                var hash = 0;
                foreach (var element in entMan.GetComponent<SpriteComponent>(e)
        ...
        public int GetHashCode(EntityUid e) => GetHashCodeList[_depth](e, _entMan);
    }
}
