    ...
    ISharedAdminManager
{
    ...
    public bool IsActive() => _adminData?.Active ?? false;

    public bool HasFlag(AdminFlags flag) => _adminData?.HasFlag(flag) ?? false;
    ...
    public bool CanViewVar() => CanCommand("vv");

    public bool CanAdminPlace() => _adminData?.CanAdminPlace() ?? false;

    public bool CanScript() => _adminData?.CanScript() ?? false;

    public bool CanAdminMenu() => _adminData?.CanAdminMenu() ?? false;
