...
public sealed class GasCanisterBoundUserInterface : BoundUserInterface
{
    ...
    private void OnTankEjectPressed() => SendPredictedMessage(new GasCanisterHoldingTankEjectMessage());

    private void OnReleasePressureSet(float value) =>
        SendPredictedMessage(new GasCanisterChangeReleasePressureMessage(value));

    private void OnReleaseValveOpenPressed() => SendPredictedMessage(new GasCanisterChangeReleaseValveMessage(true));

    private void OnReleaseValveClosePressed() => SendPredictedMessage(new GasCanisterChangeReleaseValveMessage(false));
    ...
    protected override void UpdateState(BoundUserInterfaceState state)
    {
        ...
        var tankLabel = component.GasTankSlot.Item != null
            ? Identity.Name(component.GasTankSlot.Item.Value, EntMan)
            : null;
