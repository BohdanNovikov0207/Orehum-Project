...
public sealed class AtmosMonitoringConsoleNavMapControl : NavMapControl
{
    ...
    private readonly Dictionary<Color, Dictionary<Vector2i, Vector2i>> _horizLines = new();
    private readonly Dictionary<Color, Dictionary<Vector2i, Vector2i>> _horizLinesReversed = new();
    ...
    private readonly Dictionary<Color, Dictionary<Vector2i, Vector2i>> _vertLines = new();
    private readonly Dictionary<Color, Dictionary<Vector2i, Vector2i>> _vertLinesReversed = new();
    ...
    private void DrawPipeNetwork(DrawingHandleScreen handle, List<AtmosMonitoringConsoleLine> atmosPipeNetwork)
    {
        ...
        if (WorldRange / WorldMaxRange > 0.5f)
        {
            ...
            foreach (var (color, subNetwork) in pipeNetworks)
            {
        ...
        else
        {
            ...
            foreach (var chunkedLine in atmosPipeNetwork)
            {
                ...
                var leftBottom = ScalePosition(new Vector2(
                                                   Math.Min(chunkedLine.Origin.X, chunkedLine.Terminus.X) -
                                                   LineThickness,
                                                   Math.Max(chunkedLine.Origin.Y, chunkedLine.Terminus.Y) +
                                                   LineThickness)
                                               - offset);

                var rightBottom = ScalePosition(new Vector2(
                                                    Math.Max(chunkedLine.Origin.X, chunkedLine.Terminus.X) +
                                                    LineThickness,
                                                    Math.Max(chunkedLine.Origin.Y, chunkedLine.Terminus.Y) +
                                                    LineThickness)
                                                - offset);
            ...
            foreach (var (color, pipeVertexUV) in pipeVertexUVs)
            {
    ...
    private List<AtmosMonitoringConsoleLine> GetDecodedAtmosPipeChunks(Dictionary<Vector2i, AtmosPipeChunk>? chunks,
        MapGridComponent? grid)
    {
        ...
        foreach (var (color, horizLines) in _horizLines)
        {
            ...
            {
                decodedOutput.Add(new AtmosMonitoringConsoleLine(ConvertVector2iToVector2(origin, 1f / ScaleModifier),
    ...
    private Vector2i ConvertVector2ToVector2i(Vector2 vector, float scale = 1f) =>
        new((int) MathF.Round(vector.X * scale), (int) MathF.Round(vector.Y * scale));
