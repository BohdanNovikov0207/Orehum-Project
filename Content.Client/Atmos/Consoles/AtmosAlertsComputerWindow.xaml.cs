...
public sealed partial class AtmosAlertsComputerWindow : FancyWindow
{
    ...
    public void UpdateUI(EntityCoordinates? consoleCoords,
        ...
        AtmosAlertsFocusDeviceData? focusData)
    {
        ...
        _activeAlarms = _allAlarms.Where(x => x.AlarmState > AtmosAlarmType.Normal &&
                                              (!silenced.Contains(x.NetEntity) ||
                                               _deviceSilencingProgress.ContainsKey(x.NetEntity)));
        ...
        if (MasterTabContainer.CurrentTab == 0 && activeAlarmCount == 0)
        {
            ...
            label.SetMarkup(Loc.GetString("atmos-alerts-window-no-active-alerts",
                ("color", _statusTextColor.ToHexNoAlpha())));
        ...
        if (activeAlarmCount == 0)
            ...
            MasterTabContainer.SetTabTitle(0,
        ...
        {
            var regionOverlays =
                _navMapSystem.GetNavMapRegionOverlays(_owner.Value, navMap, AtmosAlertsComputerUiKey.Key);
    ...
    private void AddTrackedEntityToNavMap(AtmosAlertsDeviceNavMapData metaData, AtmosAlarmType alarmState)
    {
        ...
        var selectable = true;
        var blip = new NavMapBlip(coords,
    ...
    private void UpdateUIEntry(AtmosAlertsComputerEntry entry,
        int index,
        Control table,
        ...
        AtmosAlertsFocusDeviceData? focusData = null)
    {
        ...
        {
            var newEntryContainer =
                new AtmosAlarmEntryContainer(entry.NetEntity, _entManager.GetCoordinates(entry.Coordinates));
        ...
        {
            var silenced = console.SilencedDevices;
            entryContainer.SilenceCheckBox.Pressed = silenced.Contains(entry.NetEntity) ||
                                                     _deviceSilencingProgress.ContainsKey(entry.NetEntity);
        }
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        foreach (var (device, remainingTime) in _deviceSilencingProgress)
        {
    ...
    private (SpriteSpecifier.Texture, Color)? GetBlipTexture(AtmosAlarmType alarmState)
    {
        ...
        switch (alarmState)
        {
            case AtmosAlarmType.Invalid:
                output = (new SpriteSpecifier.Texture(new ResPath("/Textures/Interface/NavMap/beveled_circle.png")),
                    _inactiveColor);
                break;
            ...
            case AtmosAlarmType.Warning:
                output = (new SpriteSpecifier.Texture(new ResPath("/Textures/Interface/NavMap/beveled_triangle.png")),
                    _warningColor);
                break;
            case AtmosAlarmType.Danger:
                output = (new SpriteSpecifier.Texture(new ResPath("/Textures/Interface/NavMap/beveled_square.png")),
                    _dangerColor);
                break;
        }
