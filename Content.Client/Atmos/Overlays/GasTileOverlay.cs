...
public sealed class GasTileOverlay : Overlay
{
    ...
    public const int
        GasOverlayZIndex = (int) Shared.DrawDepth.DrawDepth.Effects; // Under ghosts, above mostly everything else
    ...
    public GasTileOverlay(GasTileOverlaySystem system,
        ...
        SharedTransformSystem xformSys)
    {
        ...
        for (var i = 0; i < _gasCount; i++)
        {
            ...
            if (!string.IsNullOrEmpty(gasPrototype.GasOverlaySprite) &&
                ...
                overlay = new SpriteSpecifier.Rsi(new ResPath(gasPrototype.GasOverlaySprite),
                    gasPrototype.GasOverlayState);
            else if (!string.IsNullOrEmpty(gasPrototype.GasOverlayTexture))
                overlay = new SpriteSpecifier.Texture(new ResPath(gasPrototype.GasOverlayTexture));
            else
                continue;
