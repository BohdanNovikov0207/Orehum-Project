...
public sealed partial class MonumentMenu : FancyWindow
{
    ...
    public MonumentMenu()
    {
        ...
        _cfg.OnValueChanged(DCCVars.CosmicCultistEntropyValue,
            entropy =>
            {
    ...
    private void UpdateBar(MonumentBuiState state)
    {
        ...
        ProgressBarPercentage.Text = Loc.GetString("monument-interface-progress-bar",
            ("percentage", percentComplete.ToString("0")));
    }
    ...
    private void UpdateEntropy(MonumentBuiState state)
    {
        ...
        var min = entropyToNextStage == 0
            ? 0
            : 1; // I have no idea what to call this. makes it so that it shows 0 crew for the final stage but at least one at all other times
        var crewToNextStage =
            Math.Max(entropyToNextStage / _entropyPerCultist,
                min); // Why the fuck did you make such an ugly cast for an approximation anyway???

        AvailableEntropy.Text = Loc.GetString("monument-interface-entropy-value", ("infused", availableEntropy));
        EntropyUntilNextStage.Text =
            Loc.GetString("monument-interface-entropy-value", ("infused", entropyToNextStage.ToString()));
