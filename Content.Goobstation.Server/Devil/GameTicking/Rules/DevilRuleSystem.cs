...
public sealed class DevilRuleSystem : GameRuleSystem<DevilRuleComponent>
{
    ...
    private bool MakeDevil(EntityUid target, DevilRuleComponent rule)
    {
        ...
        var briefing = Loc.GetString("devil-role-greeting",
            ("trueName", devilComp.TrueName),
            ("playerName", Name(target)));
    ...
    private string MakeBriefing(EntityUid ent) =>
        !TryComp<DevilComponent>(ent, out var devilComp)
            ? null!
...

    {
        ...
        sb.AppendLine(Loc.GetString(
            $"roundend-prepend-devil-contracts{(!string.IsNullOrWhiteSpace(mostContractsName) ? "-named" : "")}",
            ("name", mostContractsName),
            ("number", mostContracts)));
