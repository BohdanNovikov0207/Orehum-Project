...
public sealed class SecretPlusSystem : GameRuleSystem<SecretPlusComponent>
{
    ...
    private readonly ProtoId<TagPrototype> _loneSpawnTag = "LoneRunRule";
    ...
    public override void Initialize()
    {
        ...
        Subs.CVar(_cfg,
            GoobCVars.RoundstartChaosScoreMultiplier,
            value => _roundstartChaosScoreMultiplier = value,
            true);
    }

    protected override void Added(EntityUid uid,
        SecretPlusComponent scheduler,
    ...
    /// </summary>
    private void SetupEvents(Entity<SecretPlusComponent> scheduler,
        PlayerCount count,
    ...
    private void SelectFromTable(Entity<SecretPlusComponent> scheduler,
        PlayerCount count,
        SelectedGameRulesComponent? selectedRules)
    {
        if (selectedRules == null)
            return;
        ...
        float frameTime)
    {
        ...
        var amt = TimeSpan.FromSeconds(
            _random.NextDouble(scheduler.EventIntervalMin.TotalSeconds, scheduler.EventIntervalMax.TotalSeconds) /
            ramp / speedup);
        scheduler.TimeNextEvent = currTime + amt;
        // generally more useful than curTime
        LogMessage(
            $"Chaos score: {scheduler.ChaosScore}, Next event at: {_ticker.RoundDuration() + amt} (ramping {ramp})");
    ...
    private void TrySpawnRoundstartAntags(Entity<SecretPlusComponent> scheduler)
    {
        ...
        while (scheduler.Comp.ChaosScore < 0 && i < maxIters)
        {
            ...
            if (!_prototypeManager.TryIndex(pick, out var entProto)
                || !entProto.TryGetComponent(out ruleComp, _factory)
               )
                continue;
    ...
    public float? GetChaosScore(Entity<GameRuleComponent?> rule, int? players = null)
    {
        ...
        if (TryComp<AntagSelectionComponent>(rule, out var selection))
        {
            ...
            foreach (var def in selection.Definitions)
            {
                ...
                any = true;
                var count = _antagSelection.GetTargetAntagCount((rule, selection),
    ...
    {
        if (ruleComp == null && !ruleProto.TryGetComponent(out ruleComp, _factory))
            return null;
        ...
        if (ruleProto.TryGetComponent<AntagSelectionComponent>(out var selection, _factory))
        {
            ...
            foreach (var def in selection.Definitions)
            {
                ...
                any = true;
                var count = _antagSelection.GetTargetAntagCount((EntityUid.Invalid, selection),
                    players ?? GetTotalPlayerCount(_playerManager.Sessions),
                    def);
                score += def.ChaosScore.Value * count;
            }
    ...
    public int GetTotalPlayerCount(IList<ICommonSession> pool) =>
        _antagSelection.GetTotalPlayerCount(pool) + _event.PlayerCountBias;
    ...
    private SelectedEvent? ChooseEvent(Entity<SecretPlusComponent> scheduler)
    {
        ...
        foreach (var ev in possible)
        {
            ...
            weight += weight < 0f
                ? -scheduler.Comp.ChaosThreshold
                : scheduler.Comp
                    .ChaosThreshold; // make sure it's not in (-1, 1) to not get absurdly low event probabilities
            var delta = ChaosDelta(-scheduler.Comp.ChaosScore,
                weight,
                scheduler.Comp.ChaosMatching,
                scheduler.Comp.ChaosThreshold * scheduler.Comp.ChaosThreshold);
