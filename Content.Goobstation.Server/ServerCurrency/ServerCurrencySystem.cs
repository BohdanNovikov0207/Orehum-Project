...
public sealed class ServerCurrencySystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        Subs.CVar(_cfg,
            GoobCVars.GoobcoinLowpopMultiplierStrength,
            value => _goobcoinsLowPopMultiplierStrength = value,
            true);
        Subs.CVar(_cfg, GoobCVars.GoobcoinUseShortRoundPenalty, value => _goobcoinsUseShortRoundPenalty = value, true);
        Subs.CVar(_cfg,
            GoobCVars.GoobcoinShortRoundPenaltyTargetMinutes,
            value => _goobcoinsShortRoundPenaltyTargetMinutes = value,
            true);
    }
    ...
    private void OnRoundEndText(RoundEndTextAppendEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out var mindContainer))
        {
            ...
            if (mindContainer.Mind.HasValue)
            {
                ...
                if (mind is not null
                    && (isBorg ||
                        !_mind.IsCharacterDeadIc(
                            mind)) // Borgs count always as dead so I'll just throw them a bone and give them an exception.
                    ...
                    && _players.TryGetSessionById(mind.UserId, out var session))
                {
                    ...
                    if (_goobcoinsUseShortRoundPenalty)
                    {
                        ...
                        money = (int) (money * Math.Min(1,
                            roundMinutesActual / _goobcoinsShortRoundPenaltyTargetMinutes));
                    }
    ...
    private void OnBalanceChange(PlayerBalanceChangeEvent ev)
    {
        ...
        if (ev.UserSes.AttachedEntity.HasValue)
        {
            ...
            if (ev.NewBalance > ev.OldBalance)
                _popupSystem.PopupEntity("+" + _currencyMan.Stringify(ev.NewBalance - ev.OldBalance),
                    userEnt,
                    userEnt,
                    PopupType.Medium);
            else if (ev.NewBalance < ev.OldBalance)
                _popupSystem.PopupEntity("-" + _currencyMan.Stringify(ev.OldBalance - ev.NewBalance),
                    userEnt,
                    userEnt,
