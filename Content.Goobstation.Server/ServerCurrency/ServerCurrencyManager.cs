...
public sealed class ServerCurrencyManager : ICommonCurrencyManager
{
    ...
    public void Initialize() => _sawmill = Logger.GetSawmill("server_currency");

    /// <inheritdoc />
    public void Shutdown() => _task.BlockWaitOnTask(Task.WhenAll(_pendingSaveTasks));
    ...
    public (int, int) TransferCurrency(NetUserId sourceUserId, NetUserId targetUserId, int amount)
    {
        ...
        _sawmill.Info(
            $"Transferring {amount} currency from {sourceUserId} to {targetUserId}. Current balances: {newAccountValues.Item1}, {newAccountValues.Item2}");
    ...
    public int GetBalance(NetUserId? userId = null) =>
        userId == null ? 0 : Task.Run(() => GetBalanceAsync(userId.Value)).GetAwaiter().GetResult();
