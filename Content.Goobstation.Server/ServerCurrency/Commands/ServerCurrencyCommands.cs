...
public sealed class GiftServerCurrencyCommand : IConsoleCommand
{
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (targetPlayer == shell.Player.UserId)
        {
        ...
        if (!_currencyMan.CanAfford(shell.Player.UserId, amount, out var balance))
        {
        ...
        var giver = Loc.GetString("server-currency-gift-command-giver",
            ("player", args[0]),
            ("amount", _currencyMan.Stringify(amount)));
        var reciever = Loc.GetString("server-currency-gift-command-reciever",
...
public sealed class AddServerCurrencyCommand : IConsoleCommand
{
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (!int.TryParse(args[1], out var currency))
        {
            shell.WriteError(Loc.GetString("server-currency-command-error-2"));
            return;
        }
