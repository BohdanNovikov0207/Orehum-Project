...
public struct FixedPoint4 : ISelfSerialize, IComparable<FixedPoint4>, IEquatable<FixedPoint4>, IFormattable
{
    ...
    private readonly double ShiftDown() => Value / (double) ShiftConstant;
    ...
    public static FixedPoint4 New(long value) => new(value * ShiftConstant);
    public static FixedPoint4 FromTenThousandths(long value) => new(value);

    public static FixedPoint4 New(float value) => new((long) ApplyFloatEpsilon(value * ShiftConstant));

    private static float ApplyFloatEpsilon(float value) => value + FloatEpsilon * Math.Sign(value);

    private static double ApplyFloatEpsilon(double value) => value + FloatEpsilon * Math.Sign(value);

    /// <summary>
    /// Create the closest <see cref="FixedPoint4" /> for a float value, always rounding up.
    /// </summary>
    public static FixedPoint4 NewCeiling(float value) => new((long) MathF.Ceiling(value * ShiftConstant));

    public static FixedPoint4 New(double value) => new((long) ApplyFloatEpsilon(value * ShiftConstant));

    public static FixedPoint4 New(string value) => New(Parse.Float(value));
    ...
    public static FixedPoint4 operator *(FixedPoint4 a, FixedPoint4 b) => new(b.Value * a.Value / ShiftConstant);

    public static FixedPoint4 operator *(FixedPoint4 a, float b) => new((long) ApplyFloatEpsilon(a.Value * b));

    public static FixedPoint4 operator *(FixedPoint4 a, double b) => new((long) ApplyFloatEpsilon(a.Value * b));

    public static FixedPoint4 operator *(FixedPoint4 a, long b) => new(a.Value * b);

    public static FixedPoint4 operator /(FixedPoint4 a, FixedPoint4 b) => new(ShiftConstant * a.Value / b.Value);

    public static FixedPoint4 operator /(FixedPoint4 a, float b) => new((long) ApplyFloatEpsilon(a.Value / b));

    public static bool operator <=(FixedPoint4 a, long b) => a <= New(b);
    ...
    public readonly float Float() => (float) ShiftDown();

    public readonly double Double() => ShiftDown();

    public readonly long Long() => Value / ShiftConstant;

    public readonly int Int() => (int) Long();
    ...
    public static FixedPoint4 Min(params FixedPoint4[] fixedPoints) => fixedPoints.Min();

    public static FixedPoint4 Min(FixedPoint4 a, FixedPoint4 b) => a < b ? a : b;

    public static FixedPoint4 Max(FixedPoint4 a, FixedPoint4 b) => a > b ? a : b;
    ...
    public static FixedPoint4 Abs(FixedPoint4 a) => FromTenThousandths(Math.Abs(a.Value));

    public static FixedPoint4 Dist(FixedPoint4 a, FixedPoint4 b) => Abs(a - b);
    ...
    public readonly bool Equals(FixedPoint4 other) => Value == other.Value;
