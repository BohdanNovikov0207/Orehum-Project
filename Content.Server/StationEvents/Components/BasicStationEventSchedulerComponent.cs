...
public sealed partial class BasicStationEventSchedulerComponent : Component
{
    ...
    /// </summary>
    /// Reminder that though we could do all selection via the EntityTableSelector, we also need to consider various
    /// <see cref="StationEventComponent" />
    /// restrictions.
    /// As such, we want to pass a list of acceptable game rules, which are then parsed for restrictions by the
    /// <see cref="EventManagerSystem" />
    /// .
    [DataField(required: true)]
    public EntityTableSelector ScheduledGameRules = default!;

    /// <summary>
    /// How long until the next check for an event runs, is initially set based on MinimumTimeUntilFirstEvent &
    /// MinMaxEventTiming.
    /// </summary>
    [DataField]
