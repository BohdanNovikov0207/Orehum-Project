...
internal sealed class GasLeakRule : StationEventSystem<GasLeakRuleComponent>
{
        ...
        GameRuleStartedEvent args)
    {
        ...
        if (TryFindRandomTile(out component.TargetTile,
                out var target,
                ...
                out component.TargetCoords))
        {
            ...
            component.MolesPerSecond =
                RobustRandom.Next(component.MinimumMolesPerSecond, component.MaximumMolesPerSecond);
            // Goobstation start
            if (gameRule.Delay is { } startAfter)
            {
                ...
                _adminLogger.Add(LogType.EventRan,
                    LogImpact.High,
                    $"Gasleak placing {totalGas} moles of {component.LeakGas} at {component.TargetTile} in grid {component.TargetGrid}.");
        ...
        if (RobustRandom.NextFloat() <= component.SparkChance)
        {
            if (!component.FoundTile ||
                ...
                (!Exists(component.TargetGrid)
                    ...
                    : MetaData(component.TargetGrid).EntityLifeStage) >= EntityLifeStage.Deleted ||
                !_atmosphere.IsSimulatedGrid(component.TargetGrid))
                return;
