...
public sealed class BureaucraticErrorRule : StationEventSystem<BureaucraticErrorRuleComponent>
{
        ...
        GameRuleStartedEvent args)
    {
        ...
        if (RobustRandom.Prob(0.25f))
        {
            ...
            {
                if (!_stationJobs.TryGetJobSlot(chosenStation.Value, job, out var currentSlots) ||
                    !currentSlots.HasValue || currentSlots <= 0)
                    continue;
                ...
                _adminLog.Add(LogType.AdminMessage,
                    LogImpact.Low,
                    $"Bureaucratic Error: Changed {job} slots from {currentSlots} to {newSlots}");
            }
        }
        else
        {
            ...
            for (var i = 0; i < num; i++)
            {
                    ...
                    currentSlots.HasValue && currentSlots > 0)
                {
                    ...
                    _chat.SendAdminAlert(
                        $"Bureaucratic Error: Changed {chosenJob} slots from {currentSlots} to {newSlots}");
                    _adminLog.Add(LogType.AdminMessage,
                        LogImpact.Low,
                        $"Bureaucratic Error: Changed {chosenJob} slots from {currentSlots} to {newSlots}");
                }
