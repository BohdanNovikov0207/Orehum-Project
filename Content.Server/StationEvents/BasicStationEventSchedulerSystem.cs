...
public sealed class BasicStationEventSchedulerSystem : GameRuleSystem<BasicStationEventSchedulerComponent>
{
    ...
    protected override void Started(EntityUid uid,
        ...
        component.TimeUntilNextEvent = RobustRandom.NextFloat(component.MinimumTimeUntilFirstEvent,
...
public sealed class StationEventCommand : ToolshedCommand
{
    ...
    public IEnumerable<(string, float)> Simulate([CommandArgument] Prototype<EntityPrototype> eventSchedulerProto,
        ...
        [CommandArgument] float roundEndStdDev)
    {
        ...
        eventSchedulerProto.Deconstruct(out var eventScheduler);
        ...
        for (var i = 0; i < rounds; i++)
        {
            ...
            var randomEndTime =
                _random.NextGaussian(roundEndMean, roundEndStdDev) *
                60; // Its in minutes, should probably be a better time format once we get that in toolshed like [hh:mm:ss]
            if (randomEndTime <= 0)
                continue;
    ...
    public IEnumerable<(string, float)> LsProb([CommandArgument] Prototype<EntityPrototype> eventSchedulerProto)
    {
        ...
        var totalWeight =
            events.Sum(x =>
                x.Value.Weight); // Well this shit definitely isnt correct now, and I see no way to make it correct.
        // Its probably *fine* but it wont be accurate if the EntityTableSelector does any subsetting.
        foreach (var (proto, comp) in
                 events) // The only solution I see is to do a simulation, and we already have that, so...!
        {
    ...
    public IEnumerable<(string, float)> LsProbTime([CommandArgument] Prototype<EntityPrototype> eventSchedulerProto,
        [CommandArgument] int playerCount,
    ...
    public float Prob([CommandArgument] Prototype<EntityPrototype> eventSchedulerProto,
        [CommandArgument] string eventId)
    {
        _compFac ??= IoCManager.Resolve<IComponentFactory>();
        _stationEvent ??= GetSys<EventManagerSystem>();
