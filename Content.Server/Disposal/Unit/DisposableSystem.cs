...
public sealed class DisposableSystem : EntitySystem
{
    ...
    public void ExitDisposals(EntityUid uid,
        ...
        TransformComponent? holderTransform = null)
    {
        ...
        while (children.MoveNext(out var entity))
        {
            ...
            if (holder.Container.Contains(entity))
                _containerSystem.Remove((entity, null, meta), holder.Container, false, true);
    ...
    private void UpdateComp(EntityUid uid, DisposalHolderComponent holder, float frameTime)
    {
        while (frameTime > 0)
        {
            ...
            _containerSystem.Remove(uid, _disposalTubeQuery.GetComponent(currentTube).Contents, false, true);
