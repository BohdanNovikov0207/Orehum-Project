...
public sealed class SurveillanceCameraMonitorSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        Subs.BuiEvents<SurveillanceCameraMonitorComponent>(SurveillanceCameraMonitorUiKey.Key,
            subs =>
            {
    ...
    public override void Update(float frameTime)
    {
        ...
        var queryThree =
            EntityQueryEnumerator<HasMobileCamerasSurveillanceCameraMonitorComponent,
                SurveillanceCameraMonitorComponent>();
        while (queryThree.MoveNext(out var uid, out _, out var monitor))
        {
            if (monitor.KnownMobileCameras.Count > 0)
            {
                ...
                foreach (var (key, cameraData) in monitor.KnownMobileCameras)
                {
                    ref var lastSent = ref CollectionsMarshal.GetValueRefOrAddDefault(
                        monitor.KnownMobileCamerasLastHeartbeatSent,
                        key,
                        out var sentExists);
                    ref var lastHeartbeat = ref CollectionsMarshal.GetValueRefOrAddDefault(
                        monitor.KnownMobileCamerasLastHeartbeat,
                        key,
                        out var hbExists);
    ...
    private void SendHeartbeat(EntityUid uid,
        string cameraAdress,
        SurveillanceCameraMonitorComponent? monitor = null) // Goobstation
    {
    ...
    private void AddViewer(EntityUid uid, EntityUid player, SurveillanceCameraMonitorComponent? monitor = null)
    {
        ...
        if (TryComp<ActorComponent>(player, out var actor))
        {
            ...
            {
                _pvsOverrideSystem.AddSessionOverride(_entityManager.GetEntity(camera.Value.Item2.Item2.NetEntity),
                    actor.PlayerSession);
            }
        }
        // Goobstation end
    ...
    private void RemoveViewer(EntityUid uid, EntityUid player, SurveillanceCameraMonitorComponent? monitor = null)
    {
        ...
        if (TryComp<ActorComponent>(player, out var actor))
        {
            foreach (var camera in monitor.KnownMobileCameras)
            {
                _pvsOverrideSystem.RemoveSessionOverride(_entityManager.GetEntity(camera.Value.Item2.Item2.NetEntity),
                    actor.PlayerSession);
            }
        }
        // Goobstation start
    ...
    // to the new camera.
    private void TrySwitchCameraByUid(EntityUid uid,
        EntityUid newCamera,
        SurveillanceCameraMonitorComponent? monitor = null)
    {
        if (!Resolve(uid, ref monitor))
    ...
    public void AfterOpenUserInterface(EntityUid uid,
        EntityUid player,
        SurveillanceCameraMonitorComponent? monitor = null,
        ...
        EntityUid? player = null)
    {
        ...
        var state = new SurveillanceCameraMonitorUiState(GetNetEntity(monitor.ActiveCamera), // Goobstation
            ...
            monitor.KnownMobileCameras); // Goobstation
            ...
            switch (command)
            {
                ...
                case SurveillanceCameraSystem.CameraDataMessage:
                    ...
                    if (!args.Data.TryGetValue(SurveillanceCameraSystem.CameraNameData, out string? name)
                        ...
                        || !args.Data.TryGetValue(SurveillanceCameraSystem.CameraNetEntity,
                            out (NetEntity, NetCoordinates)? netEntity)
                    ...
                    if (mobile.HasValue && mobile.Value) // if camera is mobile, it should be in the mobile cameras list
                    {
                        ...
                        if (!component.KnownMobileCameras.ContainsKey(address))
                        {
                            ...
                            {
                                if (TryComp<ActorComponent>(player, out var actor))
                                    _pvsOverrideSystem.AddSessionOverride(
                                        _entityManager.GetEntity(netEntity.Value.Item2.NetEntity),
                                        actor.PlayerSession);
                            }
                        }
                    }
    ...
    private void OnDisconnectMessage(EntityUid uid,
        SurveillanceCameraMonitorComponent component,
        SurveillanceCameraDisconnectMessage message) =>
        DisconnectCamera(uid, true, component);

    private void OnRefreshCamerasMessage(EntityUid uid,
        SurveillanceCameraMonitorComponent component,
    ...
    private void RefreshCameras(EntityUid uid, SurveillanceCameraMonitorComponent component)
    {
        foreach (var player in component.Viewers)
        {
            if (TryComp<ActorComponent>(player, out var actor))
            {
                ...
                {
                    _pvsOverrideSystem.RemoveSessionOverride(
                        _entityManager.GetEntity(camera.Value.Item2.Item2.NetEntity),
                        actor.PlayerSession);
                }
            }
        }
    ...
    private void OnRefreshSubnetsMessage(EntityUid uid,
        SurveillanceCameraMonitorComponent component,
        SurveillanceCameraRefreshSubnetsMessage message) =>
        RefreshSubnets(uid, component);

    private void OnSwitchMessage(EntityUid uid,
        SurveillanceCameraMonitorComponent component,
    ...
    private void OnToggleInterface(EntityUid uid,
        SurveillanceCameraMonitorComponent component,
    ...
    private void OnSurveillanceCameraDeactivate(EntityUid uid,
        SurveillanceCameraMonitorComponent monitor,
        SurveillanceCameraDeactivateEvent args) => DisconnectCamera(uid, false, monitor);
