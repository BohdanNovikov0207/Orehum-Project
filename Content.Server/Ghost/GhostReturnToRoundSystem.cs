...
public sealed class GhostReturnToRoundSystem : EntitySystem
{
    ...
    private void OnGhostReturnToRoundRequest(GhostReturnToRoundRequest msg, EntitySessionEventArgs args)
    {
        ...
        var channel = args.SenderSession.Channel;
        var userId = args.SenderSession.UserId;

        TryGhostReturnToRound(uid.Value, channel, userId, out var message, out var wrappedMessage);
    ...
    private void TryGhostReturnToRound(EntityUid uid, INetChannel channel, NetUserId userId, out string message, out string wrappedMessage)
    {
        ...
        if (timePast >= timeUntilRespawn)
        {
            ...
            _adminLogger.Add(LogType.Mind, LogImpact.Medium, $"{Loc.GetString("ghost-respawn-log-return-to-lobby", ("userName", channel.UserName))}");
