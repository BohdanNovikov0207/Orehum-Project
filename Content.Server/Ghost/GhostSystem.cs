...
public sealed class GhostSystem : SharedGhostSystem
{
    ...
    private void OnEntityStorageInsertAttempt(EntityUid uid,
        GhostComponent comp,
        ref InsertIntoEntityStorageAttemptEvent args) => args.Cancelled = true;
    ...
    public bool OnGhostAttempt(EntityUid mindId,
        ...
        MindComponent? mind = null)
    {
        ...
        if (playerEntity != null && viaCommand)
        {
            if (forced)
                _adminLog.Add(LogType.Mind,
                    $"{ToPrettyString(playerEntity.Value):player} was forced to ghost via command");
            else
                _adminLog.Add(LogType.Mind,
                    $"{ToPrettyString(playerEntity.Value):player} is attempting to ghost via command");
        }
        ...
        {
            if (_player.TryGetSessionById(mind.UserId,
                    out var session)) // Logging is suppressed to prevent spam from ghost attempts caused by movement attempts
            {
                _chatManager.DispatchServerMessage(session,
                    Loc.GetString("comp-mind-ghosting-prevented"),
                    true);
            }
            ...
            TryComp(playerEntity, out MobStateComponent? mobState))
        {
            if (_mobState.IsCritical(playerEntity.Value, mobState))
            {
                ...
                if (!HasComp<XenomorphPreventSuicideComponent>(playerEntity.Value))
                {
                    ...
                    {
                        var playerDeadThreshold =
                            _mobThresholdSystem.GetThresholdForState(playerEntity.Value, MobState.Dead, thresholds);
        ...
        if (playerEntity != null)
            _adminLog.Add(LogType.Mind,
                $"{ToPrettyString(playerEntity.Value):player} ghosted{(!canReturn ? " (non-returnable)" : "")}");
    ...
    private void OnGhostWarpToTargetRequest(GhostWarpToTargetRequestEvent msg, EntitySessionEventArgs args)
    {
        ...
        {
            Log.Warning(
                $"User {args.SenderSession.Name} tried to warp to an invalid/hidden target: {ToPrettyString(target)}");
            return;
        }
        // WWDP EDIT END
    ...
    private List<GhostWarp> GetMindContainersWarps()
    {
        ...
        while (query.MoveNext(out var entity, out var mindContainer))
        {
            ...
            if (TryComp<RoleCacheComponent>(entity, out var roleCacheComponent))
            {
                ...
                {
                    var warp = SetupWarp(entity,
                        mindContainer,
                        departmentPrototype.Name,
                        departmentPrototype.Color,
                        jobPrototype.Name);
    ...
    private GhostWarp SetupWarp(EntityUid entity,
        MindContainerComponent mindContainer,
        string subGroup,
        Color? color,
        string? description)
    {
        var hasAnyMind = mindContainer.HasMind;
    ...
    private bool IsHiddenFromGhostWarps(EntityUid entity) => _tag.HasTag(entity, "HideFromGhostWarps");
