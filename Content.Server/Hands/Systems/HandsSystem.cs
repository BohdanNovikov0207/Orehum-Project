...
public sealed class HandsSystem : SharedHandsSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<HandsComponent, DisarmedEvent>(OnDisarmed,
            new[] { typeof(StunSystem), typeof(SharedStaminaSystem) });
    ...
    private void OnDisarmed(EntityUid uid, HandsComponent component, ref DisarmedEvent args)
    {
        ...
        if (TryComp(uid, out PullerComponent? puller) && TryComp(puller.Pulling, out PullableComponent? pullable))
            _pullingSystem.TryStopPull(puller.Pulling.Value,
                pullable,
                ignoreGrab: true); // Goobstation edit added check for grab

        var offsetRandomCoordinates =
            _transformSystem.GetMoverCoordinates(args.Target).Offset(_random.NextVector2(1f, 1.5f));
    ...
    public bool ThrowHeldItem(EntityUid player, EntityCoordinates coordinates, float minDistance = 0.1f)
    {
        ...
        var ev = new BeforeThrowEvent(throwEnt.Value,
            direction * (1f + modifier * 0.1f),
            throwSpeed * (1f + modifier * 0.05f),
            player);
        // Goobstation end
        ...
        // Goob edit start
        if (!ev.GrabThrow)
            _throwingSystem.TryThrow(ev.ItemUid,
                ev.Direction,
                ev.ThrowSpeed,
                ev.PlayerUid,
