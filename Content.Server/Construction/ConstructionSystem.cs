...
public sealed partial class ConstructionSystem : SharedConstructionSystem
{
    ...
    private void OnConstructionInit(Entity<ConstructionComponent> ent, ref ComponentInit args)
    {
        ...
        {
            Log.Warning(
                $"Prototype {Comp<MetaDataComponent>(ent).EntityPrototype?.ID}'s construction component has an invalid graph specified.");
            return;
        }
        ...
        {
            Log.Warning(
                $"Prototype {Comp<MetaDataComponent>(ent).EntityPrototype?.ID}'s construction component has an invalid node specified.");
            return;
        }
        ...
        if (construction.EdgeIndex is { } edgeIndex)
        {
            ...
            {
                Log.Warning(
                    $"Prototype {Comp<MetaDataComponent>(ent).EntityPrototype?.ID}'s construction component has an invalid edge index specified.");
                return;
            }
        ...
        if (construction.TargetNode is { } targetNodeId)
        {
            ...
            {
                Log.Warning(
                    $"Prototype {Comp<MetaDataComponent>(ent).EntityPrototype?.ID}'s construction component has an invalid target node specified.");
                return;
            }
