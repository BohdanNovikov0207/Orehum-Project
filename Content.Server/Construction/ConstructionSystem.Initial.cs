...
public sealed partial class ConstructionSystem
{
    ...
    private IEnumerable<EntityUid> EnumerateNearby(EntityUid user)
    {
        ...
        foreach (var near in _lookupSystem.GetEntitiesInRange(pos,
                     ConstructGrabRange,
                     LookupFlags.Contained | LookupFlags.Dynamic | LookupFlags.Sundries | LookupFlags.Approximate))
        {
            if (near == user)
                continue;
            if (_interactionSystem.InRangeUnobstructed(pos, near, ConstructGrabRange) &&
        ...
        Angle angle = default)
    {
        ...
        if (existed)
        {
            _popup.PopupEntity(Loc.GetString("construction-system-construct-cannot-start-another-construction"),
                user,
                user);
            return null;
        }
            ...
            switch (step)
            {
                case MaterialConstructionGraphStep materialStep:
                    foreach (var entity in EnumerateNearby(user))
                    {
                        ...
                        var splitStack =
                            _stackSystem.Split(entity, materialStep.Amount, user.ToCoordinates(0, 0), stack);
        ...
        var newEntityProto = graph.Nodes[edge.Target].Entity.GetId(null, user, new GraphNodeEntityArgs(EntityManager));
        ...
        foreach (var (name, cont) in containers)
        {
            ...
            foreach (var entity in cont.ContainedEntities.ToArray())
            {
                _container.Remove(entity, cont, false, true);
    ...
    // LEGACY CODE. See warning at the top of the file!
    private async void HandleStartStructureConstruction(TryStartStructureConstructionMessage ev,
