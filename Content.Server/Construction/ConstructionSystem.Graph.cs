...
public sealed partial class ConstructionSystem
{
    ...
    /// the entity does not have a <see cref="ConstructionComponent" />.
    /// </returns>
    ...
    /// does not have a <see cref="ConstructionComponent" />.
    /// </returns>
    ...
    /// null if the entity does not have a <see cref="ConstructionComponent" />.
    /// </returns>
    ...
    /// </summary>
    public (ConstructionGraphNode?, ConstructionGraphEdge?) GetCurrentNodeAndEdge(EntityUid uid,
    ...
    /// <see cref="ConstructionComponent" />.
    /// </returns>
    ...
    public ConstructionGraphNode? GetNodeFromGraph(ConstructionGraphPrototype graph, string id) =>
        graph.Nodes.TryGetValue(id, out var node) ? node : null;
    ...
    public ConstructionGraphEdge? GetEdgeFromNode(ConstructionGraphNode node, int index) =>
        node.Edges.Count > index ? node.Edges[index] : null;
    ...
    public ConstructionGraphStep? GetStepFromEdge(ConstructionGraphEdge edge, int index) =>
        edge.Steps.Count > index ? edge.Steps[index] : null;
    ...
    public bool ChangeNode(EntityUid uid,
        EntityUid? userUid,
        string id,
        ...
        ConstructionComponent? construction = null)
    {
        ...
        if (node.Entity.GetId(uid, userUid, new GraphNodeEntityArgs(EntityManager)) is { } newEntity
    ...
    private EntityUid? ChangeEntity(EntityUid uid,
        EntityUid? userUid,
        string newEntity,
        ConstructionComponent? construction = null,
        ...
        ContainerManagerComponent? containerManager = null)
    {
        ...
        if (containerManager != null)
        {
            ...
            foreach (var container in construction.Containers)
            {
                ...
                for (var i = ourContainer.ContainedEntities.Count - 1; i >= 0; i--)
                {
                    ...
                    _container.Remove(entity, ourContainer, false, true);
        ...
        RaiseLocalEvent(newUid, entChangeEv, true);

        foreach (var logic in GetCurrentNode(newUid, newConstruction)!.TransformLogic)
        {
            logic.Transform(uid, newUid, userUid, new GraphTransformArgs(EntityManager));
        }
    ...
    /// a <see cref="ConstructionComponent" />.
    /// </returns>
    public bool ChangeGraph(EntityUid uid,
        EntityUid? userUid,
        string graphId,
        string nodeId,
        bool performActions = true,
        ConstructionComponent? construction = null)
    {
        ...
        if (GetNodeFromGraph(graph, nodeId) is null)
            return false;
