...
public abstract class ServerDbBase
{
    ...
    protected DateTime? NormalizeDatabaseTime(DateTime? time) =>
        time != null ? NormalizeDatabaseTime(time.Value) : time;
    ...
    protected void LogDbOp(string? name) => _opsLog.Verbose($"Running DB operation: {name ?? "unknown"}");
        ...
        CancellationToken cancel = default)
    {
        ...
        return new PlayerPreferences(profiles,
    ...
    public async Task<PlayerPreferences> InitPrefsAsync(NetUserId userId, ICharacterProfile defaultProfile)
    {
        ...
        return new PlayerPreferences(new[] { new KeyValuePair<int, ICharacterProfile>(0, defaultProfile) },
            0,
            Color.FromHex(prefs.AdminOOCColor),
            []);
    }
    ...
    public async Task SaveConstructionFavoritesAsync(NetUserId userId,
    ...
    public async Task EditServerBan(int id,
        string reason,
        ...
        CancellationToken cancel = default)
    {
        ...
        var exemption = await db.DbContext.BanExemption
            .SingleOrDefaultAsync(e => e.UserId == userId.Value.UserId, cancel);
    ...
    public async Task EditServerRoleBan(int id,
        string reason,
        NoteSeverity severity,
        DateTimeOffset? expiration,
        Guid editedBy,
    ...
    protected async Task<bool> PlayerRecordExists(DbGuard db, NetUserId userId) =>
        await db.DbContext.Player.AnyAsync(p => p.UserId == userId);
    ...
    {
        await using var db = await GetDb();
        var lastRolled = await db.DbContext.Player
            .Where(dbPlayer => dbPlayer.UserId == userId)
    ...
    public async Task EditAdminNote(int id,
        string message,
        NoteSeverity severity,
        bool secret,
        Guid editedBy,
    ...
    public async Task EditAdminWatchlist(int id,
        string message,
        Guid editedBy,
        DateTimeOffset editedAt,
        DateTimeOffset? expiryTime)
    ...
    public async Task EditAdminMessage(int id,
        string message,
        Guid editedBy,
        DateTimeOffset editedAt,
        DateTimeOffset? expiryTime)
    ...
    private AdminWatchlistRecord MakeAdminWatchlistRecord(AdminWatchlist entity) => new(entity.Id,
        MakeRoundRecord(entity.Round),
        MakePlayerRecord(entity.Player),
        entity.PlaytimeAtNote,
        entity.Message,
    ...
    public async Task<bool> AddPollVoteAsync(int pollId,
        int optionId,
        NetUserId userId,
        CancellationToken cancel = default)
    {
    ...
    public async Task<bool> RemovePollVoteAsync(int pollId,
        int optionId,
        NetUserId userId,
        CancellationToken cancel = default)
    {
        ...
        var vote = await db.DbContext.PollVotes
            .FirstOrDefaultAsync(v =>
                    v.PollId == pollId && v.PollOptionId == optionId && v.PlayerUserId == userId.UserId,
                cancel);
    ...
    public async Task<List<PollVote>> GetPlayerVotesAsync(int pollId,
        NetUserId userId,
        CancellationToken cancel = default)
    {
        await using var db = await GetDb(cancel);
