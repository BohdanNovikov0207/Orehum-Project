...
public sealed class ServerBanDef
{
    ...
    public string FormatBanMessage(IConfigurationManager cfg, ILocalizationManager loc)
    {
        ...
        if (ExpirationTime is { } expireTime)
        {
            ...
            expires = loc.GetString("ban-expires",
    ...
    private static string GetUsername(string? userId)
    {
        ...
        {
            var apiUrl = "https://auth.spacestation14.com/api/query/userid?userid=" + userId;

            var response = client.Send(new HttpRequestMessage(HttpMethod.Get, apiUrl));
