...
internal sealed partial class PointingSystem : SharedPointingSystem // Goobstation: Partial Class
{
    ...
    public bool InRange(EntityUid pointer, EntityCoordinates coordinates)
    {
        ...
        return _examine.InRangeUnOccluded(pointer, coordinates, 15, e => e == pointer);
    }
    ...
    public bool TryPoint(ICommonSession? session, EntityCoordinates coordsPointed, EntityUid pointed)
    {
        ...
        {
            pointing.StartPosition = _transform.ToCoordinates((arrow, Transform(arrow)),
        ...
        if (Exists(pointed))
        {
            ...
            if (containingInventory != null)
            {
                ...
                if (pointingAtOwnItem)
                {
                    ...
                    viewerMessage = Loc.GetString("pointing-system-point-in-own-inventory-others",
                        ("item", itemName),
                        ("pointer", playerName));
                }
                else
        ...
        else
        {
            ...
            viewerMessage = Loc.GetString("pointing-system-other-point-at-tile",
                ("otherName", playerName),
                ("tileName", name));
