...
public sealed partial class VoteManager : IVoteManager
{
    ...
    public void Initialize()
    {
        ...
        _cfg.OnValueChanged(CCVars.VoteEnabled,
            _ =>
            {
                DirtyCanCallVoteAll();
            });
    ...
    public bool CanCallVote(ICommonSession initiator, StandardVoteType? voteType = null) =>
        CanCallVote(initiator, voteType, out _, out _);
    ...
    private void AdminPermsChanged(AdminPermsChangedEventArgs obj) => DirtyCanCallVote(obj.Player);
    ...
    private bool IsValidOption(VoteReg voteReg, int? option) =>
        option == null || option >= 0 && option < voteReg.Entries.Length;
    ...
    private void DirtyCanCallVoteAll() => _playerCanCallVoteDirty.UnionWith(_playerManager.Sessions);
    ...
    public bool CheckVoterEligibility(ICommonSession player, VoterEligibility eligibility)
    {
        ...
        if (eligibility == VoterEligibility.Ghost || eligibility == VoterEligibility.GhostMinimumPlaytime)
        {
            ...
            if (eligibility == VoterEligibility.GhostMinimumPlaytime)
            {
                ...
                if ((int) _timing.RealTime.Subtract(ghostComp.TimeOfDeath).TotalSeconds <
    ...
    private void DirtyCanCallVote(ICommonSession player) => _playerCanCallVoteDirty.Add(player);
    ...
    private sealed class VoteHandle : IVoteHandle
    {
        ...
        public bool IsValidOption(int optionId) => _mgr.IsValidOption(_reg, optionId);

        public void CastVote(ICommonSession session, int? optionId) => _mgr.CastVote(_reg, session, optionId);

        public void Cancel() => _mgr.CancelVote(_reg);
        ...
        private sealed class VoteDict : IReadOnlyDictionary<object, int>
        {
            ...
            public IEnumerator<KeyValuePair<object, int>> GetEnumerator() =>
                _reg.Entries.Select(e => KeyValuePair.Create(e.Data, e.Votes)).GetEnumerator();
