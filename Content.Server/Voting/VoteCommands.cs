...
public sealed class CreateVoteCommand : LocalizedEntityCommands
{
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (!Enum.TryParse<StandardVoteType>(args[0], true, out var type))
        {
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        vote.OnFinished += (_, eventArgs) =>
        {
            if (eventArgs.Winner == null)
            {
                ...
                _chatManager.DispatchServerAnnouncement(Loc.GetString("cmd-customvote-on-finished-tie",
                    ("title", options.Title),
                    ("ties", ties)));
            }
            else
            {
                ...
                _chatManager.DispatchServerAnnouncement(Loc.GetString("cmd-customvote-on-finished-win",
                    ("title", options.Title),
                    ("winner", args[(int) eventArgs.Winner])));
            }
