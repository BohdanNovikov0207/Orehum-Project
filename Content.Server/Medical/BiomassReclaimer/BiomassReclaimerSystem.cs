...
public sealed class BiomassReclaimerSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out _, out var reclaimer))
        {
            ...
            if (reclaimer.RandomMessTimer <= 0)
            {
                ...
                {
                    var thrown = Spawn(_robustRandom.Pick(reclaimer.SpawnedEntities).PrototypeId,
            ...
            reclaimer.CurrentExpectedYield =
                reclaimer.CurrentExpectedYield - actualYield; // store non-integer leftovers
    ...
    private void OnSuicideByEnvironment(Entity<BiomassReclaimerComponent> ent, ref SuicideByEnvironmentEvent args)
    {
        ...
        _popup.PopupEntity(Loc.GetString("biomass-reclaimer-suicide-others", ("victim", args.Victim)),
            ent,
    ...
    private void
        OnUnanchorAttempt(EntityUid uid, ActiveBiomassReclaimerComponent component, UnanchorAttemptEvent args) =>
        args.Cancel();
    ...
    private void OnAfterInteractUsing(Entity<BiomassReclaimerComponent> reclaimer, ref AfterInteractUsingEvent args)
    {
        ...
        var canProcess = CanGib(reclaimer, args.Used);
    ...
    private void
        OnDragDropTarget(Entity<BiomassReclaimerComponent> reclaimer, ref DragDropTargetEvent args) // # GoobStation
    {
    ...
    private void StartProcessing(EntityUid toProcess,
        ...
        PhysicsComponent? physics = null)
    {
        ...
        var canProcess = CanGib(ent, toProcess);
    ...
    private void AddToStartingProcess(EntityUid toProcess,
        Entity<BiomassReclaimerComponent> ent,
        PhysicsComponent physics)
    {
        ...
        if (TryComp<BloodstreamComponent>(toProcess, out var stream))
            component.BloodReagents.Add(stream.BloodReagent); // Goobstation
