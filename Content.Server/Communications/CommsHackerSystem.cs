...
public sealed class CommsHackerSystem : SharedCommsHackerSystem
{
    ...
    private void OnDoAfter(EntityUid uid, CommsHackerComponent comp, TerrorDoAfterEvent args)
    {
        if (args.Cancelled || args.Handled || args.Target == null ||
            !_powerReceiverSystem.IsPowered(args.Target.Value)) // Goobsstation - is powered
            return;
    ...
    public void CallInThreat(NinjaHackingThreatPrototype ninjaHackingThreat)
    {
        ...
        _chat.DispatchGlobalAnnouncement(Loc.GetString(ninjaHackingThreat.Announcement),
            playSound: true,
...
/// For example, you could add a marker component after a threat is called in then check if the user doesn't have that
/// marker before adding CommsHackerComponent.
/// </remarks>
[ByRefEvent]
