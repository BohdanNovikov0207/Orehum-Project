...
public sealed class DroneSystem : SharedDroneSystem
{
    ...
    private void OnUseAttempt(EntityUid uid, DroneComponent component, UseAttemptEvent args)
    {
        if (_whitelist.IsWhitelistPass(component.Whitelist, args.Used) &&
            NonDronesInRange(uid,
                component)) /// tag whitelist. sends proximity warning popup if the item isn't whitelisted. Doesn't prevent actions. Takes precedent over blacklist.
        {
            ...
            (_whitelist.IsWhitelistPass(component.Blacklist,
                 args.Used)) // imp special. blacklist. this one *does* prevent actions. it would probably be best if this read from the component or something.
        {
            args.Cancel();
            if (_gameTiming.CurTime >= component.NextProximityAlert)
            {
                _popupSystem.PopupEntity(Loc.GetString("drone-cant-use-nearby", ("being", component.NearestEnt)),
                    uid,
                    uid);
                component.NextProximityAlert = _gameTiming.CurTime + component.ProximityDelay;
            }
    ...
    public void UpdateUI(EntityUid uid, DroneComponent? component = null)
    {
        ...
        if (_powerCell.TryGetBatteryFromSlot(uid, out var battery) &&
            _battery.TryGetBatteryComponent(uid, out var batteryComponent, out _))
        {
            hasBattery = true;
            chargePercent =
                MathF.Round(batteryComponent.CurrentCharge / batteryComponent.MaxCharge * 10f); // max is 10 min is 0
        }
    ...
    private void UpdateBatteryAlert(Entity<DroneComponent> ent)
    {
        ...
        if (_battery.TryGetBatteryComponent(ent.Owner, out var batteryComponent, out _))
        ...
        if (chargePercent == 5 && chargePercent < ent.Comp.LastChargePercent)
        {
            ...
            {
                _popupSystem.PopupEntity(Loc.GetString("drone-med-battery"),
                    ent.Owner,
                    ent.Owner,
        ...
        if (chargePercent == 2 && chargePercent < ent.Comp.LastChargePercent)
        {
            if (_gameTiming.CurTime >= ent.Comp.NextProximityAlert)
            {
                _popupSystem.PopupEntity(Loc.GetString("drone-low-battery"),
                    ent.Owner,
                    ent.Owner,
    ...
    private bool NonDronesInRange(EntityUid uid, DroneComponent component)
    {
        ...
        foreach (var entity in _lookup.GetEntitiesInRange(_transform.GetMapCoordinates(xform),
                     component.InteractionBlockRange))
        {
            // Return true if the entity is/was controlled by a player and is not a drone or ghost.
            if (HasComp<MindContainerComponent>(entity) && !HasComp<DroneComponent>(entity) &&
                !HasComp<GhostComponent>(entity))
            {
                ...
                if (_gameTiming.IsFirstTimePredicted)
                    component.NearestEnt =
                        Identity.Entity(entity,
                            EntityManager); // imp. instead of doing popups in here, set a variable to the nearest entity for use elsewhere.
                return true;
            }
        }
