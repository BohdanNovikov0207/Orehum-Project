...
public sealed class ExtensionCableSystem : EntitySystem
{
    ...
    private void OnProviderAnchorStateChanged(Entity<ExtensionCableProviderComponent> provider,
    ...
    private void Connect(Entity<ExtensionCableProviderComponent> provider)
    {
        ...
        foreach (var receiver in FindAvailableReceivers(provider.Owner, provider.Comp.TransferRange))
        {
            ...
            RaiseLocalEvent(receiver, new ProviderConnectedEvent(provider), false);
            RaiseLocalEvent(provider, new ReceiverConnectedEvent(receiver), false);
        }
    }
    ...
    private void ResetReceivers(Entity<ExtensionCableProviderComponent> provider)
    {
        ...
        foreach (var receiver in receivers)
        {
            ...
            RaiseLocalEvent(receiverId, new ProviderDisconnectedEvent(provider), false);
            RaiseLocalEvent(providerId, new ReceiverDisconnectedEvent((receiverId, receiver)), false);
        }
    ...
    private IEnumerable<Entity<ExtensionCableReceiverComponent>> FindAvailableReceivers(EntityUid owner, float range)
    {
        ...
        var nearbyEntities = _map.GetCellsInSquareArea(xform.GridUid.Value,
            grid,
            coordinates,
        ...
        foreach (var entity in nearbyEntities)
        {
            ...
            if ((Transform(entity).LocalPosition - xform.LocalPosition).Length() <=
    ...
    public void SetReceiverReceptionRange(EntityUid uid, int range, ExtensionCableReceiverComponent? receiver = null)
    {
        ...
        RaiseLocalEvent(uid, new ProviderDisconnectedEvent(provider), false);

        if (provider != null)
        {
            RaiseLocalEvent(provider.Value, new ReceiverDisconnectedEvent((uid, receiver)), false);
    ...
    private void OnReceiverAnchorStateChanged(Entity<ExtensionCableReceiverComponent> receiver,
        ref AnchorStateChangedEvent args)
    {
        if (args.Anchored)
            Connect(receiver);
    ...
    private void Disconnect(Entity<ExtensionCableReceiverComponent> receiver)
    {
        ...
        RaiseLocalEvent(receiver, new ProviderDisconnectedEvent(receiver.Comp.Provider), false);
        if (receiver.Comp.Provider != null)
        {
            RaiseLocalEvent(receiver.Comp.Provider.Value, new ReceiverDisconnectedEvent(receiver), false);
    ...
    private void TryFindAndSetProvider(Entity<ExtensionCableReceiverComponent> receiver,
        TransformComponent? xform = null)
    {
        ...
        RaiseLocalEvent(uid, new ProviderConnectedEvent(provider), false);
        RaiseLocalEvent(provider.Value, new ReceiverConnectedEvent((uid, receiver)), false);
    }

    private bool TryFindAvailableProvider(EntityUid owner,
        float range,
    ...
    /// <summary>
    /// Sent when a <see cref="ExtensionCableProviderComponent" /> disconnects from a
    /// <see cref="ExtensionCableReceiverComponent" />
    /// </summary>
    ...
    /// <summary>
    /// Sent when a <see cref="ExtensionCableReceiverComponent" /> disconnects from a
    /// <see cref="ExtensionCableProviderComponent" />
    /// </summary>
