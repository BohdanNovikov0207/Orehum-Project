...
public sealed class PowerNetSystem : SharedPowerNetSystem
{
    ...
    public override void Initialize()
    {
        ...
        _solver = new BatteryRampPegSolver(_cfg.GetCVar(CCVars.DebugPow3rDisableParallel));
    ...
    private void DebugPow3rDisableParallelChanged(bool val) => _solver = new BatteryRampPegSolver(val);
    ...
    private void ApcPowerReceiverShutdown(EntityUid uid,
    ...
    private static void
        BatteryPaused(EntityUid uid, PowerNetworkBatteryComponent component, ref EntityPausedEvent args) =>
        component.NetworkBattery.Paused = true;

    private static void BatteryUnpaused(EntityUid uid,
        PowerNetworkBatteryComponent component,
        ref EntityUnpausedEvent args) => component.NetworkBattery.Paused = false;
    ...
    private static void
        PowerConsumerPaused(EntityUid uid, PowerConsumerComponent component, ref EntityPausedEvent args) =>
        component.NetworkLoad.Paused = true;

    private static void PowerConsumerUnpaused(EntityUid uid,
        PowerConsumerComponent component,
        ref EntityUnpausedEvent args) => component.NetworkLoad.Paused = false;
    ...
    private static void
        PowerSupplierPaused(EntityUid uid, PowerSupplierComponent component, ref EntityPausedEvent args) =>
        component.NetworkSupply.Paused = true;

    private static void PowerSupplierUnpaused(EntityUid uid,
        PowerSupplierComponent component,
        ref EntityUnpausedEvent args) => component.NetworkSupply.Paused = false;
    ...
    private void AllocLoad(PowerState.Load load) => _powerState.Loads.Allocate(out load.Id) = load;

    private void AllocSupply(PowerState.Supply supply) => _powerState.Supplies.Allocate(out supply.Id) = supply;

    private void AllocBattery(PowerState.Battery battery) => _powerState.Batteries.Allocate(out battery.Id) = battery;

    private void AllocNetwork(PowerState.Network network) => _powerState.Networks.Allocate(out network.Id) = network;
    ...
    /// </summary>
    public void Validate() => _solver.Validate(_powerState);
}
