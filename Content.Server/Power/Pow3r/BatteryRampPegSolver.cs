...
public sealed class BatteryRampPegSolver : IPowerSolver
{
    ...
    public BatteryRampPegSolver(bool disableParallel = false)
    {
        ...
        _networkJob = new UpdateNetworkJob
        {
            Solver = this,
        };
    }
    ...
    private void UpdateNetwork(Network network, PowerState state, float frameTime)
    {
        ...
        foreach (var batteryId in network.BatteryLoads)
        {
            ...
            DebugTools.Assert(battery.CurrentStorage <= battery.Capacity ||
        ...
        foreach (var batteryId in network.BatterySupplies)
        {
            ...
            DebugTools.Assert(battery.MaxEffectiveSupply * relativeTargetBatteryOutput <= battery.LoadingNetworkDemand
                              || MathHelper.CloseToPercent(battery.MaxEffectiveSupply * relativeTargetBatteryOutput,
                                  battery.LoadingNetworkDemand,
                                  0.001));
        }
    }
    ...
    private static void RecursivelyEstimateNetworkDepth(PowerState state,
        Network network,
        List<List<Network>> groupedNetworks)
    {
        ...
        if (network.Height >= groupedNetworks.Count)
            groupedNetworks.Add(new List<Network> { network });
        else
    ...
    private record struct UpdateNetworkJob : IParallelRobustJob
    {
        ...
        public void Execute(int index) => Solver.UpdateNetwork(Networks[index], State, FrameTime);
    }
