...
public sealed partial class PathfindingSystem : SharedPathfindingSystem
{
    ...
    private PathRequest GetRequest(EntityUid entity,
        EntityCoordinates start,
        EntityCoordinates end,
        float range,
        CancellationToken cancelToken,
    ...
    public PathFlags GetFlags(NPCBlackboard blackboard)
    {
        ...
        if (blackboard.TryGetValue<bool>(NPCBlackboard.NavBlob, out var blob, EntityManager) &&
...
#pragma warning restore RA0004
    ...
    private void SendDebug(PathRequest request)
    {
        ...
        foreach (var session in _subscribedSessions)
        {
            ...
            RaiseNetworkEvent(new PathRouteMessage(request.Polys.Select(GetDebugPoly).ToList(),
    ...
    private bool IsCrumb(PathfindingDebugMode mode) =>
        (mode & (PathfindingDebugMode.Breadcrumbs | PathfindingDebugMode.Crumb)) != 0x0;

    private bool IsPoly(PathfindingDebugMode mode) => (mode & (PathfindingDebugMode.Chunks |
                                                               PathfindingDebugMode.Polys | PathfindingDebugMode.Poly |
                                                               PathfindingDebugMode.PolyNeighbors)) != 0x0;

    private bool IsRoute(PathfindingDebugMode mode) =>
        (mode & (PathfindingDebugMode.Routes | PathfindingDebugMode.RouteCosts)) != 0x0;
    ...
    private void SendPolys(GridPathfindingChunk chunk,
        EntityUid gridUid,
