...
public sealed class ExpelGasEffectSystem : EntitySystem
{
    ...
    public override void Initialize() => SubscribeLocalEvent<ExpelGasComponent, ComponentInit>(OnInit);

    private void OnInit(EntityUid uid, ExpelGasComponent component, ComponentInit args)
    {
        var mix = _atmos.GetContainingMixture((uid, Transform(uid)), true, true) ?? new GasMixture();
