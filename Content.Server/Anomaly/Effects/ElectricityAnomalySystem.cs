...
public sealed class ElectricityAnomalySystem : EntitySystem
{
    ...
    private void OnPulse(Entity<ElectricityAnomalyComponent> anomaly, ref AnomalyPulseEvent args)
    {
        ...
        var boltCount =
            (int) MathF.Floor(MathHelper.Lerp(anomaly.Comp.MinBoltCount, anomaly.Comp.MaxBoltCount, args.Severity));
    ...
    private void OnSupercritical(Entity<ElectricityAnomalyComponent> anomaly, ref AnomalySupercriticalEvent args)
    {
        ...
        _emp.EmpPulse(_transform.GetMapCoordinates(anomaly),
            range,
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var elec, out var anom, out var xform))
        {
            ...
            foreach (var (ent, comp) in _lookup.GetEntitiesInRange<StatusEffectsComponent>(
                         _transform.GetMapCoordinates(uid, xform),
                         range))
            {
                _electrocution.TryDoElectrocution(ent,
                    uid,
                    damage,
                    duration,
                    true,
