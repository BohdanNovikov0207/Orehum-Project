...
public sealed class InjectionAnomalySystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<InjectionAnomalyComponent, AnomalySupercriticalEvent>(OnSupercritical,
            new[] { typeof(SharedSolutionContainerSystem) });

        _injectableQuery = GetEntityQuery<InjectableSolutionComponent>();
    }

    private void OnPulse(Entity<InjectionAnomalyComponent> entity, ref AnomalyPulseEvent args) => PulseScalableEffect(
        entity,
    ...
    private void PulseScalableEffect(Entity<InjectionAnomalyComponent> entity, float injectRadius, float maxInject)
    {
        ...
        var allEnts = _lookup
            .GetEntitiesInRange<InjectableSolutionComponent>(_transform.GetMapCoordinates(entity, xform), injectRadius)
            .Select(x => x.Owner)
            .ToList();
