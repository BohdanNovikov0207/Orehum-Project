...
public sealed partial class BwoinkSystem : SharedBwoinkSystem
{
    ...
    private async void OnWebhookChanged(string url)
    {
        ...
        if (!match.Success)
        {
            ...
            await GetWebhookData(
                url); // Frontier - Support for Custom URLS, we still want to see if theres Webhook data available
            return;
        }
    ...
    private async void ProcessQueue(NetUserId userId, Queue<DiscordRelayedData> messages)
    {
            ...
            _onCallData != null)
        {
            ...
            if (!string.IsNullOrEmpty(roleMention))
            {
                ...
                payload = GeneratePayload(message.ToString(),
                    existingEmbed.Username,
                    userId,
                    existingEmbed.CharacterName);
                ...
                if (!request.IsSuccessStatusCode)
                    _sawmill.Log(LogLevel.Error,
                        $"Discord returned bad status code when posting relay message (perhaps the message is too long?): {request.StatusCode}\nResponse: {content}");
            }
        }
        else
    ...
    private WebhookPayload
        GeneratePayload(string messages,
            string username,
            Guid userId,
            string? characterName = null) // Frontier: added Guid
    {
        // Add character name
    ...
    /// <summary>
    /// Sends a bwoink. Common to both internal messages (sent via the ahelp or admin interface) and webhook messages (sent
    /// through the webhook, e.g. via Discord)
    /// </summary>
    ...
    private void OnBwoinkInternal(BwoinkParams bwoinkParams)
    {
        ...
        if (bwoinkParams.SenderAdmin is not null)
        {
            ...
            else if (bwoinkParams.FromWebhook ||
                     bwoinkParams.SenderAdmin.HasFlag(AdminFlags
                         .Adminhelp)) // Frontier: anything sent via webhooks are from an admin.
        ...
        bwoinkText =
            $"{(bwoinkParams.Message.AdminOnly ? Loc.GetString("bwoink-message-admin-only") : !bwoinkParams.Message.PlaySound ? Loc.GetString("bwoink-message-silent") : "")} {bwoinkText}: {escapedText}";
        ...
        var playSound = (bwoinkParams.SenderAdmin == null || bwoinkParams.Message.PlaySound) &&
                        !bwoinkParams.Message.AdminOnly;
        var msg = new BwoinkTextMessage(bwoinkParams.Message.UserId,
        ...
        if (_config.GetCVar(CCVars.AhelpAdminPrefixWebhook) && bwoinkParams.SenderAdmin is not null &&
            bwoinkParams.SenderAdmin.Title is not null)
            ...
            !bwoinkParams.Message.AdminOnly)
        {
            if (!admins.Contains(session.Channel))
            {
                ...
                if (_overrideClientName != string.Empty)
                {
                    ...
                    else if (bwoinkParams.SenderAdmin is not null &&
                    ...
                    overrideMsgText =
                        $"{(bwoinkParams.Message.PlaySound ? "" : "(S) ")}{overrideMsgText}: {escapedText}";
