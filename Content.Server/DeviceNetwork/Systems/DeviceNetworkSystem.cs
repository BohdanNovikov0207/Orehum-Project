...
/// Device networking allows machines and devices to communicate with each other while adhering to restrictions like range
/// or being connected to the same powernet.
/// </summary>
[UsedImplicitly]
public sealed class DeviceNetworkSystem : SharedDeviceNetworkSystem
{
    ...
    public override bool QueuePacket(EntityUid uid,
        string? address,
        NetworkPayload data,
        uint? frequency = null,
        int? network = null,
        DeviceNetworkComponent? device = null)
    {
        ...
        _nextQueue.Enqueue(new DeviceNetworkPacketEvent(network.Value,
            address,
            frequency.Value,
            device.Address,
            uid,
    ...
    /// <summary>
    /// Sends the <see cref="BeforeBroadcastAttemptEvent" /> to the sending entity if the packets
    /// SendBeforeBroadcastAttemptEvent field is set to true.
