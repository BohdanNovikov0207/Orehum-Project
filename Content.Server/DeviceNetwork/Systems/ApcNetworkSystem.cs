...
public sealed class ApcNetworkSystem : EntitySystem
{
    ...
    private void OnBeforePacketSent(EntityUid uid, ApcNetworkComponent receiver, BeforePacketSentEvent args)
    {
        ...
        if (sender.ConnectedNode?.NodeGroup == null ||
            !sender.ConnectedNode.NodeGroup.Equals(receiver.ConnectedNode?.NodeGroup))
            args.Cancel();
    }

    private void OnProviderConnected(EntityUid uid,
        ApcNetworkComponent component,
        ExtensionCableSystem.ProviderConnectedEvent args)
    {
        if (!TryComp(args.Provider.Owner, out NodeContainerComponent? nodeContainer))
            return;
    ...
    private void OnProviderDisconnected(EntityUid uid,
        ApcNetworkComponent component,
        ExtensionCableSystem.ProviderDisconnectedEvent args) => component.ConnectedNode = null;
}
