...
public sealed class EmoteOnDamageSystem : EntitySystem
{
    ...
    /// </summary>
    public bool AddEmote(EntityUid uid,
        float threshold,
        string emotePrototypeId,
        EmoteOnDamageComponent? emoteOnDamage = null) // CorvaxGoob-AutoEmote : added threshold arg
    {
        if (!Resolve(uid, ref emoteOnDamage, false))
            return false;

        DebugTools.Assert(emoteOnDamage.LifeStage <= ComponentLifeStage.Running);
        DebugTools.Assert(_prototypeManager.HasIndex<EmotePrototype>(emotePrototypeId),
    ...
    public bool RemoveEmote(EntityUid uid,
        ...
        bool removeEmpty = true) // CorvaxGoob-AutoEmote : added threshold arg
    {
        if (!Resolve(uid, ref emoteOnDamage, false))
            return false;
