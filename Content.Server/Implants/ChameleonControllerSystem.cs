...
public sealed class ChameleonControllerSystem : SharedChameleonControllerSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ChameleonClothingComponent, InventoryRelayedEvent<ChameleonControllerOutfitSelectedEvent>>(
            ChameleonControllerOutfitItemSelected);
    }

    private void OnSelected(Entity<SubdermalImplantComponent> ent, ref ChameleonControllerSelectedOutfitMessage args)
    {
        if (!_delay.TryResetDelay(ent.Owner, true) || ent.Comp.ImplantedEntity == null ||
            !HasComp<ChameleonControllerImplantComponent>(ent))
            return;
    ...
    private void ChangeChameleonClothingToOutfit(EntityUid user, ProtoId<ChameleonOutfitPrototype> outfit)
    {
        ...
        GetJobEquipmentInformation(jobPrototype,
            user,
            out var customRoleLoadout,
            out var defaultRoleLoadout,
            out var jobStartingGearPrototype);
    ...
    private void ChameleonControllerOutfitItemSelected(Entity<ChameleonClothingComponent> ent,
        ref InventoryRelayedEvent<ChameleonControllerOutfitSelectedEvent> args)
    {
    ...
    public string? GetGearForSlot(InventoryRelayedEvent<ChameleonControllerOutfitSelectedEvent> ev, string slotName) =>
        GetGearForSlot(ev.Args.ChameleonOutfit,
            ev.Args.CustomRoleLoadout,
            ev.Args.DefaultRoleLoadout,
            ev.Args.JobStartingGearPrototype,
            ev.Args.StartingGearPrototype,
            slotName);
    ...
    public string? GetGearForSlot(ChameleonOutfitPrototype? chameleonOutfitPrototype,
        ...
        string slotName)
    {
        ...
        if (chameleonOutfitPrototype != null &&
            chameleonOutfitPrototype.Equipment.TryGetValue(slotName, out var forSlot))
            return forSlot;

        var startingGear = startingGearPrototype != null
            ? ((IEquipmentLoadout) startingGearPrototype).GetGear(slotName)
            : "";
        if (startingGear != "")
            return startingGear;
        ...
        var jobStartingGear = jobStartingGearPrototype != null
            ? ((IEquipmentLoadout) jobStartingGearPrototype).GetGear(slotName)
            : "";
        if (jobStartingGear != "")
