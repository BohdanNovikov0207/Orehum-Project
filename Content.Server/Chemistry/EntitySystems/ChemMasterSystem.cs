...
public sealed class ChemMasterSystem : EntitySystem
{
    ...
    private void SubscribeUpdateUiState<T>(Entity<ChemMasterComponent> ent, ref T ev) => UpdateUiState(ent);
    ...
    private void OnCycleSortingTypeMessage(Entity<ChemMasterComponent> chemMaster,
    ...
    private void OnReagentButtonMessage(Entity<ChemMasterComponent> chemMaster,
        ref ChemMasterReagentAmountButtonMessage message)
    {
        ...
        switch (chemMaster.Comp.Mode)
        {
            case ChemMasterMode.Transfer:
                TransferReagents(chemMaster,
                    message.ReagentId,
                    message.Amount.GetFixedPoint(),
                    message.FromBuffer,
                    message.Actor); // goob - logging
                break;
    ...
    private void TransferReagents(Entity<ChemMasterComponent> chemMaster,
        ReagentId id,
        FixedPoint2 amount,
        bool fromBuffer,
        EntityUid? actor = null) // goob - logging
    {
        ...
        if (fromBuffer) // Buffer to container
        {
            ...
            amount = bufferSolution.RemoveReagent(id, amount, true);
        ...
        else // Container to buffer
        {
            ...
            if (bufferSolution.MaxVolume.Value > 0) //Goobstation - chemicalbuffer if no limit
                amount = FixedPoint2.Min(amount,
                    containerSolution.GetReagentQuantity(id),
                    bufferSolution.AvailableVolume);
        ...
        UpdateUiState(chemMaster, true);
    }

    private void DiscardReagents(Entity<ChemMasterComponent> chemMaster,
        ReagentId id,
        FixedPoint2 amount,
        bool fromBuffer)
    {
        if (fromBuffer)
        {
            if (_solutionContainerSystem.TryGetSolution(chemMaster.Owner,
                    ...
                    out _,
                    out var bufferSolution))
                bufferSolution.RemoveReagent(id, amount, true);
            else
                return;
        }
        else
        ...
        UpdateUiState(chemMaster, fromBuffer);
    }
    ...
    private void OnCreatePillsMessage(Entity<ChemMasterComponent> chemMaster, ref ChemMasterCreatePillsMessage message)
    {
        ...
        for (var i = 0; i < message.Number; i++)
        {
            ...
            _storageSystem.Insert(container, item, out _, user, storage);
    ...
    private void OnOutputToBottleMessage(Entity<ChemMasterComponent> chemMaster,
        ref ChemMasterOutputToBottleMessage message)
    {
        var user = message.Actor;
        var maybeContainer = _itemSlotsSystem.GetItemOrNull(chemMaster, SharedChemMaster.OutputSlotName);
        if (maybeContainer is not { Valid: true } container
            || !_solutionContainerSystem.TryGetSolution(container,
    ...
    private bool WithdrawFromBuffer(
        ...
        EntityUid? user,
        [NotNullWhen(true)] out Solution? outputSolution)
    {
        outputSolution = null;
    ...
    private void ClickSound(Entity<ChemMasterComponent> chemMaster) => _audioSystem.PlayPvs(chemMaster.Comp.ClickSound,
        chemMaster,
    ...
    private ContainerInfo? BuildOutputContainerInfo(EntityUid? container)
    {
        ...
        var pills = storage.Container.ContainedEntities.Select(
                (Func<EntityUid, (string, FixedPoint2 quantity)>) (pill =>
                {
