...
public sealed partial class ExplosionSystem
{
    ...
    private readonly Dictionary<EntityUid, Dictionary<Vector2i, TileData>> _airtightMap = new();
    ...
    public void UpdateAirtightMap(EntityUid gridId,
        Vector2i tile,
        ...
        EntityQuery<AirtightComponent>? query = null)
    {
        ...
        if (!_airtightMap.ContainsKey(gridId))
            _airtightMap[gridId] = new Dictionary<Vector2i, TileData>();
        ...
        {
            if (!query.Value.TryGetComponent(uid, out var airtight) ||
                !airtight.AirBlocked && !airtight.BlockExplosions) // Goob edit
                continue;
        ...
        if (blockedDirections != AtmosDirection.Invalid)
            _airtightMap[gridId][tile] = new TileData(tolerance, blockedDirections);
        else
    ...
    private void OnAirtightDamaged(EntityUid uid, AirtightComponent airtight, DamageChangedEvent args)
    {
        ...
        UpdateAirtightMap(transform.GridUid.Value,
            grid,
