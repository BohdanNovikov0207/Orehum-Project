...
public sealed class DrowsinessSystem : SharedDrowsinessSystem
{
    ...
    private void OnEffectApplied(Entity<DrowsinessStatusEffectComponent> ent, ref StatusEffectAppliedEvent args) =>
        ent.Comp.NextIncidentTime = _timing.CurTime +
                                    TimeSpan.FromSeconds(_random.NextFloat(ent.Comp.TimeBetweenIncidents.X,
                                        ent.Comp.TimeBetweenIncidents.Y));
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var drowsiness, out var statusEffect))
        {
            ...
            // Set the new time.
            drowsiness.NextIncidentTime = _timing.CurTime +
                                          TimeSpan.FromSeconds(_random.NextFloat(drowsiness.TimeBetweenIncidents.X,
                                              drowsiness.TimeBetweenIncidents.Y));

            // sleep duration
            var duration =
                TimeSpan.FromSeconds(
                    _random.NextFloat(drowsiness.DurationOfIncident.X, drowsiness.DurationOfIncident.Y));

            // Make sure the sleep time doesn't cut into the time to next incident.
            drowsiness.NextIncidentTime += duration;

            _statusEffects.TryAddStatusEffectDuration(statusEffect.AppliedTo.Value,
