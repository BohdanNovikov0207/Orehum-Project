...
public sealed class LatheSystem : SharedLatheSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        var heatQuery =
            EntityQueryEnumerator<LatheHeatProducingComponent, LatheProducingComponent, TransformComponent>();
    ...
    [PublicAPI]
    public bool TryGetAvailableRecipes(EntityUid uid,
    ...
    public List<ProtoId<LatheRecipePrototype>> GetAvailableRecipes(EntityUid uid,
            ...
            if (currentRecipe.Result is { } resultProto)
            {
                ...
                else
                {
                    ...
                    if (TryComp<ScannableForPointsComponent>(result, out var scannable)) // Goobstation
                        scannable.Points =
                            0; // Goobstation, this thing is to prevent ntr duping points via an emagged lathe
                }
            }
    ...
    private void
        OnHeatStartPrinting(EntityUid uid, LatheHeatProducingComponent component, LatheStartPrintingEvent args) =>
        component.NextSecond = _timing.CurTime;

    private void
        OnMaterialAmountChanged(EntityUid uid, LatheComponent component, ref MaterialAmountChangedEvent args) =>
        UpdateUserInterfaceState(uid, component);
    ...
    private void OnDatabaseModified(EntityUid uid, LatheComponent component, ref TechnologyDatabaseModifiedEvent args)
    {
        ...
        foreach (var pack in component.DynamicPacks)
        {
            ...
            if (proto is null)
                continue;
            recipesCount +=
                proto.Recipes.Intersect(args.UnlockedRecipes)
                    .Count(); // which recipes we can use are the ones just unlocked?
        }
    ...
    private void OnResearchRegistrationChanged(EntityUid uid,
        LatheComponent component,
        ref ResearchRegistrationChangedEvent args) => UpdateUserInterfaceState(uid, component);

    protected override bool HasRecipe(EntityUid uid, LatheRecipePrototype recipe, LatheComponent component) =>
        GetAvailableRecipes(uid, component).Contains(recipe.ID);
