...
public sealed partial class ConnectionManager : IConnectionManager
{
    ...
    public void PostInit() => InitializeWhitelist();
    ...
    */
        ...
        NetConnectingArgs e)
    {
        ...
        var bans = await _db.GetServerBansAsync(addr, userId, hwId, modernHwid, false);
        if (bans.Count > 0)
        {
            var firstBan = bans[0];
    ...
    private bool HasTemporaryBypass(NetUserId user) =>
        _temporaryBypasses.TryGetValue(user, out var time) && time > _gameTiming.RealTime;
