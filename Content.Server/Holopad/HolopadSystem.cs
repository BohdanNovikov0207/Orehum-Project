...
public sealed class HolopadSystem : SharedHolopadSystem
{
    ...
    private void OnTypingChanged(HolopadUserTypingChangedEvent ev, EntitySessionEventArgs args)
    {
        ...
        foreach (var linkedHolopad in holopadUser.LinkedHolopads)
        {
            ...
            foreach (var receiverHolopad in receiverHolopads)
            {
                ...
                _appearanceSystem.SetData(receiverHolopad.Comp.Hologram.Value.Owner,
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var holopad, out var telephone, out var xform))
        {
            ...
            if (holopad.User != null &&
                ...
                !_xformSystem.InRange((holopad.User.Value, Transform(holopad.User.Value)),
                    (uid, xform),
    ...
    public void UpdateUIState(Entity<HolopadComponent> entity, TelephoneComponent? telephone = null)
    {
        ...
        while (query.MoveNext(out var receiverUid, out _, out var receiverTelephone))
        {
    ...
    private void SyncHolopadHologramAppearanceWithTarget(Entity<HolopadComponent> entity,
        ...
        if (TryComp<StationAiHeldComponent>(args.Actor, out var userAiHeld))
        {
            ...
            if (source != null)
            {
                ...
                if (TryComp<TelephoneComponent>(stationAiCore, out var stationAiTelephone) &&
                    ...
                    !_telephoneSystem.IsSourceInRangeOfReceiver((stationAiCore.Owner, stationAiTelephone),
    ...
    private void
        OnHolopadActivateProjector(Entity<HolopadComponent> entity, ref HolopadActivateProjectorMessage args) =>
    ...
    {
        if (TryComp<TelephoneComponent>(entity, out var telphone) &&
