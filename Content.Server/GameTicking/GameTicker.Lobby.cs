...
public sealed partial class GameTicker
{
    ...
    private TickerConnectionStatusEvent GetConnectionStatusMsg() => new(RoundStartTimeSpan);

    private TickerLobbyStatusEvent GetStatusMsg(ICommonSession session)
    {
        _playerGameStatuses.TryGetValue(session.UserId, out var status);
        return new TickerLobbyStatusEvent(RunLevel != GameRunLevel.PreRoundLobby,
            LobbyBackground,
    ...
    private TickerLobbyInfoEvent GetInfoMsg() => new(GetInfoText());

    private void UpdateLateJoinStatus() => RaiseNetworkEvent(new TickerLateJoinStatusEvent(DisallowLateJoin));

    private TickerInGameInfoEvent GetInGameInfoMsg() => new(GetInfoText(true));
