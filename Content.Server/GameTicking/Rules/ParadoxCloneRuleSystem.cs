...
public sealed class ParadoxCloneRuleSystem : GameRuleSystem<ParadoxCloneRuleComponent>
{
    ...
    private void OnAntagSelectEntity(Entity<ParadoxCloneRuleComponent> ent, ref AntagSelectEntityEvent args)
    {
        ...
        {
            if (Deleted(ent.Comp.OriginalBody.Value) ||
                !_mind.TryGetMind(ent.Comp.OriginalBody.Value, out var originalMindId, out _))
            {
        ...
        else
        {
            ...
            allAliveHumanoids.RemoveWhere(human =>
                _whitelist.IsBlacklistPass(ent.Comp.TargetBlacklist, human)); // Goobstation
        ...
        if (ent.Comp.OriginalBody == null || !_cloning.TryCloning(ent.Comp.OriginalBody.Value,
        ...
        gibComp.PreventGibbingObjectives =
            new HashSet<EntProtoId> { "ParadoxCloneKillObjective" }; // don't gib them if they killed the original.
    ...
    private void AfterAntagEntitySelected(Entity<ParadoxCloneRuleComponent> ent, ref AfterAntagEntitySelectedEvent args)
    {
        ...
        _mind.CopyObjectives(ent.Comp.OriginalMind.Value,
