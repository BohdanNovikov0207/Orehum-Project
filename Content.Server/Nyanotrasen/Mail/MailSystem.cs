...
public sealed class MailSystem : EntitySystem
{
    ...
    {
        var mailEntityStrings =
            component.IsLarge
                ? MailConstants.MailLarge
                : MailConstants.Mail; //Frontier: mail types stored per type (large mail)
        ...
        args.PushMarkup(Loc.GetString(mailEntityStrings.DescClose,
            ...
            ("job", component.RecipientJob))); // Frontier: mail constants struct
    ...
    public void PenalizeStationFailedDelivery(EntityUid uid, MailComponent component, string localizationString)
    {
        ...
        _chatSystem.TrySendInGameICMessage(uid,
            Loc.GetString(localizationString, ("credits", component.Penalty)),
    ...
    public bool TryMatchJobTitleToDepartment(string jobTitle, [NotNullWhen(true)] out string? jobDepartment)
    {
        foreach (var department in _prototypeManager.EnumeratePrototypes<DepartmentPrototype>())
        {
            ...
            {
                if (_prototypeManager.TryIndex(role, out var _jobPrototype)
    ...
    public List<EntityUid> GetUndeliveredParcels(EntityUid uid)
    {
        ...
        foreach (var entityInTile in _turf.GetEntitiesInTile(Transform(uid).Coordinates,
    ...
    /// </summary>
    /// <param name="uid">The mail teleporter to check.</param>
    public uint GetUndeliveredParcelCount(EntityUid uid) => (uint) GetUndeliveredParcels(uid).Count();

    /// <summary>
    /// Try to match a mail receiver to a mail teleporter.
    /// </summary>
    public bool TryGetMailTeleporterForReceiver(MailReceiverComponent receiver,
        [NotNullWhen(true)] out MailTeleporterComponent? teleporterComponent)
    {
        foreach (var mailTeleporter in EntityQuery<MailTeleporterComponent>())
        {
            if (_stationSystem.GetOwningStation(receiver.Owner) ==
                _stationSystem.GetOwningStation(mailTeleporter.Owner))
            {
    ...
    /// </summary>
    public bool TryGetMailRecipientForReceiver(MailReceiverComponent receiver,
    ...
    public List<MailRecipient> GetMailRecipientCandidates(EntityUid uid)
    {
        ...
        while (query.MoveNext(out var receiver, out var receiverComp))
        {
            ...
            if (TryGetMailRecipientForReceiver(receiverComp, out var recipient))
...
public struct MailRecipient
{
    ...
    public MailRecipient(string name,
        string job,
        string jobIcon,
