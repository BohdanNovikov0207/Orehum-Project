...
internal sealed class ReagentGrinderSystem : EntitySystem
{
    ...
    private void OnToggleAutoModeMessage(Entity<ReagentGrinderComponent> entity,
        ref ReagentGrinderToggleAutoModeMessage message)
    {
        entity.Comp.AutoMode =
            (GrinderAutoMode) (((byte) entity.Comp.AutoMode + 1) % Enum.GetValues(typeof(GrinderAutoMode)).Length);
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var active, out var reagentGrinder))
        {
            ...
            var inputContainer =
                _containerSystem.EnsureContainer<Container>(uid, SharedReagentGrinder.InputContainerId);
            var outputContainer = _itemSlotsSystem.GetItemOrNull(uid, SharedReagentGrinder.BeakerSlotId);
            if (outputContainer is null || !_solutionContainersSystem.TryGetFitsInDispenser(outputContainer.Value,
            ...
            foreach (var item in inputContainer.ContainedEntities.ToList())
            {
                ...
                if (TryComp<StackComponent>(item, out var stack))
                {
                    ...
                    var fitsCount = (int) (stack.Count *
                                           FixedPoint2.Min(containerSolution.AvailableVolume / totalVolume + 0.01, 1));
                    if (fitsCount <= 0)
                        continue;
            ...
            _userInterfaceSystem.ServerSendUiMessage(uid,
        ...
        ContainerModifiedMessage args)
    {
        ...
        {
            var program = reagentGrinder.AutoMode == GrinderAutoMode.Grind
                ? GrinderProgram.Grind
                : GrinderProgram.Juice;
    ...
    private void OnInteractUsing(Entity<ReagentGrinderComponent> entity, ref InteractUsingEvent args)
    {
        ...
        var inputContainer =
            _containerSystem.EnsureContainer<Container>(entity.Owner, SharedReagentGrinder.InputContainerId);
        ...
        if (!HasComp<ExtractableComponent>(heldEnt))
        {
            if (!HasComp<FitsInDispenserComponent>(heldEnt))
            {
                ...
                _popupSystem.PopupEntity(Loc.GetString("reagent-grinder-component-cannot-put-entity-message"),
                    entity.Owner,
                    args.User);
            }
    ...
    private void OnEjectChamberAllMessage(Entity<ReagentGrinderComponent> entity,
    ...
    private void OnEjectChamberContentMessage(Entity<ReagentGrinderComponent> entity,
    ...
    private void DoWork(EntityUid uid, ReagentGrinderComponent reagentGrinder, GrinderProgram program)
    {
        ...
        reagentGrinder.AudioStream = _audioSystem.PlayPvs(sound,
                uid,
    ...
    private bool CanJuice(EntityUid uid) => CompOrNull<ExtractableComponent>(uid)?.JuiceSolution is not null;
}
