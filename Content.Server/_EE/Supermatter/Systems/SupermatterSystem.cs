...
using Content.Shared._EE.Supermatter;
...
public sealed partial class SupermatterSystem : EntitySystem
{
    ...
    public void OnSupermatterUpdated(EntityUid uid, SupermatterComponent sm, AtmosDeviceUpdateEvent args)
    {
        ...
        if (sm.Power > _config.GetCVar(CCVars.SupermatterPowerPenaltyThreshold) || sm.Damage > sm.DamagePenaltyPoint)
        {
            SupermatterZap(uid, sm, args.dt);
    ...
    private SupermatterStatusType GetStatus(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (mix.Temperature > Atmospherics.T0C + _config.GetCVar(CCVars.SupermatterHeatPenaltyThreshold) * 0.8)
