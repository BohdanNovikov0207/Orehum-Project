...
public sealed partial class TabletopChessSetup : TabletopSetup
{
    ...
    private void SpawnPieces(TabletopSession session,
        ...
        float separation = 1f)
    {
        ...
        // Extra queens
        var tempQualifier = entityManager.SpawnEntity("BlackQueen",
            new MapCoordinates(x + 9 * separation + 9f / 32, y - 3 * separation, mapId));
        session.Entities.Add(tempQualifier);
        var tempQualifier1 = entityManager.SpawnEntity("WhiteQueen",
    ...
    private void SpawnPiecesRow(TabletopSession session,
        IEntityManager entityManager,
        string color,
        MapCoordinates left,
        float separation = 1f)
    {
        ...
        for (var i = 0; i < 8; i++)
        {
            ...
            {
                case 'r':
                    var tempQualifier = entityManager.SpawnEntity(color + "Rook",
                        new MapCoordinates(x + i * separation, y, mapId));
                    session.Entities.Add(tempQualifier);
                    break;
                case 'n':
                    var tempQualifier1 = entityManager.SpawnEntity(color + "Knight",
                        new MapCoordinates(x + i * separation, y, mapId));
                    session.Entities.Add(tempQualifier1);
                    break;
                case 'b':
                    var tempQualifier2 = entityManager.SpawnEntity(color + "Bishop",
                        new MapCoordinates(x + i * separation, y, mapId));
                    session.Entities.Add(tempQualifier2);
                    break;
                case 'q':
                    var tempQualifier3 = entityManager.SpawnEntity(color + "Queen",
                        new MapCoordinates(x + i * separation, y, mapId));
                    session.Entities.Add(tempQualifier3);
                    break;
                case 'k':
                    var tempQualifier4 = entityManager.SpawnEntity(color + "King",
                        new MapCoordinates(x + i * separation, y, mapId));
    ...
    // TODO: refactor to load FEN instead
    private void SpawnPawns(TabletopSession session,
        IEntityManager entityManager,
        string color,
        MapCoordinates left,
        float separation = 1f)
