...
public sealed partial class NitriumProductionReaction : IGasReactionEffect
{
        ...
        float heatScale)
    {
        ...
        var efficiency = Math.Min(mixture.Temperature / 2984f,
            Math.Min(initBZ * 20f, Math.Min(initTritium, initNitrogen)));
        ...
        if (efficiency <= 0 || initTritium - tritiumRemoved < 0 || initNitrogen - nitrogenRemoved < 0 ||
            initBZ - bzRemoved < 0)
            return ReactionResult.NoReaction;
