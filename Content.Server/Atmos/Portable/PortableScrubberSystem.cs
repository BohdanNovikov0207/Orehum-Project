...
public sealed class PortableScrubberSystem : EntitySystem
{
    ...
    private bool IsFull(PortableScrubberComponent component) => component.Air.Pressure >= component.MaxPressure;
    ...
    private void OnAnchorChanged(EntityUid uid, PortableScrubberComponent component, ref AnchorStateChangedEvent args)
    {
        ...
        portableNode.ConnectionsEnabled = args.Anchored &&
                                          _gasPortableSystem.FindGasPortIn(Transform(uid).GridUid,
    ...
    private void OnDestroyed(EntityUid uid, PortableScrubberComponent component, DestructionEventArgs args)
    {
        ...
        _adminLogger.Add(LogType.CanisterPurged,
            LogImpact.Medium,
            $"Portable scrubber {ToPrettyString(uid):canister} purged its contents of {component.Air} into the environment.");
        component.Air.Clear();
    }

    private bool Scrub(float timeDelta, PortableScrubberComponent scrubber, GasMixture? tile) => _scrubberSystem.Scrub(
        timeDelta,
