...
public sealed partial class AtmosphereSystem
{
    ...
    /// </summary>
    public float PumpSpeedup() => Speedup;

    /// <summary>
    /// Calculates the thermal energy for a gas mixture.
    /// </summary>
    public float GetThermalEnergy(GasMixture mixture) => mixture.Temperature * GetHeatCapacity(mixture);
    ...
    public bool ReleaseGasTo(GasMixture mixture, GasMixture? output, float targetPressure)
    {
        ...
        var pressureDelta = MathF.Min(targetPressure - outputStartingPressure,
            (inputStartingPressure - outputStartingPressure) / 2f);
        var transferMoles = pressureDelta * (output?.Volume ?? Atmospherics.CellVolume) /
                            (mixture.Temperature * Atmospherics.R);
    ...
    /// <summary>
    /// Determines the number of moles that need to be removed from a <see cref="GasMixture" /> to reach a target pressure
    /// threshold.
    /// </summary>
            ...
            for (var i = 0; i < prototype.MinimumRequirements.Length; i++)
            {
                if (i >= Atmospherics.TotalNumberOfGases)
                    throw new IndexOutOfRangeException(
                        "Reaction Gas Minimum Requirements Array Prototype exceeds total number of gases!");
