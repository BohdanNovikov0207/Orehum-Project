...
public sealed partial class AtmosphereSystem
{
    ...
    public SoundSpecifier? SpaceWindSound { get; } =
        new SoundCollectionSpecifier(DefaultSpaceWindSounds, AudioParams.Default.WithVariation(0.125f));
                ...
                && TryComp<MovedByPressureComponent>(uid, out var component))
            {
                foreach (var (id, fixture) in fixtures.Fixtures)
                {
                    if (component.TableLayerRemoved.Contains(id))
                        _physics.AddCollisionMask(uid, id, fixture, (int) CollisionGroup.TableLayer, fixtures);
                }
            }
        }
    ...
    private void AddMobMovedByPressure(EntityUid uid, MovedByPressureComponent component, PhysicsComponent body)
    {
        ...
        foreach (var (id, fixture) in fixtures.Fixtures)
        {
            ...
            if ((fixture.CollisionMask & (int) CollisionGroup.TableLayer) != 0)
            {
                ...
                _physics.RemoveCollisionMask(uid, id, fixture, (int) CollisionGroup.TableLayer, fixtures);
            }
        }
        ...
        if (tile.PressureDifference > 15 && !tile.Hotspot.Valid)
        {
            if (_spaceWindSoundCooldown == 0 && SpaceWindSound != null)
            {
                ...
                _audio.PlayPvs(SpaceWindSound,
                    coordinates,
                    SpaceWindSound.Params.WithVolume(MathHelper.Clamp(tile.PressureDifference / 10, 10, 100)));
            }
        }
    ...
    /// Objects can only be thrown if the force of air flow is greater than the SQUARE of their mass or
    /// {SpaceWindMinimumCalculatedMass}, whichever is heavier
    /// This means that the heavier an object is, the exponentially more force is required to move it
    /// The force of a throw is equal to the force of air pressure, divided by an object's mass. So not only are heavier
    /// objects
    /// less likely to be thrown, they are also harder to throw,
    /// while lighter objects are yeeted easily, and from great distance.
    /// For a human sized entity with a standard weight of 80kg and a spacing between a hard vacuum and a room pressurized at
    /// 101kpa,
        ...
        PhysicsComponent? physics = null)
    {
            ...
            && !float.IsPositiveInfinity(component.MoveResist))
        {
            ...
            if (moveForce > physics.Mass)
            {
                ...
                {
                    var pos = throwTarget.ToMap(EntityManager, _transformSystem).Position - xform.WorldPosition +
                              dirVec;
                    _throwing.TryThrow(uid, pos.Normalized() * MathF.Min(moveForce, SpaceWindMaxVelocity), moveForce);
                }
                else
                    _throwing.TryThrow(uid,
                        dirVec.Normalized() * MathF.Min(moveForce, SpaceWindMaxVelocity),
                        moveForce);
