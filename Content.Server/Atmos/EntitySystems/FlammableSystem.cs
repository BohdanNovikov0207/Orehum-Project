...
public sealed class FlammableSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<FlammableComponent, GetFireStateEvent>(
            OnGetFireStateEvent); // goob edit - retrieve fire state
    ...
    private void OnExtinguishActivateInWorld(EntityUid uid,
    ...
    private void OnCollide(EntityUid uid, FlammableComponent flammable, ref StartCollideEvent args)
    {
        ...
        var avgPen = (flammable.FireProtectionPenetration + otherFlammable.FireProtectionPenetration) /
                     total; // Goobstation
        ...
        SetFireStacks(uid, avg * mass2, flammable, true, avgPen * mass2); // Goob edit
        SetFireStacks(otherUid, avg * mass1, otherFlammable, true, avgPen * mass1); // Goob edit
    }

    private void OnIsHot(EntityUid uid, FlammableComponent flammable, IsHotEvent args) => args.IsHot = flammable.OnFire;
    ...
    public void AdjustFireStacks(EntityUid uid,
        ...
        FlammableComponent? flammable = null,
        bool ignite = false,
        float fireProtectionPenetration = 0f) // Goob edit
    {
        if (!Resolve(uid, ref flammable))
            return;

        SetFireStacks(uid,
            flammable.FireStacks + relativeFireStacks,
            flammable,
            ignite,
            fireProtectionPenetration); // Goob edit
    }
        ...
        float fireProtectionPenetration = 0f) // Goob edit
    {
        ...
        if (stacks > 0)
            fireProtectionPenetration = MathHelper.Lerp(flammable.FireProtectionPenetration,
                fireProtectionPenetration,
                1f - flammable.FireStacks / stacks);
    ...
    private void OnDamageChanged(EntityUid uid, IgniteOnHeatDamageComponent component, DamageChangedEvent args)
    {
        ...
        if (args.DamageDelta.DamageDict.TryGetValue("Heat", out var value))
        {
        ...
        while (query.MoveNext(out var uid, out _))
        {
            ...
            if (flammable.FireStacks > 0)
            {
                ...
                if (TryComp(uid, out TemperatureComponent? temp))
                    _temperatureSystem.ChangeHeat(uid,
                        _addHeatFirestack * flammable.FireStacks,
                        false,
                        temp); // goob edit: 12500 -> 1500
                ...
                if (multiplier > 0f &&
                    ...
                    _damageableSystem.TryChangeDamage(uid,
                        flammable.Damage * flammable.FireStacks * multiplier,
                        interruptsDoAfters: false,
                        targetPart: TargetBodyPart.All,
                        partMultiplier: 2f); // Lavaland: Nerf fire delimbing
