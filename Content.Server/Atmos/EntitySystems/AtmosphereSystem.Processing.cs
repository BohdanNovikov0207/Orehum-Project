...
public sealed partial class AtmosphereSystem
{
    ...
    private readonly
        List<Entity<GridAtmosphereComponent, GasTileOverlayComponent, MapGridComponent, TransformComponent>>
    ...
    private TileAtmosphere GetOrNewTile(EntityUid owner,
        ...
        Entity<GridAtmosphereComponent, GasTileOverlayComponent, MapGridComponent, TransformComponent> ent)
    {
        ...
        if (!atmosphere.ProcessingPaused)
        {
            ...
            {
                var tile = GetOrNewTile(uid, atmosphere, indices, false);
    ...
    {
        var num = (int) AtmosphereProcessingState.NumStates;
    ...
    private void UpdateProcessing(float frameTime)
    {
        ...
        if (!_simulationPaused)
        {
            ...
            var query =
                EntityQueryEnumerator<GridAtmosphereComponent, GasTileOverlayComponent, MapGridComponent,
                    TransformComponent>();
        ...
        for (; _currentRunAtmosphereIndex < _currentRunAtmosphere.Count; _currentRunAtmosphereIndex++)
        {
            ...
            {
                Log.Error(
                    $"Attempted to process atmos without a map? Entity: {ToPrettyString(owner)}. Map: {ToPrettyString(xform?.MapUid)}. MapId: {xform?.MapID}");
                continue;
            }
            ...
            var map = new Entity<MapAtmosphereComponent?>(xform.MapUid.Value,
