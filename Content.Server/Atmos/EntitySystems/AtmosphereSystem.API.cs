...
public partial class AtmosphereSystem
{
    ...
    public bool IsTileMixtureProbablySafe(Entity<GridAtmosphereComponent?>? grid,
        Entity<MapAtmosphereComponent?> map,
        Vector2i tile) => IsMixtureProbablySafe(GetTileMixture(grid, map, tile));

    public float GetTileHeatCapacity(Entity<GridAtmosphereComponent?>? grid,
        Entity<MapAtmosphereComponent?> map,
        Vector2i tile) => GetHeatCapacity(GetTileMixture(grid, map, tile) ?? GasMixture.SpaceGas);

    public TileMixtureEnumerator GetAdjacentTileMixtures(Entity<GridAtmosphereComponent?> grid,
        Vector2i tile,
    ...
    public bool AddPipeNet(Entity<GridAtmosphereComponent?> grid, PipeNet pipeNet) =>
        _atmosQuery.Resolve(grid, ref grid.Comp, false) && grid.Comp.PipeNets.Add(pipeNet);
