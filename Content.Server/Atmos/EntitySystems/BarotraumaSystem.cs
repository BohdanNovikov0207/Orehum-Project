...
public sealed class BarotraumaSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<PressureProtectionComponent, ComponentInit>(
            OnPressureProtectionChanged); // Goobstation - Update component state on toggle
        SubscribeLocalEvent<PressureProtectionComponent, ComponentRemove>(
            OnPressureProtectionChanged); // Goobstation - Update component state on toggle
    ...
    private void OnPressureProtectionChanged(EntityUid uid,
        ...
        EntityEventArgs args)
    {
        ...
        if (_inventorySystem.TryGetContainingEntity(uid, out var entity) &&
    ...
    private void OnPressureProtectionEquipped(EntityUid uid,
        PressureProtectionComponent pressureProtection,
    ...
    private void OnPressureProtectionUnequipped(EntityUid uid,
        PressureProtectionComponent pressureProtection,
    ...
    /// The ProtectionSlots field of the Barotrauma component specifies which parts must be protected for the protection to
    /// have any effet.
    /// </summary>
    ...
    /// <summary>
    /// Returns adjusted pressure after having applied resistances from equipment and innate (if any), to check against a low
    /// pressure hazard threshold
    /// </summary>
    ...
    /// <summary>
    /// Returns adjusted pressure after having applied resistances from equipment and innate (if any), to check against a high
    /// pressure hazard threshold
    /// </summary>
        ...
        while (enumerator.MoveNext(out var uid, out var barotrauma, out var damageable))
        {
            ...
            if (pressure <= Atmospherics.HazardLowPressure)
            {
                ...
                if (!barotrauma.TakingDamage)
                {
                    ...
                    _adminLogger.Add(LogType.Barotrauma,
                     ...
                     !_spellblade.IsHoldingItemWithComponent<FireSpellbladeEnchantmentComponent>(uid)) // Goob edit
            {
                ...
                var damageScale =
                    MathF.Min((pressure / Atmospherics.HazardHighPressure - 1) * Atmospherics.PressureDamageCoefficient,
