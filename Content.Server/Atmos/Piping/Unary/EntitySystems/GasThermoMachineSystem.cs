...
public sealed class GasThermoMachineSystem : SharedGasThermoMachineSystem
{
    ...
    private void OnThermoMachineUpdated(EntityUid uid,
        ...
        ref AtmosDeviceUpdateEvent args)
    {
        ...
        var targetTemp = thermoMachine.TargetTemperature;
        var highTemp = targetTemp + sign * thermoMachine.TemperatureTolerance;
        var temp = heatExchangeGasMixture.Temperature;
        ...
        var dQ = thermoMachine.HeatCapacity * thermoMachine.Cp * args.dt;

        // Clamps the heat transferred to not overshoot
        var Cin = _atmosphereSystem.GetHeatCapacity(heatExchangeGasMixture, true);
        var dT = targetTemp - temp;
        var dQLim = dT * Cin;
        var scale = 1f;
        ...
        {
            var dQLeak = dQActual * thermoMachine.EnergyLeakPercentage;
            var dQPipe = dQActual - dQLeak;
            _atmosphereSystem.AddHeat(heatExchangeGasMixture, dQPipe);
            thermoMachine.LastEnergyDelta = dQPipe;

            if (dQLeak != 0f && _atmosphereSystem.GetContainingMixture(uid, args.Grid, args.Map, excite: true) is
                    { } containingMixture)
    ...
    /// <summary>
    /// Returns the gas mixture with which the thermomachine will exchange heat (the local atmosphere if atmospheric or the
    /// inlet pipe
    /// air if not). Returns null if no gas mixture is found.
    /// </summary>
    private void GetHeatExchangeGasMixture(EntityUid uid,
        GasThermoMachineComponent thermoMachine,
