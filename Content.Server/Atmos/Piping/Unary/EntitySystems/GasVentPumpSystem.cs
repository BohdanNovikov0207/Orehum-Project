...
public sealed class GasVentPumpSystem : EntitySystem
{
    ...
    private void OnGasVentPumpUpdated(EntityUid uid, GasVentPumpComponent vent, ref AtmosDeviceUpdateEvent args)
    {
        ...
        var lockout = environment.Pressure < vent.UnderPressureLockoutThreshold &&
                      !vent.IsPressureLockoutManuallyDisabled;
    ...
    private void OnGasVentPumpLeaveAtmosphere(EntityUid uid,
        GasVentPumpComponent component,
        ref AtmosDeviceDisabledEvent args) => UpdateState(uid, component);

    private void OnGasVentPumpEnterAtmosphere(EntityUid uid,
        GasVentPumpComponent component,
        ref AtmosDeviceEnabledEvent args) => UpdateState(uid, component);
    ...
    private void OnPacketRecv(EntityUid uid, GasVentPumpComponent component, DeviceNetworkPacketEvent args)
    {
        ...
        switch (cmd)
        {
            ...
            case DeviceNetworkConstants.CmdSetState:
                ...
                {
                    var enabled = setData.Enabled ? "enabled" : "disabled";
                ...
                {
                    var enabled = setData.PressureLockoutOverride ? "enabled" : "disabled";
                    _adminLogger.Add(LogType.AtmosDeviceSetting,
                        LogImpact.Medium,
    ...
    private void OnExamine(EntityUid uid, GasVentPumpComponent component, ExaminedEvent args)
    {
        ...
        {
            if ((pumpComponent.PumpDirection == VentPumpDirection.Releasing) & pumpComponent.UnderPressureLockout &
                !pumpComponent.PressureLockoutOverride & !pumpComponent.IsPressureLockoutManuallyDisabled)
