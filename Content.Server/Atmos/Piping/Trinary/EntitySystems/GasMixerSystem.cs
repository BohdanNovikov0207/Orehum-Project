...
public sealed class GasMixerSystem : EntitySystem
{
    ...
    private void OnInit(EntityUid uid, GasMixerComponent mixer, ComponentInit args) => UpdateAppearance(uid, mixer);
    ...
    private void OnMixerUpdated(EntityUid uid, GasMixerComponent mixer, ref AtmosDeviceUpdateEvent args)
    {
        ...
        var transferMolesOne = inletOne.Air.Temperature > 0
            ? mixer.InletOneConcentration * generalTransfer / inletOne.Air.Temperature
            : 0f;
        var transferMolesTwo = inletTwo.Air.Temperature > 0
            ? mixer.InletTwoConcentration * generalTransfer / inletTwo.Air.Temperature
            : 0f;
        ...
        else
        {
            ...
            {
                var ratio = MathF.Min(inletOne.Air.TotalMoles / transferMolesOne,
                    inletTwo.Air.TotalMoles / transferMolesTwo);
    ...
    private void DirtyUI(EntityUid uid, GasMixerComponent? mixer)
    {
        ...
        _userInterfaceSystem.SetUiState(uid,
            GasMixerUiKey.Key,
            new GasMixerBoundUserInterfaceState(Comp<MetaDataComponent>(uid).EntityName,
    ...
    private void OnOutputPressureChangeMessage(EntityUid uid,
    ...
    private void OnChangeNodePercentageMessage(EntityUid uid,
        GasMixerComponent mixer,
        GasMixerChangeNodePercentageMessage args)
    {
        ...
        _adminLogger.Add(LogType.AtmosRatioChanged,
            LogImpact.Medium,
    ...
    private void OnMixerAnalyzed(EntityUid uid, GasMixerComponent component, GasAnalyzerScanEvent args)
    {
        ...
        if (_nodeContainer.TryGetNode(uid, component.InletOneName, out PipeNode? inletOne) && inletOne.Air.Volume != 0f)
        {
            ...
            args.GasMixtures.Add(($"{inletOne.CurrentPipeDirection} {Loc.GetString("gas-analyzer-window-text-inlet")}",
                inletOneAirLocal));
        }
        ...
        if (_nodeContainer.TryGetNode(uid, component.InletTwoName, out PipeNode? inletTwo) && inletTwo.Air.Volume != 0f)
        {
            ...
            args.GasMixtures.Add(($"{inletTwo.CurrentPipeDirection} {Loc.GetString("gas-analyzer-window-text-inlet")}",
                inletTwoAirLocal));
        }
