...
public sealed class CloningConsoleSystem : EntitySystem
{
    ...
    public void TryClone(EntityUid uid,
    ...
    {
        if (!Resolve(uid, ref consoleComponent) || !Resolve(cloningPodUid, ref cloningPod) ||
        ...
        if (_cloningPodSystem.TryCloning(cloningPodUid,
                body.Value,
                (mindId, mind),
                cloningPod,
        ...
        _damage.TryChangeDamage(body.Value,
            cloningPod.CloneDamage,
            true); // Goobstation - Damage the og body if the clone successful
    }

    public void RecheckConnections(EntityUid console,
        ...
        CloningConsoleComponent? consoleComp = null)
    {
        ...
        if (scanner != null)
        {
            Transform(scanner.Value)
                .Coordinates.TryDistance(EntityManager, Transform(console).Coordinates, out var scannerDistance);
        ...
        if (cloningPod != null)
        {
            Transform(cloningPod.Value)
                .Coordinates.TryDistance(EntityManager, Transform(console).Coordinates, out var podDistance);
    ...
    private CloningConsoleBoundUserInterfaceState GetUserInterfaceState(CloningConsoleComponent consoleComponent)
    {
        ...
        if (consoleComponent.CloningPod != null && TryComp<CloningPodComponent>(consoleComponent.CloningPod,
                                                    out var clonePod)
