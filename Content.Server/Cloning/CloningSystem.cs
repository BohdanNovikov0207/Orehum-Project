...
public sealed partial class CloningSystem : EntitySystem
{
    ...
    /// </summary>
    public bool TryCloning(EntityUid original,
        ...
        [NotNullWhen(true)] out EntityUid? clone)
    {
        ...
        if (HasComp<HolographicCloneComponent>(original) &&
            !settings.ForceCloning) // Goobstation - This has to be separate because I don't want to touch the other check.
            return false;

        if (HasComp<UncloneableComponent>(original) &&
            !settings.ForceCloning) // Goob: enable forcecloning bypass for antagctrl admemes on vox/ipc.
            return
                false; // Goobstation: Don't clone IPCs and voxes. It could be argued it should be in the CloningPodSystem instead
        ...
        if (settings.CopyEquipment != null)
            CopyEquipment(original,
                clone.Value,
                settings.CopyEquipment.Value,
                settings.Whitelist,
                settings.Blacklist,
                settings.MakeEquipmentUnremoveable,
                settings.CopyStorage,
                settings.InternalContentsUnremoveable); // Goob edit
    ...
    public void CloneComponents(EntityUid original, EntityUid clone, CloningSettingsPrototype settings)
    {
        ...
        {
            var statusComps = statusComp.ActiveEffects.Values.Select(s => s.RelevantComponent)
                .Where(s => s != null)
                .ToList();
        ...
        foreach (var componentName in componentsToCopy)
        {
            ...
            if (EntityManager.TryGetComponent(original,
                    componentRegistration.Type,
                    out var sourceComp)) // Does the original have this component?
    ...
    /// </summary>
    public void CopyEquipment(Entity<InventoryComponent?> original,
        ...
        bool internalContentsUnremoveable = false) // Goob edit
    {
        ...
        while (slotEnumerator.NextItem(out var item, out var slot))
        {
            ...
            if (!_inventory.TryEquip(clone, cloneItem.Value, slot.Name, true, inventory: clone.Comp))
            {
    ...
    /// </remarks>
    public EntityUid? CopyItem(EntityUid original,
        EntityCoordinates coords,
        EntityWhitelist? whitelist = null,
        EntityWhitelist? blacklist = null,
        bool copyStorage = true) // Goob edit
    {
        ...
        if (TryComp<StorageComponent>(original, out var originalStorage) &&
            TryComp<StorageComponent>(spawned, out var spawnedStorage)) // Goob edit
        {
            ...
            foreach (var (itemUid, itemLocation) in originalStorage.StoredItems)
            {
    ...
    /// </summary>
    public void CopyStorage(Entity<StorageComponent?> original,
        ...
        EntityWhitelist? blacklist = null)
    {
        ...
        // recursively replace them
        foreach (var (itemUid, itemLocation) in original.Comp.StoredItems)
        {
            var copy = CopyItem(itemUid, coords, whitelist, blacklist);
            if (copy != null)
    ...
    public void CopyImplants(Entity<ImplantedComponent?> original,
        EntityUid target,
        ...
        EntityWhitelist? blacklist = null)
    {
        ...
        foreach (var originalImplant in original.Comp.ImplantContainer.ContainedEntities)
        {
            ...
            if (copyStorage)
                CopyStorage(originalImplant,
                    targetImplant.Value,
                    whitelist,
                    blacklist); // only needed for storage implants
        }
    }
}
