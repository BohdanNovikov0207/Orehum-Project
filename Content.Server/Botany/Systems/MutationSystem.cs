...
public sealed class MutationSystem : EntitySystem
{
    ...
    public override void Initialize() => _randomMutations = _prototypeManager.Index(RandomPlantMutations);
    ...
    private void CrossChemicals(ref Dictionary<string, SeedChemQuantity> val,
    ...
    private void CrossFloat(ref float val, float other) => val = Random(0.5f) ? val : other;

    private void CrossInt(ref int val, int other) => val = Random(0.5f) ? val : other;

    private void CrossBool(ref bool val, bool other) => val = Random(0.5f) ? val : other;

    private bool Random(float p) => _robustRandom.Prob(p);
}
