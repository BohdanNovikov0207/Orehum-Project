...
public sealed partial class ZombieSystem : SharedZombieSystem
{
    ...
    private void OnGetCharacterUnrevivableIC(EntityUid uid,
    ...
    private bool IsUserBlocking(BlockingUserComponent? component) // Goobstation
        =>
            TryComp<BlockingComponent>(component?.BlockingItem, out var blockComp) && blockComp.IsBlocking;
    ...
    private void OnMeleeHit(EntityUid uid, ZombieComponent component, MeleeHitEvent args)
    {
        ...
        foreach (var entity in args.HitEntities)
        {
            ...
            if (TryComp<BlockingUserComponent>(entity, out var blockingUser) &&
                IsUserBlocking(blockingUser)) // Goobstation edit - prevents infection if user is actively blocking
                return;

            if (HasComp<ZombieComponent>(entity) ||
                HasComp<InitialInfectedComponent>(entity)) // Goobstation edit - prevent zombies from damaging IIs
    ...
    private void OnMindAdded(Entity<ZombieComponent> ent, ref MindAddedMessage args)
    {
        if (!_role.MindHasRole<ZombieRoleComponent>(args.Mind))
            _role.MindAddRole(args.Mind, "MindRoleZombie", args.Mind.Comp);
    }
    ...
    private void OnShutdown(Entity<ZombieComponent> ent, ref ComponentShutdown args)
    {
        ...
        _language.UpdateEntityLanguages(ent
            .Owner); // This uses ent.Owner because UpdateEntityLanguages checks for <LanguageSpeakerComponent>.
    }
