...
public sealed class ServerPreferencesManager : IServerPreferencesManager, IPostInjectInit
{
    ...
    public async Task SetProfile(NetUserId userId, int slot, ICharacterProfile profile)
    {
        ...
        var sponsorPrototypes =
            _sponsors != null && _sponsors.TryGetServerPrototypes(session.UserId, out var prototypes)
                ? prototypes.ToArray()
                : [];
    ...
    public void OnClientDisconnected(ICommonSession session) => _cachedPlayerPrefs.Remove(session.UserId);

    public bool HavePreferencesLoaded(ICommonSession session) => _cachedPlayerPrefs.ContainsKey(session.UserId);
    ...
    private async void HandleDeleteCharacterMessage(MsgDeleteCharacter message)
    {
        ...
        prefsData.Prefs = new PlayerPreferences(arr,
        ...
        return new PlayerPreferences(prefs.Characters.Select(p =>
            {
                return new KeyValuePair<int, ICharacterProfile>(p.Key,
                    p.Value.Validated(session, collection, sponsorPrototypes));
            }),
    ...
    internal static bool ShouldStorePrefs(LoginType loginType) => loginType.HasStaticUserId();
