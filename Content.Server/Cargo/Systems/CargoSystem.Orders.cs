...
public sealed partial class CargoSystem
{
    ...
    private void OnInteractUsingSlip(Entity<CargoOrderConsoleComponent> ent,
        ...
        CargoSlipComponent slip)
    {
        ...
        var data = new CargoOrderData(orderId,
            product.Product,
            product.Name,
            product.Cost + (slip.SecuredDelivery ? ent.Comp.SecureOrderCost : 0),
            slip.OrderQuantity,
            slip.Requester,
            slip.DeliveryDestination,
            slip.Note,
            slip.Account,
            product.Cooldown,
            slip.SecuredDelivery);
    ...
    private void OnInteractUsing(EntityUid uid, CargoOrderConsoleComponent component, ref InteractUsingEvent args)
    {
        ...
        else if (TryComp<CargoSlipComponent>(args.Used, out var slip) &&
    ...
    /// <summary>
    /// Gets orders relevant to this account, i.e. orders on the account directly or orders on behalf of the account in the
    /// primary account.
    /// </summary>
    private List<CargoOrderData> RelevantOrders(Entity<StationCargoOrderDatabaseComponent> station,
        Entity<CargoOrderConsoleComponent> console)
    {
        ...
        var otherOrders = station.Comp.Orders[bank.PrimaryAccount]
            .Where(order => order.Account == console.Comp.Account);

        return ourOrders.Concat(otherOrders).ToList();
    }

    private void ConsolePopup(EntityUid actor, string text) => _popup.PopupCursor(text, actor);
    ...
    private static CargoOrderData GetOrderData(CargoConsoleAddOrderMessage args,
        ...
        new(id,
            ...
            cargoProduct.Name,
            cargoProduct.Cost + extraPrice,
            args.Amount,
            requester,
            args.DeliveryDestination,
            args.Note,
            account,
            cargoProduct.Cooldown,
            args.SecuredDelivery);
    // CorvaxGoob-CargoFeatures-End

    public int GetOutstandingOrderCount(Entity<StationCargoOrderDatabaseComponent> station,
        ProtoId<CargoAccountPrototype> account)
    {
        var amount = 0;
    ...
    )
    {
        ...
        // GoobStation - cooldown on Cargo Orders (specifically gamba)
        var order = new CargoOrderData(id,
            spawnId,
            name,
            cost,
            qty,
            ...
            account,
            0,
            securedDelivery); // CorvaxGoob-CargoFeatures
    ...
    public bool TryAddOrder(EntityUid dbUid,
        ProtoId<CargoAccountPrototype> account,
        CargoOrderData data,
        StationCargoOrderDatabaseComponent component)
    {
    ...
    public static int GenerateOrderId(StationCargoOrderDatabaseComponent orderDB) =>
        ...
        ++orderDB.NumOrdersCreated;

    public void RemoveOrder(EntityUid dbUid,
        ProtoId<CargoAccountPrototype> account,
        int index,
    ...
    /// </summary>
    [PublicAPI]
    private bool FulfillNextOrder(StationCargoOrderDatabaseComponent orderDB,
        ProtoId<CargoAccountPrototype> account,
    ...
    private bool FulfillOrder(CargoOrderData order,
        ProtoId<CargoAccountPrototype> account,
        EntityCoordinates spawn,
        string? paperProto,
        out EntityUid? nextCrate,
        out List<string>? excessItemsOut,
        EntityUid? previousCrate = null,
        List<string>? excessItemsIn = null)
    {
        ...
        if (previousCrate is not null) // Если указан ящик который будет заполняться
            ...
            (order.SecuredDelivery && accountProto
                                       .SecureCratePrototype is not null // Если поставлена галочка на защиту заказа
                                   && (productProto.TryGetComponent<ItemComponent>(out var itemComponent) ||
                                       productProto
                                           .HasComponent<
                                               EntityStorageComponent>())) // и может ли этот заказ быть помещен в ящик?
        {
            ...
            && TryComp<EntityStorageComponent>(item, out var crateEntityStorage))
        {
            ...
            {
                if (crateEntityStorage.Contents.Count >= crateEntityStorage.Capacity &&
                    spawns.Count <= crateEntityStorage.Capacity)
                    doExcessFill = true;
        ...
        if (previousCrate is null) // // CorvaxGoob-CargoFeatures : Проверяем является ли это новым ящиком или старым
        {
            ...
            if (TryComp<PaperComponent>(printed, out var paper))
            {
                ...
                var val = Loc.GetString("cargo-console-paper-print-name",
                    ("orderNumber", order.OrderId),
                    ("detailName", itemName),
                    ("detailQuantity", order.OrderQuantity));
                // CorvaxGoob-CargoFeatures-End
                ...
                _paperSystem.SetContent((printed, paper),
                    Loc.GetString(
                        ...
                        ("destination",
                            string.IsNullOrWhiteSpace(order.DeliveryDestination)
                                ? Loc.GetString("cargo-console-paper-delivery-destination-default")
                                : order.DeliveryDestination), // CorvaxGoob-CargoFeatures
                        ("note",
                            string.IsNullOrWhiteSpace(order.Note)
                                ? Loc.GetString("cargo-console-paper-note-default")
                                : order.Note), // CorvaxGoob-CargoFeatures
    ...
    private bool FulfillOrder(CargoOrderData order,
        ProtoId<CargoAccountPrototype> account,
        EntityCoordinates spawn,
        string? paperProto) => FulfillOrder(order, account, spawn, paperProto, out _, out _);
    ...
    public bool TryGetOrderDatabase([NotNullWhen(true)] EntityUid? stationUid,
        [MaybeNullWhen(false)] out StationCargoOrderDatabaseComponent dbComp) => TryComp(stationUid, out dbComp);

    #endregion

    #region Interface

    private void OnApproveOrderMessage(EntityUid uid,
        ...
        CargoConsoleApproveOrderMessage args)
    {
        ...
        var order = orderDatabase.Orders[component.Account]
        ...
        if (order.Cooldown > 0)
        {
            if (orderDatabase.ProductCooldownTime.TryGetValue(order.ProductId, out var cooldownTime) &&
                cooldownTime > _timing.CurTime)
            {
                var timeLeft = cooldownTime - _timing.CurTime;
                (int count, string units) timeInfo =
                    timeLeft.Minutes > 0 ? (timeLeft.Minutes, "minutes") : (timeLeft.Seconds, "seconds");
                ConsolePopup(args.Actor,
                    Loc.GetString("cargo-console-cooldown-active",
                        ("product", order.ProductName),
                        ("timeCount", timeInfo.count),
                        ("timeUnits", timeInfo.units)));
                PlayDenySound(uid, component);
                return;
            }
    ...
    private EntityUid? TryFulfillOrder(Entity<StationDataComponent> stationData,
        ProtoId<CargoAccountPrototype> account,
    ...
    private void OnRemoveOrderMessage(EntityUid uid,
        CargoOrderConsoleComponent component,
    ...
    private void OnAddOrderMessageSlipPrinter(EntityUid uid,
        CargoOrderConsoleComponent component,
        CargoConsoleAddOrderMessage args,
        CargoProductPrototype product,
        string requester) // CorvaxGoob-CargoFeatures : Добавлено арг requester
    {
        ...
        msg.AddMarkupPermissive(Loc.GetString("cargo-acquisition-slip-body",
            ...
            ("amount", args.Amount),
            ("cost",
                product.Cost * args.Amount +
                (args.SecuredDelivery ? component.SecureOrderCost : 0)), // CorvaxGoob-CargoFeatures-Start : доп.цена
            ("orderer", requester),
            ("destination",
                args.DeliveryDestination ?? Loc.GetString("cargo-console-paper-delivery-destination-default")),
    ...
    private void OnAddOrderMessage(EntityUid uid,
        CargoOrderConsoleComponent component,
        CargoConsoleAddOrderMessage args)
    {
        if (args.Actor is not { Valid: true } player)
        ...
        if (args.SecuredDelivery) // CorvaxGoob-CargoFeatures : Валидация поля запроса
            args.SecuredDelivery = CanBeSecuredDelivery((uid, component),
                _protoMan.Index<CargoProductPrototype>(args.CargoProductId));
        ...
        {
            OnAddOrderMessageSlipPrinter(uid,
                component,
                args,
                product,
                requester); // CorvaxGoob-CargoFeatures : арг. requester
            return;
        }
        ...
        var data = GetOrderData(args,
            ...
            component.Account,
            requester,
            args.SecuredDelivery ? component.SecureOrderCost : default);
