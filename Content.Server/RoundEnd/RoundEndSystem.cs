...
public sealed class RoundEndSystem : EntitySystem
{
    ...
    private void SetAutoCallTime() => AutoCallStartTime = _gameTiming.CurTime;
    ...
    public EntityUid? GetStation()
    {
        AllEntityQuery<StationEmergencyShuttleComponent, StationDataComponent>()
            .MoveNext(out var uid, out _, out var data);
        if (data == null)
            return null;
    ...
    public bool CanCallOrRecall() => _cooldownTokenSource == null;

    public bool IsRoundEndRequested() => _countdownTokenSource != null;
            ...
            var payload = new NetworkPayload
            {
                ...
                [ShuttleTimerMasks.SourceTime] = countdownTime +
                                                 TimeSpan.FromSeconds(_shuttle.TransitTime +
                                                                      _cfg.GetCVar(CCVars.EmergencyShuttleDockTime)),
