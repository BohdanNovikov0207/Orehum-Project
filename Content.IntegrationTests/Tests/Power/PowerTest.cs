...
";
    ...
    public async Task ApcChargingTest()
    {
        ...
        await server.WaitAssertion(() =>
        {
            Assert.Multiple(() =>
            {
                Assert.That(substationNetBattery.CurrentSupply,
                    Is.GreaterThan(0)); //substation should be providing power
    ...
    public async Task ApcNetTest()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            var powerReceiverEnt =
                entityManager.SpawnEntity("ApcPowerReceiverDummy", grid.Owner.ToCoordinates(0, range - 1));
            receiver = entityManager.GetComponent<ApcPowerReceiverComponent>(powerReceiverEnt);

            // Create an unpowered receiver outside range
            var unpoweredReceiverEnt =
                entityManager.SpawnEntity("ApcPowerReceiverDummy", grid.Owner.ToCoordinates(0, range));
