...
";
    ...
    public async Task Test()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(entityManager.TryGetComponent(human, out cuffed!),
                ...
                Assert.That(entityManager.TryGetComponent(cuffs, out HandcuffComponent? _),
                    $"Handcuff has no {nameof(HandcuffComponent)}");
                Assert.That(entityManager.TryGetComponent(secondCuffs, out HandcuffComponent? _),
            ...
            Assert.That(cuffed.CuffedHandCount,
                Is.GreaterThan(0),
    ...
    private static void AddHand(NetEntity to, IServerConsoleHost host) => host.ExecuteCommand(null, $"addhand {to}");
}
