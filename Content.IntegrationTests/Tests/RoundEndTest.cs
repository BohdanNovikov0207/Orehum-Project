...
public sealed class RoundEndTest
{
    private sealed class RoundEndTestSystem : EntitySystem
    {
        ...
        private void OnRoundEnd(RoundEndSystemChangedEvent ev) => RoundCount += 1;
    }
    ...
    public async Task Test()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(roundEndSystem.ExpectedCountdownEnd,
                    Is.Not.Null,
                    "Shuttle was called, but countdown time was not set");
                Assert.That(roundEndSystem.CanCallOrRecall(),
                    Is.False,
