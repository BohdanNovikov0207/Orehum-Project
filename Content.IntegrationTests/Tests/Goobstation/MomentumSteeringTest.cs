...
public sealed class MomentumSteeringTest
{
    ...
    public async Task TestPerpPenalized() =>
        await _pair.Server.WaitAssertion(() =>
        {
            ...
            Assert.That(adjusted.X,
                Is.EqualTo(wishDir.X).Within(0.01f),
                "Forward component should be unpenalized");
            Assert.That(Math.Abs(adjusted.Y),
                Is.LessThan(wishDir.Y * 0.2f),
    ...
    public async Task TestBrakingFactor() =>
        await _pair.Server.WaitAssertion(() =>
        {
            ...
            Assert.That(result, Is.True);
            Assert.That(adjusted.X,
                Is.EqualTo(-_comp.BrakingFactor).Within(0.01f),
    ...
    public async Task TestMaxSpeedReduced() =>
        await _pair.Server.WaitAssertion(() =>
        {
            ...
            Assert.That(friction,
                Is.EqualTo(_comp.FrictionReductionAtSpeed).Within(0.01f),
