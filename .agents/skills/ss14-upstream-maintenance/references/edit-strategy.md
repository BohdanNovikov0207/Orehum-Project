...
- When creating fork-only behavior, prefer `_Orehum` paths that mirror the upstream feature layout.
- When adding or changing Orehum-specific code in a file outside any `_Orehum` path, wrap each Orehum-specific block with edit markers:
...
- Keep edit-marker ranges as narrow as practical. Do not wrap unrelated upstream code or whole files when only a small block belongs to Orehum.
- Multiple marked blocks in one file are allowed when the Orehum changes are separated by upstream code.
- For files that do not use `//` comments, use the native comment syntax while preserving `Orehum edit start` and `Orehum edit end`.
- Prefer reusable hooks, helpers, and data-driven content over one-off special cases or hardcoded values.

## Good Patterns

- Add fork-specific systems or resources beside the upstream feature in `_Orehum`.
- Extend an existing public system API instead of rewriting the whole feature.
- Mark narrow Orehum-specific edits in inherited files with paired `Orehum edit` comments.
...
- Leaving unmarked Orehum-specific code in files outside `_Orehum`.
