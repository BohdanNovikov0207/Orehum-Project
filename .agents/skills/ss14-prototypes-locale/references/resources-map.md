...
Use this file to place prototype, localization, texture, audio, and map edits in the correct `Resources/` subtree for the `orehum` fork.
...
- `_Orehum/`: fork-specific prototypes.
...
Current `_Orehum` resource content already includes:

- `Resources/Prototypes/_Orehum/Entities/Mobs/Customization/Markings/`: fork-specific marking prototypes.
- `Resources/Locale/en-US/_Orehum/commands/`: custom admin or server command strings.
- `Resources/Locale/en-US/_Orehum/markings/`: localized names for fork-specific markings.

When adding more fork-specific content, prefer extending the `_Orehum` subtree for both prototypes and locale so the fork delta stays discoverable.
