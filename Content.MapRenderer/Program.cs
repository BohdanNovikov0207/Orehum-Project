...
internal sealed class Program
{
    ...
    private static readonly Func<string, string> ChosenMapIdNotIntMessage =
        id => $"The chosen id is not a valid integer: {id}";
    ...
    internal static async Task Main(string[] args)
    {
        ...
        if (arguments.ArgumentsAreFileNames)
        {
            ...
            {
                Console.Write(
                    $"Following map files did not exist on disk directly, searching through prototypes: {string.Join(", ", lookupPrototypeFiles)}");
        ...
        ExternalTestContext testContext)
    {
        ...
        foreach (var map in toRender)
        {
            ...
            await painter.SetupView(arguments.ShowMarkers);
            ...
            try
            {
                await foreach (var renderedGrid in painter.Paint())
                {
                    ...
                    mapViewerData.Grids.Add(new GridLayer(renderedGrid,
                        Path.Combine(mapShort, Path.GetFileName(savePath))));
                    i++;
                }
            }
            catch (Exception ex)
