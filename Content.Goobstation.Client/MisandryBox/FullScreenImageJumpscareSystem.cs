...
public sealed class ClientFullScreenImageJumpscare : IFullScreenImageJumpscare, IPostInjectInit
{
    ...
    private void RegisterNetMessages() => _netManager.RegisterNetMessage<JumpscareMessage>(OnJumpscareMessage);
    ...
    {
        // Ram in
        texture.Modulate = Color.White.WithAlpha(1);
        ...
        {
            var alpha = i / 10f;
            texture.Modulate = Color.White.WithAlpha(alpha);
            await Task.Delay(75);
        }
