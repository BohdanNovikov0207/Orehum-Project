...
/// </summary>
public sealed class ResearchesContainerPanel : LayoutContainer
{
    protected override void Draw(DrawingHandleScreen handle)
    {
        foreach (var child in Children)
        {
            ...
            {
                var startCoords = new Vector2(item.PixelPosition.X + item.PixelWidth / 2,
                    item.PixelPosition.Y + item.PixelHeight / 2);
                var endCoords = new Vector2(second.PixelPosition.X + second.PixelWidth / 2,
                    second.PixelPosition.Y + second.PixelHeight / 2);
