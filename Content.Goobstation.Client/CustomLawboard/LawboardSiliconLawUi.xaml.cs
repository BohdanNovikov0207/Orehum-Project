...
public sealed partial class LawboardSiliconLawUi : FancyWindow
{
    ...
    public void SaveLaws()
    {
        ...
        Timer.Spawn(500,
            () => Save.Disabled =
                false); // This is so it feels like it did something. It's completely unnecessary but it adds some feedback

        ProcessLawsChanged(true);
    }

    public void ProcessLawsChanged(bool popup = false) =>
        LawsChangedEvent?.Invoke(_laws,
            popup); // The popup is just for flavor. There's probably a better way to do this
    ...
    public void ChangeLaw(SiliconLaw law)
    {
        ...
        if (law.LawString.Length > SharedCustomLawboardSystem.MaxLawLength)
            law.LawString =
                law.LawString
                [..SharedCustomLawboardSystem
                    .MaxLawLength]; // Makes the law have a max length to prevent the Horrors of a 9000 character law.
