...
public abstract class SharedSealableClothingSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SealableClothingControlComponent, ClothingControlSealCompleteEvent>(
            OnControlSealingComplete);
        ...
        SubscribeLocalEvent<SealableClothingControlComponent, ToggledBackClothingFullUnequipAndInsertedEvent>(
            OnBackClothingUnequipped);
        SubscribeLocalEvent<SealableClothingControlComponent, BeingUnequippedAttemptEvent>(
            OnToggleableUnequipAttemptSealCheck);
        SubscribeLocalEvent<SealableClothingControlComponent, OnToggleableUnequipAttemptEvent>(OnToggleSanityChecker);
        SubscribeLocalEvent<SealableClothingControlComponent, InventoryRelayedEvent<GetVerbsEvent<EquipmentVerb>>>(
            OnRelayedVerbRequest);

        SubscribeLocalEvent<SealableClothingComponent, OnAttachedUnequipAttemptEvent>(
            OnAttachedUnequipAttemptSealCheck);
    }
    ...
    public bool TryStartSealToggleProcess(Entity<SealableClothingControlComponent> control, EntityUid? user = null)
    {
        ...
        if (!comp.IsCurrentlySealed && _toggleableSystem.GetAttachedToggleStatus(wearer.Value, uid, false) !=
            ToggleableClothingAttachedStatus.AllToggled)
        {
    ...
    private void NextSealProcess(Entity<SealableClothingControlComponent> control)
    {
        ...
        while (true) // ugly but this used to be recursion so if we're doing this that way use iteration instead.
        {
            if (comp.WearerEntity is not { Valid: true } ||
                !comp.IsInProcess) // dont just fucking assume the fucking entity will never be null what if a dev gibbs you at 3 in the fucking morning.
                return;
            ...
            var doAfterArgs =
                new DoAfterArgs(EntityManager,
                    uid,
                    sealableComponent.SealingTime,
                    new SealClothingDoAfterEvent(),
                    uid,
                    processingPart,
                    showTo: comp.WearerEntity) { NeedHand = false, RequireCanInteract = false };
    ...
    private void OnBackClothingUnequipped(Entity<SealableClothingControlComponent> control,
        ref ToggledBackClothingFullUnequipAndInsertedEvent args)
    {
        var comp = control.Comp;

        // Check if it's in the middle of sealing/unsealing
        if (!comp.IsInProcess ||
            comp.UnequipAfterUnseal) // yes im looking at unequipafterunseal here to make sure we dont seal after untoggling with that bad way of doing it earlier.
            return;
    ...
    private void SealBreaker(EntityUid controlUid)
    {
        ...
        if (comp.WearerEntity == null)
            return;
        _popupSystem.PopupEntity(Loc.GetString(comp.SealBrokenPopup),
            controlUid,
            comp.WearerEntity.Value,
    ...
    private void OnAttachedUnequipAttemptSealCheck(Entity<SealableClothingComponent> attached,
        ref OnAttachedUnequipAttemptEvent args)
    {
        var toggleableEnt = args.Toggleable;
        // Cancel if the control unit is sealed.
        if (TryComp<SealableClothingControlComponent>(toggleableEnt, out var controlSeal) &&
            controlSeal.IsCurrentlySealed || controlSeal!.IsInProcess)
        {
            _popupSystem.PopupClient(Loc.GetString("sealable-clothing-sealed-toggle-fail"),
                toggleableEnt,
                args.UnEquipTarget);
            args.Cancel();
            return;
    ...
    private void OnToggleableUnequipAttemptSealCheck(Entity<SealableClothingControlComponent> toggleable,
    ...
    private void OnToggleSanityChecker(Entity<SealableClothingControlComponent> sealable,
        ref OnToggleableUnequipAttemptEvent args)
    {
    ...
    private void
        OnPartSealingComplete(Entity<SealableClothingComponent> part, ref ClothingPartSealCompleteEvent args) =>
    ...
    /// </summary>
    private void OnControlSealingComplete(Entity<SealableClothingControlComponent> control,
        ref ClothingControlSealCompleteEvent args)
    {
    ...
    /// </summary>
    private static void OnControlGetItemActions(Entity<SealableClothingControlComponent> control,
    ...
    private void OnRelayedVerbRequest(Entity<SealableClothingControlComponent> control,
        ref InventoryRelayedEvent<GetVerbsEvent<EquipmentVerb>> args) => OnEquipmentVerb(control, ref args.Args);

    /// <summary>
    /// Adds unsealing verbs to sealing control allowing other users to unseal/seal clothing via stripping
    /// </summary>
    private void OnEquipmentVerb(Entity<SealableClothingControlComponent> control,
    ...
    private void StartSealDoAfter(EntityUid user, Entity<SealableClothingControlComponent> control, EntityUid wearer)
    {
        ...
        var args = new DoAfterArgs(EntityManager,
            user,
            control.Comp.NonWearerSealingTime,
            new StartSealingProcessDoAfterEvent(),
            control,
            wearer,
            control)
        {
    ...
    private void OnStartSealingDoAfter(Entity<SealableClothingControlComponent> control,
        ref StartSealingProcessDoAfterEvent args)
    {
        if (args.Cancelled)
            return;
        var user = args.User;
    ...
    /// </summary>
    private void OnSealClothingDoAfter(Entity<SealableClothingControlComponent> control,
        ref SealClothingDoAfterEvent args)
    {
    ...
    public bool SealPart(Entity<SealableClothingComponent?> ent,
        Entity<SealableClothingControlComponent> control,
        bool silent)
    {
    ...
    /// </summary>
    private void OnToggleClothingAttempt(Entity<SealableClothingControlComponent> control,
        ref ToggleClothingAttemptEvent args)
    {
        var (uid, comp) = control;
        var wearer = control.Comp.WearerEntity;
        ...
        if (!comp.IsCurrentlySealed && toggleStatus == ToggleableClothingAttachedStatus.NoneToggled &&
            wearer != args.User)
