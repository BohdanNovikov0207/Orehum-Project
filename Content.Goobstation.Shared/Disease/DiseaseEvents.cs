...
/// <summary>
/// This event is raised on an entity just before it's infected. Set <see cref="CanInfect" /> to false to prevent the
/// infection.
/// </summary>
[ByRefEvent]
...
/// </summary>
[ByRefEvent]
public record struct DiseaseCheckConditionsEvent(
    DiseaseEffectComponent Comp,
    Entity<DiseaseComponent> Disease,
    Entity<DiseaseCarrierComponent> Ent,
    bool DoEffect = true);
...
/// </summary>
[ByRefEvent]
public record struct GetImmunityEvent(
    Entity<DiseaseComponent> Disease,
    float ImmunityGainRate = 0f,
    float ImmunityStrength = 0f);
...
/// </summary>
[ByRefEvent]
public record DiseaseOutgoingSpreadAttemptEvent(float Power, float Chance, ProtoId<DiseaseSpreadPrototype> Type)
    : DiseaseSpreadAttemptEvent(Power, Chance, Type);

/// <summary>
/// This event is raised on an entity to which a disease is trying to spread just before it attempts to do so.
/// </summary>
[ByRefEvent]
public record DiseaseIncomingSpreadAttemptEvent(float Power, float Chance, ProtoId<DiseaseSpreadPrototype> Type)
    : DiseaseSpreadAttemptEvent(Power, Chance, Type);
