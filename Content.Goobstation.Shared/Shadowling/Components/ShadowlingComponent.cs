...
public sealed partial class ShadowlingComponent : Component
{
    ...
    /// The first phase is the pre-hatch, where Shadowlings start with only their basic hatch ability and are disguised as
    /// their normal character.
    /// The second phase, "post-hatch", where Shadowlings transform into a seperate species with new abilities.
    /// The third phase, "ascension", where Shadowlings transform into the ascendant with new abilities, while having their
    /// previous removed.
    /// A failed phase, "failed ascension", where Shadowlings turn are slowed down and lose all their abilities. Only used for
    /// Ascension Egg.
    /// </summary>
    ...
    /// <summary>
    /// This is the damage the Shadowlings take once they try to shoot a gun. It is low, because it should just be an
    /// indication that
    ...
    /// It can be decreased once you Thrall someone, or once you use Black Recuperation (the values are changed there,
    /// however).
    /// Once a Thrall gets removed from the Shadowling's Thralls, this gets decreased from the overall resistance.
    /// The overall resistance exists in the system LightDetectionDamageModifier, which (for this case) handles taking damage
    /// while standing on light.
