...
public abstract class SharedShadowlingSystem : EntitySystem
{
    ...
    public bool CanEnthrall(EntityUid uid, EntityUid target)
    {
        ...
        {
            _popup.PopupPredicted(Loc.GetString("shadowling-enthrall-already-thrall"),
                uid,
                uid,
        ...
        if (!TryComp<MindControllableComponent>(target, out var mindControllable) ||
            mindControllable.ControlledBySomeone)
        {
            _popup.PopupPredicted(Loc.GetString("shadowling-enthrall-cant-be-controlled"),
                uid,
                uid,
                PopupType.SmallCaution);
            return false;
    ...
    private void OnExamined(EntityUid uid, ShadowlingComponent comp, ExaminedEvent args)
    {
        ...
        args.PushMarkup(Loc.GetString("shadowling-examine-self",
            ("damage", lightDet.ResistanceModifier * lightDet.DamageToDeal.GetTotal())));
    }
