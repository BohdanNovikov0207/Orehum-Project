...
public sealed class FirefighterTankRefillableSystem : EntitySystem
{
    ...
    private void OnFirefightingNozzleAfterInteract(Entity<FirefighterTankRefillableComponent> entity,
        ref AfterInteractEvent args)
    {
        ...
        if (entity.Comp.ExternalContainer)
        {
            ...
            if (!foundContainer &&
                _inventory.TryGetContainerSlotEnumerator(args.User, out var enumerator, entity.Comp.TargetSlot))
            {
        ...
        if (_solutionContainerSystem.TryGetDrainableSolution(target, out var targetSoln, out var targetSolution)
            && _solutionContainerSystem.TryGetSolution(sprayOwner,
                solutionName,
                out var solutionComp,
                out var atmosBackpackTankSolution))
        {
            ...
            if (trans > 0)
            {
                ...
                _audioSystem.PlayPredicted(entity.Comp.FirefightingNozzleRefill, entity, args.User);
                _popup.PopupClient(Loc.GetString("firefighter-nozzle-component-after-interact-refilled-message"),
                    entity,
                    args.User);
            }
                ...
                _popup.PopupClient(
                    Loc.GetString("firefighter-nozzle-component-no-water-in-tank", ("owner", args.Target)),
                    entity,
                    args.User);
