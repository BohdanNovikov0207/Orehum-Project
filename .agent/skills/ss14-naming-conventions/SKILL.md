...
4. MUST: if the entity is unique to the fork and especially if it is a forked copy of the vanilla entity, add the fork prefix (`OrehumXxx`).
...
3. MUST: if the variable stores a component, use the base part of the component name (`ActiveSuperFlashComponent` -> `superFlash`).
...
5. Are you creating a fork-only or fork copy of vanilla?
   Add a fork prefix to the ID (`Orehum*`).
...
8. ID in the style `Orehum_ActionSummonGun` with a clear domain prefix.
9. Fork ID with the prefix `Orehum*` for unique fork content.
...
12. Private component field `SuperFlash` according to the basic part of the type.
...
```yaml
- type: entity
  id: Orehum_ActionSummonGun
  name: Summon Gun
```

Comment: `Orehum` prefix and `CamelCase` are respected; action name in English.

### Example 7: Orehum prefix in fork content
