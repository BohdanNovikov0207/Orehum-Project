...
5. In `SharedVehicleSystem` the logic is decomposed into partial parts with separate blocks of responsibilities and dependencies.
6. In YAML directories, level separators `# Rank 2` / `# Rank 3` make large lists readable without overload.
7. In FTL, the correct group separator is `## Strings for the battery ...` and does not break parsing.
...
6. Edit markers (`Orehum-Start/End`, `Orehum added start/end`) instead of behavioral description.
