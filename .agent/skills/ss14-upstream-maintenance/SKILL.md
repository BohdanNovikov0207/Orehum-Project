...
description: Guide to working with Space Station 14 forks with project-folder pattern (`_Orehum`) to minimize merge conflicts with the upstream. Use when modifying vanilla code or prototypes.
---

# 🛡️ Working with Upstream code and minimizing conflicts

This skill describes the standards and patterns adopted in Space Station 14 forks with project-folder isolation (`_Orehum`) for working with code inherited from the upstream.
...
To clearly separate vanilla code from our modifications, a special project folder is used, starting with the symbol `_` (for example, _`_Orehum`).
...
// SERVERNAME edit end
```
*Where `SERVERNAME` is the project name (for example, `Orehum`).*

**Example (change value):**
```csharp
component.Field2 = 321;
// Orehum edit start - increasing radius for balance
component.Field = 123;
// Orehum edit end
```
...
```csharp
// Orehum edit start - fixing double gateways
...
**Example:**
```csharp
// Orehum added start - to know when the shy one was hit
...
Instead, create a `partial` class in your project folder (`_Orehum`).

**Pattern:**
1. Find the vanilla class (eg `SharedDoAfterSystem`).
2. Create a file in your folder: `Content.Shared/_Orehum/DoAfter/SharedDoAfterSystem.Update.cs`.
3. Declare the class as `partial` with the same namespace.
4. **Important:** Suppress the namespace mismatch warning if necessary.

**Example:**
*Vanilla file (`Content.Shared/DoAfter/SharedDoAfterSystem.cs`):*
```csharp
...
*Your file (`Content.Shared/_Orehum/DoAfter/SharedDoAfterSystem.cs`):*
```csharp
using Content.Shared.DoAfter; // We use vanilla namespace
...
2. Create a **new** YAML file in your project folder (for example, `Resources/Prototypes/_Orehum_/.../access.yml`).
...
*1. New prototype (`Orehum/Entities/Structures/Doors/Airlocks/access.yml`):*
```yaml
- type: entity
  parent: AirlockHatchSyndicate  # Inherit from the original
  id: AirlockHatchSyndicateLocked # New ID
  suffix: Syndicate, Locked
  categories: [ HideSpawnMenu ] # Orehum added - hide from spawn if this is a technical entity
  components:
  - type: AccessReader
