...
description: Creation, copying, renaming, migration and review of SS14 loadout prototypes (`roleLoadout`, `loadoutGroup`, `loadout`) in fork folders like `_Orehum`: naming, isolation from vanilla, transfer of mandatory equipment from `startingGear`, `startingGear` synchronization and processing of hidden jobs/departments. Use when changing job prototypes and any loadout system files.
---
...
1. `loadout`: `Loadout` + `ItemPrototypeId`.
2. `loadoutGroup`: `<ForkName(Orehum)>LoadoutGroup` + slot name or generic group name.
3. `roleLoadout`: `Job<RoleId>`.
4. If the same `ItemPrototypeId` is needed in two different contexts and without the suffix there will be a collision, add a short context tail: `LoadoutFlashlightLanternCargo`, `LoadoutFlashlightLanternEngineering`.
...
3. Don't put copies in technical dumps like `Imports/Orehum` if you can put them in a normal mirror path.
...
3. Give `loadout` ID in the format `Loadout<ItemPrototypeId>`.
4. Give `loadoutGroup` ID in the format `LoadoutGroup<SlotOrConcept>`, and not according to the historical copy source.
...
1. Leave the `_ForkName` role to refer to the vanilla one or `_Orehum` `loadoutGroup`.
...
```yaml
- type: loadout
  id: LoadoutKeyCardMedicalSpecialist
  equipment:
    id: KeyCardMedicalSpecialist

- type: loadoutGroup
  id: LoadoutGroupIdKeyCardMedicalSpecialist
  name: loadout-group-id
  minLimit: 1
  loadouts:
  - LoadoutKeyCardMedicalSpecialist

- type: roleLoadout
  id: JobMedicalSpecialist
  groups:
  - LoadoutGroupIdKeyCardMedicalSpecialist
```
...
```yaml
- type: loadoutGroup
  id: LoadoutGroupMedicalShoes
  name: loadout-group-shoes
  loadouts:
  - LoadoutClothingShoesColorWhite
  - LoadoutClothingShoesBootsWinterMed
...
```yaml
- type: loadoutGroup
  id: LoadoutGroupSecurityBackpack
  name: loadout-group-backpack
  loadouts:
  - LoadoutClothingBackpackSecurity
  - LoadoutClothingBackpackSatchelSecurity
  - LoadoutClothingBackpackDuffelSecurity
```
...
1. All new IDs correspond to the scheme `Loadout*` / `LoadoutGroup*` / `Job<RoleId>`.
