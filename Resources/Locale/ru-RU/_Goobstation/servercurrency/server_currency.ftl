...
gs-balanceui-shop-buy-token-admin-abuse = Купить токен на перерождение - { $price } Орешков
...
gs-balanceui-shop-token-admin-abuse = Токен перерождения
...
gs-balanceui-shop-buy-token-admin-abuse-desc = Позволяет попросить высших сил переродить вас.
...
gs-balanceui-remark-token-admin-abuse = Куплен токен перерождения.
